// commit-brief-apply-artifact.test.mjs -- shell-level proof for the extracted push-degradation script
// (task 6.1b, fix round 1, review finding 1, Critical). Drives the REAL script as a subprocess against
// disposable local git sandboxes (a work repo + a bare "origin"), the same execFileSync-subprocess
// pattern apply-record-briefs.test.mjs's own CLI integration tests already use for this family. No
// network, no GitHub Actions runner needed: a local bare repo with a `pre-receive` hook that always
// rejects is a faithful stand-in for a branch-protection rejection (git sees the identical exit-nonzero
// refusal either way), and a `pre-commit` hook that always rejects stands in for "the commit itself could
// not be made".
...
const SCRIPT_PATH = join(HERE, "commit-brief-apply-artifact.sh");
...
function makeWorkRepo({ withOrigin = false, originRejects = false } = {}) {
  const root = mkdtempSync(join(tmpdir(), "commit-brief-apply-artifact-test-"));
...
function withDirtyArtifact(work, dir = "scripts/harness-runs/brief-apply") {
  const full = join(work, dir);
  mkdirSync(full, { recursive: true });
  writeFileSync(join(full, "brief-apply-run-999.json"), JSON.stringify({ ok: true }));
}

test("nothing staged: exits 0, never commits, prints the no-op message", () => {
  const { root, work } = makeWorkRepo();
  try {
    const res = sh(["main", "12345", "scripts/harness-runs/brief-apply/"], { cwd: work });
    assert.equal(res.status, 0);
    assert.match(res.stdout, /No new brief-apply run artifact to commit/);
...
test("ref is master: commits locally, skips the push attempt entirely, exits 0 with a named warning", () => {
  ...
  try {
    ...
    const res = sh(["master", "999001", "scripts/harness-runs/brief-apply/"], { cwd: work });
    ...
    assert.match(res.stdout, /uploaded workflow artifact/);
    // Committed locally...
    assert.match(git(["log", "-1", "--format=%s"], work), /brief-apply: run 999001 artifact/);
    ...
    assert.doesNotMatch(remoteLog, /brief-apply: run 999001 artifact/, "master guard must never attempt the push");
  } finally {
    cleanup(root);
  }
});
...
test("ref is a lane branch, origin accepts: commits AND pushes, exits 0", () => {
  ...
  try {
    ...
    withDirtyArtifact(work);
    const res = sh(["lane/example", "999002", "scripts/harness-runs/brief-apply/"], { cwd: work });
    assert.equal(res.status, 0, `expected exit 0, got ${res.status}: ${res.stdout}${res.stderr}`);
    assert.match(res.stdout, /pushed to 'lane\/example'/);
    const remoteLog = git(["log", "--oneline", "lane/example"], bare);
    assert.match(remoteLog, /brief-apply: run 999002 artifact/, "the artifact commit must reach origin");
  } finally {
    cleanup(root);
  }
});
...
test("push persistently rejected (protected-branch stand-in): degrades to a warning, exits 0, NEVER 1", () => {
  ...
  try {
    git(["checkout", "-q", "-b", "lane/example"], work);
    // The bare remote's pre-receive hook rejects everything, including this branch push -- expected to
    // fail here too (this call is not part of the script under test); ignore its own failure.
    try {
      git(["push", "-q", "-u", "origin", "lane/example"], work);
    } catch {
    ...
    withDirtyArtifact(work);
    const res = sh(["lane/example", "999003", "scripts/harness-runs/brief-apply/"], { cwd: work });
    ...
    assert.match(git(["log", "-1", "--format=%s"], work), /brief-apply: run 999003 artifact/);
  } finally {
    cleanup(root);
  }
});
...
test("the commit itself cannot be made: exits 1 (the one case this script still fails the run over)", () => {
  ...
  try {
    ...
    chmodSync(hookPath, 0o755);
    withDirtyArtifact(work);
    const res = sh(["lane/example", "999004", "scripts/harness-runs/brief-apply/"], { cwd: work });
