...
//     group's `row_ids`, deduped) via `coverage_gap_candidates`' own selected columns, and reports each
//     row's post-write `disposition`. dry mode reports the underlying script's own per-group plan
//     unmodified (`result.results`).
//
// USAGE (by hand, needs DB creds):
//   node scripts/maintenance/review-apply-coverage-gaps.mjs --mode dry --arg docs/ratifications/2026-09/coverage-gaps.ruling.json
...
export async function main({ mode = "dry", arg = "" } = {}, deps) {
  ...
  if (!rulingArg) {
    summary.note =
      ...
      "Example: --arg docs/ratifications/2026-09/coverage-gaps.ruling.json (build it first with " +
