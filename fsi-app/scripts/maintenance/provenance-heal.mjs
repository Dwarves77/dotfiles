...
// ELEVENTH PASS (2026-09-05, lane ATTACH-SOURCES, W3.1): exported (was previously the anonymous
// `buildDeps` runCli() called inline below) so scripts/maintenance/attach-found-sources.mjs — a second,
// SEPARATE MAINT step that also dispatches heal-provenance.mjs's own guarded main() over a Haiku-filled
// worklist — reuses this EXACT same ~110-line real-DB/fetch wiring rather than a second copy of it. One
// guarded write path (rule 015), one place this repo builds heal-provenance's deps. Byte-identical to the
...
if (IS_MAIN) {
  await runCli({ step: "provenance-heal", main: healMain, needsDb: true, buildDeps: buildHealDeps });
}
