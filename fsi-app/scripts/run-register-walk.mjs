// run-register-walk.mjs — B2 + B3 runner (scrape-and-build plan, 2026-07-19): walk an index that
// enumerates instruments — a register's dated index (B2: EUR-Lex OJ daily views / Federal Register
// documents API) or a publisher feed (B3: RSS/Atom) — and feed the SAME ledger B1 consumes.
// Free HTTP only (the FR API needs no key; OJ daily views and feeds are plain fetches), hold-gated
// with the F16 manual caller. Consume the results with scripts/run-portal-harvest.mjs --consume.
//
// Usage:
...
//   node scripts/run-register-walk.mjs --register feed --feed <https feed url>
//   [--source <uuid|url-substring>]  attach candidates to this registered source (defaults: the
//        register's root portal row; for feeds, the feed URL's host)
//
...
const FEED = flagOpt("feed", null);
const usageOk = REGISTER === "feed" ? !!FEED : !!(REGISTER && FROM && TO);
if (!usageOk) {
  console.error("usage: run-register-walk.mjs --register eurlex-oj|federal-register --from YYYY-MM-DD --to YYYY-MM-DD [--series L] [--types RULE] [--term x] [--per-page 100] [--max-pages 5] [--source <uuid|url-substring>]\n       run-register-walk.mjs --register feed --feed <https feed url> [--source <uuid|url-substring>]");
  process.exit(2);
}
if (REGISTER === "feed" && !/^https:\/\//i.test(FEED)) { console.error(`--feed must be an https URL`); process.exit(2); }
...
const { walkFeed } = await jiti.import("../src/lib/sources/feed-walk.mjs");
...
// ── resolve the source the candidates attach to (defaults: the register's root portal row; for a
//    feed, the feed URL's host — the publisher's registered row) ─────────────────────────────────────
const DEFAULT_SOURCE_MATCH = {
  "eurlex-oj": "eur-lex.europa.eu/",
  "federal-register": "federalregister.gov/",
  feed: FEED ? new URL(FEED).host.replace(/^www\./, "") : null,
};
...
console.log(`register=${REGISTER}  ${REGISTER === "feed" ? FEED : `${FROM}..${TO}`}  → source: ${source.name ?? "(unnamed)"}  ${source.url}  [${source.id}]`);
...
if (REGISTER === "eurlex-oj") {
...
} else if (REGISTER === "feed") {
  summary = await walkFeed(
    {
      fetchText: async (url) => {
        assertFetchAllowed(url, process.env, MANUAL_INTAKE_CALLER);
        const r = await fetch(url, { headers: { ...ua, accept: "application/rss+xml, application/atom+xml, application/xml, text/xml, */*;q=0.5" }, redirect: "follow", signal: AbortSignal.timeout(20_000) });
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        return r.text();
      },
      persist,
  ...
  if (!summary.ok) { console.error(`feed walk INCONCLUSIVE: ${summary.error}`); process.exit(1); }
  console.log(`  entries=${summary.entries} upserted=${summary.upserted}`);
