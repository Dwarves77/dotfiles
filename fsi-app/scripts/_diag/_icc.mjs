...
for(const r of (await c.query(`SELECT conname, pg_get_constraintdef(oid) d FROM pg_constraint WHERE conrelid='intelligence_changes'::regclass AND contype='c'`)).rows) console.log(`${r.conname}:\n  ${r.d}`);
await c.end();
