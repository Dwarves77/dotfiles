...
async function versionOutGuarded(c, itemId, proof, cite) {
  ...
  const versionRow = {
    current_claim_id: null, // proven_inaccurate soft-ref is null (history survives the row delete)
    ...
    version_number: vnum, supersede_reason: "proven_inaccurate", inaccuracy_proof: proof,
...
const it = (await readAll("intelligence_items", "id,legacy_id,title,item_type,source_url", {})).find((x) => x.id.startsWith(key) || x.legacy_id === key || x.id === key);
...
for (const c of toVersionOut) {
  const proof = { reason: "span_absent_from_verified_primary", detail: `source_span not present in the item's verified primary capture (${it.source_url}); proven inaccurate for this item (cross-instrument conflation) per operator ruling 2026-07-16`, primary_url: it.source_url, span_excerpt: String(c.source_span).slice(0, 200) };
  try { const v = await versionOutGuarded(c, it.id, proof, cite); done++; console.log(`  versioned out ${c.id.slice(0, 8)} -> claim_versions v${v}`); }
...
// each version-out delete fires set_provenance_status, which re-runs validate and flips quarantined ->
// verified off the post-delete ledger. No nudge write needed.
...
console.log(`\nversioned out ${done}/${toVersionOut.length}. validate: valid=${val?.valid} status=${fin?.provenance_status} failures=${JSON.stringify(val?.failures || []).slice(0, 200)}`);
process.exit(0);
