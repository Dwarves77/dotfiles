...
import { canonicalGenerate, textOf } from "../lib/anthropic.mjs"; // rule 016: the canonical script-side Anthropic path (spend visibility + caps)
...
const CITE = { skill: "environmental-policy-and-innovation", reason: "unit3 fail-closed census classification: write dryrun_disposition verdicts onto census_worklist" };
...
async function classify(title, row) {
  ...
  const j = await canonicalGenerate({ model: HAIKU, maxTokens: 260, system: SYS, messages: [{ role: "user", content: user }] });
  const cost = (j.usage?.input_tokens || 0) * IN_RATE + (j.usage?.output_tokens || 0) * OUT_RATE;
  const m = (textOf(j) || "").match(/\{[\s\S]*\}/);
...
async function main() {
  ...
  // Rule 015: guardedUpdate snapshots the prior row + carries the skill cite, and THROWS on rejection —
  // the same loud-failure contract the raw upd() had, now reversible.
  async function upd(id, patch) { await guardedUpdate("census_worklist", (qb) => qb.eq("id", id), patch, { cite: CITE }); }
