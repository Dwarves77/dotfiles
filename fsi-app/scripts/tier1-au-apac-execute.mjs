 ...
 *
 * RESPAWN of prior PR #65 (closed) per Jason's broader-scope authorization
 * dated 2026-05-07. The prior dispatch authorized SG retag for ~5 rows; the
 * investigation surfaced 9 SG-content rows with jurisdiction_iso=[]. Jason
 * has explicitly authorized retag for all 9.
 *
 * Authorized scope (2026-05-07):
 *
 ...
 *
 *   PLUS SG retag-in-place on all 9 SG-content rows with
 *   jurisdiction_iso=[] (4 MPA + 1 MOT + NEA + MSE + greenplan + Customs).
 *
 *   Mirrors Wave 2 cleanup retag-in-place pattern (jurisdiction_iso only —
 *   never touches status, tier, processing_paused, admin_only, or any other
 *   field). NEA carbon-tax stays status=provisional. Customs stays
 *   status=provisional. All others preserved as-found.
 *
 * Halt thresholds (much narrower than prior dispatch per Jason's auth):
 *   - SG retag candidate count differs from 9 by more than 2 (data drift)
 *   - AU/APAC pre-existing rows at unexpected tier or admin_only=true
 *   - Insert count > 28
 *   - Per-step verification failure on any single write
 *
 *   Explicitly does NOT halt on the 5-vs-9 SG count discrepancy from the
 *   prior dispatch — the broader 9-row retag is authorized.
 *
 * Per per-step verification contract (PR-A1 pattern), every insert and
 * every retag is verified post-write with read-back.
 */
...
// (correct tier, admin_only=false). Halt if drift detected.
{
  const PRE_EXISTING = [
...
// ─── Step 0.5: SG retag-candidate enumeration (broader 9-row scope) ───
// Pull all SG-domain candidate sources and filter for jurisdiction_iso=[].
// Halt only if observed count differs from 9 by more than 2.
const sgRetagCandidates = [];
{
  ...
  // Belt-and-braces: also pull rows where name/notes contain "Singapore" and
  // jurisdiction_iso is empty, in case any SG-content row sits on a
  // non-.gov.sg URL (e.g. bilateral portals or aggregator hosts).
  const { data: sgByText } = await supabase
    .from("sources")
    .select("id, name, url, tier, status, admin_only, jurisdiction_iso, notes")
    .or("name.ilike.%Singapore%,notes.ilike.%Singapore%");

  const dedup = new Map();
  for (const r of [...(sgBroad ?? []), ...(sgByText ?? [])]) {
    dedup.set(r.id, r);
  }
  ...
  // Halt gate: |count - 9| > 2 ⇒ HALT (data drift since investigation)
  const diverges = Math.abs(count - 9) > 2;
  step(
    "sg_retag_count_gate",
    !diverges,
    `${count} candidates (expected 9 per Jason auth, halt-threshold |Δ|>2)`
  );
}
...
// ─── Cap check: must be exactly 25 inserts (hard halt at >28) ──────────
step(
  "insert_count_cap",
  insertedIds.length === 25 && insertedIds.length <= 28,
  `inserted ${insertedIds.length} rows (expected 25, hard cap 28)`
);
...
// tier, admin_only, processing_paused, and all other fields preserved.
console.log("\n=== SG RETAG-IN-PLACE ===\n");
...
console.log(
  `\n[OK] Tier 1 AU + APAC writes complete. ${insertedIds.length} inserts, ${retaggedRows.length} SG retags. Log: docs/tier1-au-apac-execute-log.json`
);
