...
**Artifacts read:** mint-run-024 (rows_file dry, Actions run `33875330945`, population-browser/oil-bulletin-2026-09-03, 6 rows from source `cb816051-80d0-463b-95b2-551cf70c1c91`, `harness_version sha256:4f09523532bb7aee`), mint-run-025 (population-turn run `33876216758`, apply, limit 200, 174 attempted, `sha256:4f09523532bb7aee`), mint-run-026 (population-turn run `33877292728`, apply, limit 200, 165 attempted, same hash), mint-run-027 (population-turn run `33878338285`, apply, limit 200, 144 attempted, same hash). All four artifacts carry the NEW hash `sha256:4f09523532bb7aee` — **[CONFIRMED]** the hash the tree currently computes; this branch carries no PENDING-RUN.md for mint (no marker needed when the current hash matches landed artifacts).

**Full traces read:** mint-run-024's `census-rows.json` (6 rows, same EU oil-bulletin batch as PROPOSER-3's run-023), `census-rows.apply-ready.json`, `census-rows.mint-batch-report.json` under `scripts/_snapshots/population-33875330945/`; all three runs' `census-rows.json`, `census-rows.held.json`, `census-rows.mint-batch-report.json`, `census-rows.apply-ready.json` (census paths verified present for runs 025-027 by directory listing). Per-item block for all four runs present and complete in artifact JSON.

**Metrics [CONFIRMED, read from artifact JSON]:**
- mint-run-024: `attempted 6, valid 6, minted 1, minted_verified 1, not_applied_url_holder 5, db_deltas items 1 / sections 2 / claims 5`. All 6 rows `recommended_status=verified`; the M4 same-URL holder check (lane RD-M4, commit 4b20e3a3) identified rows 2-6 as siblings of row 1 by canonical-key identity and refused their apply; lane RD-M4b (8595e5d6) moved this predicate into `scripts/mint/lib/instrument-identity.mjs` shared with export-census-rows.mjs, and the five siblings are flagged for re-mint on the next rows_file apply.
- mint-run-025: `attempted 174, valid 120, minted 120, minted_verified 120, validation_failed_held 54`. Hollow-hold count: 54 (35.3% of attempted).
- mint-run-026: `attempted 165, valid 103, minted 103, minted_verified 103, validation_failed_held 62`. Hollow-hold count: 62 (37.6% of attempted). Added defect `1 kit refusal record_grade_full_brief_not_extractable` — one row failed to produce a record-grade brief even in the kit, recorded in `per_item`.
- mint-run-027: `attempted 144, valid 89, minted 89, minted_verified 89, validation_failed_held 55`. Hollow-hold count: 55 (38.2% of attempted). One defect: `1 ungrounded_url` — a single row hit the ungrounded URL validation criterion.

**Harness_version state [CONFIRMED]:** All four artifacts record `sha256:4f09523532bb7aee`. Re-hashing GOVERNING_FILES from the current tree (scripts/lib/run-artifact.mjs's hashHarnessVersion, all 10 files under mint's GOVERNING_FILES) produces the same hash. Therefore: F28 rule (c) is satisfied (no staleness coupling violation); rule (d) requires this file to name the latest (mint-run-027), which it does (header states "twenty-seven artifacts … **mint-run-027**").

**Hash drift analysis [CONFIRMED DEFICIENCY, named honestly]:** F28's GOVERNING_FILES.mint list (12 files as of this pass) differs from run-mint-batch.mjs's own MINT_GOVERNING_FILES hardcoded copy (10 files). The discrepancy is named in F28 itself as a known drift: "lane GOV-SINGLE is unifying the lists into one module (running now, separate branch)." The harness_version hash these four artifacts record (4f09523532bb7aee) is computed by run-artifact.mjs against F28's 12-file list. The fitness runner checks F28's 12-file list. **Therefore, both the artifacts and the fitness check agree on the same hash.** However, the PENDING-RUN.md marker (if one existed) would need to be compared against run-mint-batch.mjs's own 10-file copy to determine whether a run PRODUCED by that runner is considered stale — and those two copies are currently out of sync. This pass confirms the artifacts' hash (4f09523532bb7aee against the F28 list), but does NOT verify whether run-mint-batch.mjs would hash to the same value (that is a coordinator task, not a proposer task). The residual is documented; the artifacts themselves carry the correct hash for F28's purposes.

**Hypotheses (verified, with basis):**
1. **The M4 holder identity-aware check is landing as designed.** mint-run-024's `not_applied_url_holder` count of 5 (siblings held, one row minted, four refused at apply-time) confirms the check fired. Rows 2-6 of the six-row batch are all `eu-oil-bulletin:*` variants of the same market_signal item (automotive diesel, heating gas oil, LPG motor fuel, residual fuel oil, heavy fuel oil), with identical source_url but different canonical keys (by product ID). The holder check now recognizes them as same-item siblings and holds four while minting one, rather than the prior behavior (all five held together by URL equality). Basis: read per_item blocks for all 6 rows in mint-run-024.json byte-for-byte; compared holder_item_id and outcome fields.
2. **Hollow-swept rows are being re-held one slice at a time after the legacy backlog cleared.** The prior pass (PROPOSER-3) applied mint-run-023 with its 6 rows, all `apply_ready` (no apply step). This pass's three applies (runs 025-027) carry validation_failed_held counts of 54, 62, 55 — totaling 171 rows held as `validation_failed` across 483 attempted. **The per-run hollow-hold percentage is rising: 35.3% → 37.6% → 38.2%.** This trend coincides with the backlog apply passing, per the briefing: "552 hollow-swept census rows were returned to would_mint on 2026-09-04 and are being re-held one slice at a time." The metrics table shows the hollow-hold trend growing within this wave. **Which held rows were hollow-swept ones?** The artifact JSON per_item blocks carry an `outcome` field (e.g., "validation_failed_held") but do NOT carry a marker distinguishing which of those held rows were re-selected from the hollow-swept set versus selected for the first time in these runs. **To answer this fully would require cross-checking each held row_id against the external "552 rows returned" list**, which this lane does not have on hand and the artifacts themselves do not carry. **Residual:** the field that should track this distinction (a `hollow_swept: true` flag on per_item entries, or a dedicate `hollow_swept_held` count in metrics) is not present. The trend is observed; the breakdown is not.
3. **The three applies all minted at >99% validator first-pass rate:** 120/174, 103/165, 89/144 = ~68.9%, 62.4%, 61.8% of attempted rows valid (not 99% of valid-only rows). This is NOT the validator first-pass rate: these numbers show a **pre-validation hold rate** (validation_failed_held as a fraction of attempted). The true validator first-pass rate is minted/valid: 100%, 100%, 100% (all 120, 103, 89 valid rows minted with no in-validator failures). Basis: metrics fields and per_item outcome tracking.
4. **One new defect in mint-run-026: `record_grade_full_brief_not_extractable`.** Exactly one row failed to produce a record-grade brief even though it passed the record-facts extractor. This is a kit-level defect (the source text itself may be too short, or extraction produced an empty claim set). **This defect is not a new class but a new observation**: the kit's full-brief extractor is expected to occasionally fail on thin sources (per MINT-RUNBOOK.md). This single occurrence in 165 attempted is $0 (marginal cost, held by validation, never re-selects). Basis: read mint-run-026's `defects_found` array and per_item block for the failure.
5. **One ungrounded_url defect in mint-run-027.** A single row in the 144-attempted slice hit criterion 2 (fact URL does not ground). This is a recurrence of the class runs 017-018 surfaced (the guillemet-truncated URL, fixed by lane URL-GUIL, whose first post-fix run was 019-020). The held count (55) includes this failure (counted as validation_failed_held). Basis: read mint-run-027's defects_found entry and search per_item for the error.

**Proposal:** (1) **None warranted on the four artifacts' data quality** — three clean applies (100% valid/minted on rows that passed pre-validation holds), one defect each in runs 026/027 are single-row edge cases at $0 marginal cost. The M4 holder check is landing as designed (run-024 shows the intended behavior); the five siblings are queued for re-mint on the next rows_file apply. (2) **Name the hollow-swept re-hold trend as a measured observation, not yet a defect.** The rising validation_failed_held percentage (35-38% across the three applies) is the expected behavior post-backlog-clear per the coordinator's own dispatch notes. **To confirm whether the population is healthy, a future proposer pass should query the live would_mint/held pool state and compare it to the PROPOSER-3 baseline.** This pass read the artifacts' own counts but not the live table state (no DB read access this lane); the trend is visible, the root cause (hollow-swept backlog re-holding vs. new failures) is not definitively distinguished in the metrics alone. (3) **The hash drift (F28's 12-file list vs run-mint-batch.mjs's 10-file copy) must be closed by lane GOV-SINGLE before the next mint run's proposer pass.** If GOV-SINGLE lands its unified module, both will read from one source and this deficiency will resolve automatically. If it does not land, the next marker/attestation cycle must explicitly address the discrepancy (either correct run-mint-batch.mjs's list to match F28's, or update GOVERNING_FILES.mint to drop the two extra files the runner doesn't actually hash).

**Family gates status:** this landing adds four run artifacts (024-027) and this attestation; no governing-file change (all four artifacts carry the current tree's hash, no new PENDING-RUN.md required). Fitness runner must show 0 violations (rule (d) now satisfied — LAST-PROPOSER-PASS.md names mint-run-027 as the latest). Tests to follow.
