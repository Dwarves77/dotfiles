...
**Harness_version state [CONFIRMED, coordinator, measured on this branch with `hashHarnessVersion` from `scripts/lib/run-artifact.mjs`]:** all four artifacts record `sha256:4f09523532bb7aee`. That is the hash of `run-mint-batch.mjs`'s OWN `MINT_GOVERNING_FILES` copy as it stood on master `f2800ea9`, NOT the hash F28 computes: on this branch F28's `GOVERNING_FILES.mint` hashes to `sha256:714d22dadb03b8a1` (the value `mint/PENDING-RUN.md` is pinned at after RD-M4/RD-M4b moved MINT-RUNBOOK.md) and the runner's copy hashes to `sha256:6f805846c97cf4a4`. The two lists are hand-synced copies and have drifted (F28's carries `src/lib/agent/gate-a-scan.mjs` and `gate-a-match.mjs` since #580; the runner's does not). Consequence: under F28 rule (c) no mint artifact produced by the current runner can ever discharge the mint marker, because the runner never stamps the hash the gate computes. Lane GOV-SINGLE (same wave, separate branch) makes every family runner and F28 import one table; until it lands, the mint marker stays and these four artifacts' `harness_version` is a runner-copy hash, historical and git-traceable but not gate-comparable. The first draft of this pass stated the opposite ([CONFIRMED] that the tree re-hashes to `4f09523532bb7aee`); that statement was wrong and is withdrawn here rather than edited away, per PROPOSER-RUNBOOK §5.

**Hash drift analysis:** superseded by the corrected paragraph above.
