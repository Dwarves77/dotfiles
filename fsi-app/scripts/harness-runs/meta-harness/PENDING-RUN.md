...
**harness_version at write time:** `sha256:7fd76beaa3b4eb6d` (recomputed via `hashHarnessVersion` against the CURRENT `GOVERNING_FILES['meta-harness']`, 18 entries: the five files above plus the 13 family.json paths, sorted).
...
**Known open item, named honestly (lane N2).** Adding a `family.json` descriptor directly inside each family directory also changed what F28's own `scanArtifacts` (`.discipline/fitness/functions/F28-harness-run-integrity.mjs`, a file this lane's write set forbids editing) treats as a run-artifact candidate: its family-level glob matches any `<family>/<file>.json`, so it now also matches `<family>/family.json` and reports it as an INVALID ARTIFACT (missing every run-artifact field) for all thirteen families. `scripts/lib/run-artifact.mjs`'s own `readRunHistory` was fixed in this same lane to exclude the literal filename `family.json` from its own `*.json` glob (in this lane's write set), but F28-harness-run-integrity.mjs's separate `scanArtifacts` re-implementation was not, since it is outside this lane's write set. This is reported in the lane's own report, not worked around here; the fix is a one-line exclusion in `scanArtifacts` (skip the literal filename `family.json`, mirroring `readRunHistory`'s own fix) for a follow-up lane with write access to that file.
