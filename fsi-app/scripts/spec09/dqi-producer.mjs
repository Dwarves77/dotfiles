#!/usr/bin/env node
// dqi-producer.mjs — spec 09 §1.4, tce_data_quality (migration 297, org-scoped by a follow-up migration
// this lane — see docs/inventories/shared-dataset-ownership.md).
//
...
const TABLE = "tce_data_quality";

export const CITE = Object.freeze({
  skill: "spec09-dqi-producer",
  reason:
    "Lane SPEC-09 (wave 3, 2026-09-03; CSV upload flow wired train 2026-09-05): tce_data_quality producer, " +
    "spec 09 §1.4. Parses a customer-uploaded shipment DQI CSV (carrier telemetry / fuel receipts / " +
    "verified MRV, per element) through the shared contract and inserts the accepted rows, org-scoped " +
    "server-side. See scripts/spec09/SOURCES.md.",
});
...
export async function main({ mode = "dry" } = {}, deps = {}) {
  ...
  if (!deps.csvText) {
    summary.gap = "no customer CSV given this run — tce_data_quality has no rows until a workspace member " +
      "uploads their shipment's own DQI evidence (POST /api/workspace/spec09-upload). See scripts/spec09/SOURCES.md.";
    return summary;
  }
  if (!deps.orgId) {
  ...
  if (apply && deps.guardedInsertMany && rows.length) {
    ...
    const res = await deps.guardedInsertMany("tce_data_quality", rows, { cite: CITE });
