...
CREATE TABLE IF NOT EXISTS public.emission_factors (
    ...
    mode            text NOT NULL CHECK (mode IN ('road','rail','sea','air','inland_waterway')),
    vehicle_class   text,
    energy_carrier  text,
...
BEGIN
  ...
  probe_a := cl_corridor_id('CNSHA', 'NLRTM', 'sea', 1, 'suez', ARRAY['EGSUZ']);
  probe_b := cl_corridor_id('CNSHA', 'NLRTM', 'sea', 1, 'cape',  ARRAY['ZACPT']);
