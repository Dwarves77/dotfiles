// FUNCTIONALLY VERBATIM copy of src/lib/agent/gate-a-match.mjs (M0 mint kit, 2026-08-31) — the SINGLE
// literal-and-exact matcher for every Gate-A coverage decision, mirrored here so the local validator
// (validate-mint-payload.mjs) scores a payload's Gate-A coverage with the EXACT SAME rule the live
// pipeline / DB criterion 7 uses. Only difference from the src/ original: the two Unicode space code
// points in collapsePctSpace are written as   /   escape sequences here instead of literal
// glyphs (identical regex semantics, safer to hand-author byte-for-byte). If the original changes,
// re-copy it here (see MINT-RUNBOOK.md "keeping the kit in sync"). Do not hand-edit the matching logic
// independently.
//
