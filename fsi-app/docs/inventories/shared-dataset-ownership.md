...
| `scripts/maintenance/origin-class-backfill.mjs` (Lane MAINT, 2026-09-02; registered here lane RULINGS-EXEC, 2026-09-05 — a pre-existing gap: this writer predates this row but was never added to the allowlist, missed by the scanner's `guardedUpdateByIds(` regex, see `.discipline/shared-writer-registry.test.mjs`'s `GUARDED_RE`, which matches `guardedUpdate(` but not the `ByIds` suffix) | UPDATE — `origin_class` only, per `item_type`+`sources.tier` group (`scripts/maintenance/lib/origin-class-map.mjs`'s `originClassFor`, ruling R-E), on every `intelligence_items` row with `origin_class IS NULL` and a resolvable rule. Idempotent (`applyMatch` re-checks `origin_class IS NULL` per chunk); `arg=tool` and any no-rule/no-source_id row are deliberately left NULL, never guessed. | `guardedUpdateByIds("intelligence_items", ids, { origin_class: oc }, { cite: CITE, applyMatch: (q) => q.is("origin_class", null) })` |
...
| `scripts/maintenance/timeline-backfill.mjs` (Lane W9 task 6.1c, 2026-09-12, ADR-030) | INSERT (`item_timelines`, at most one row per item that has none, derived only from the instrument's own dates: title date verified against the capture, Federal Register URL path, legislation.gov.uk Made or Royal Assent line, earliest forward event, source-page dateline; never `added_date`, never an invented day) + INSERT (`integrity_flags`, exactly one row per run: category `data_quality`, subject_type `system`, subject_ref `timeline-backfill`, listing every item the derivation could not date, so the undated residue is a named question, never a silent gap). `item_timelines` is outside this registry's tracked table set (the same posture the entity tables hold in the rows above); the flag write is what this row registers. Dry by default; apply through the guarded path with read-back; `--limit`/`--after-id` resumable. | `guardedInsert("item_timelines", row, { cite: CITE, select: "id" })` in `buildDeps().insertTimelineRow` and `guardedInsert("integrity_flags", buildUndateableFlagRow(items), { cite: CITE, select: "id" })` in `buildDeps().writeUndateableFlag` |
| `src/lib/entities/link-item-entities.mjs` (Lane W9 part 1, task 1.1, 2026-09-11) | UPDATE (`intelligence_items`, `instrument_entity_id` only). The mint-time and substantive-update-time twin of `scripts/entities/backfill-entities.mjs`'s own FK-update write (row above): rule 16(e), reached from `mint-item.ts`'s post-insert participation and from `apply-staged-update.ts`'s substantive `update_item` path when `jurisdiction_iso` or `canonical_instrument_key` changed. Shares `src/lib/entities/entity-plan.mjs`'s four planners with the backfill script, so mint time and backfill time can never plan a row differently for the same input. This writer's other tables (`entities`, `entity_identifiers`, `entity_refs`) fall outside this registry's scope for the same reason stated in the backfill script's own row above. | `sb.from("intelligence_items").update({ instrument_entity_id }).eq("id", item.id)` in `linkItemEntities` |
...
| (any namespace, operator-gated) | `scripts/connections/ratify-flag-to-census.mjs` | **RESOLVED (lane W71-WIRE, 2026-09-05):** read end to end — it does NOT consume a specific `created_by` namespace at all. It requires the flag's `resolution_note` to carry the literal `ratify:census` marker plus `source_id=<uuid> url=<document-url>` (see the script's own header) — a marker an OPERATOR adds by hand while manually investigating any flag, regardless of namespace. It therefore does NOT automatically close the two `intake-seek-study`/`intake-relevance` OPEN leaks above (neither namespace's flags carry a document url an operator could cite without doing the same manual work this marker already requires) — it is a generic ratification vehicle an operator can apply to ANY flag once they've found the document, not a targeted resolver for those two namespaces. Now wired as a `maintenance.yml` step (dry/apply, `arg`=flag id). | ratify-flag-to-census.mjs's own `parseRatificationNote`/`evaluateRatification` (lines 62-109) |
...
| `classification:*` (Axis 3/4/5 source classification, drift, anomaly) | `scripts/classification/propose-classifications.mjs` (lane AXIS, 2026-09-02): writes `integrity_flags` rows proposing Axis 3/4/5 source classification / drift / anomaly findings for operator ratification (TAG pattern; never a silent write to `sources` or `intelligence_items`). `scripts/classification/apply-classifications.mjs` writes `sources.scope_*`/`expected_output`/`jurisdiction_iso` (never the legacy `jurisdictions` column; D9, lane L14, 2026-09-13, migration 033 gave Axis 3 a safe, ISO-shaped home) two ways: reads a RATIFIED row (`resolution_note` containing `ratify:classification`), or, lane GSIG, 2026-09-03, operator ruling retiring the human gate, evaluates an OPEN row directly via `--auto-adopt` and, once every APPLICABLE proposal is covered, writes `integrity_flags.status='resolved'` itself (`resolution_note='auto-adopted:classification:<fields>'`, `resolved_by='apply-classifications.mjs'`), the new `integrity_flags` write the shared-writer-registry scan (`guardedUpdate("integrity_flags", ...)` via the `resolveFlag` dep) now requires listing here. | propose-classifications.mjs (`planReflect` imported from propose-tags.mjs); apply-classifications.mjs |
| `classification:*` (same namespace, MAINT caller) | `scripts/maintenance/apply-classifications.mjs` (Lane CLASSIFY-STEP, 2026-09-04) | The MAINT dispatch runtime for the full propose→auto-adopt pipeline (docs/runbooks/MAINTENANCE-RUNBOOK.md): orchestrates `propose-classifications.mjs`'s logic to emit fresh classification/drift/anomaly proposals (via builders imported unmodified), then calls `autoAdoptClassification()` from `scripts/classification/apply-classifications.mjs` for each OPEN flag, resolving flags once nothing APPLICABLE remains (via `resolveFlag` dep). Second CALLER of the same flag-resolve path, not a second write path; same self-resolving convention. The `integrity_flags` writes via `guardedUpdate` here are the same namespace as the propose/apply callers above (all `created_by='classification:*'` variants). **AMENDED 2026-09-12 (Part 7 task 7.2 / ADR-030 rider)**: `autoAdoptClassification` now decides EVERY proposal on an open flag (scope_topics per-topic via `scope.mjs`'s own keyword table; jurisdiction always declines, architectural gate, never a write; scope_modes/scope_verticals/expected_output keep their existing write rule but a non-writing residue now declines with a reason) and ALWAYS resolves the flag -- no residue stays open. **CORRECTED 2026-09-13 (D9, lane L14)**: the jurisdiction-always-declines line above is RETIRED -- migration 033 added `sources.jurisdiction_iso`, a safe write target, so `jurisdiction_iso` now follows the same decisive-match rule as `scope_modes`/`scope_verticals` (confidence `"high"` adopts, else declines; a value outside `vocab.mjs`'s shape always declines). The legacy `sources.jurisdictions` column is still never written. `resolution_note` uses the shared `src/lib/connections/decision-note.mjs` `DECISIONS_JSON` grammar. **AMENDED AGAIN 2026-09-12 (D17 families 4/5, defect-fix-plan-2026-09-12, lane L10)**: (1) a zero-proposal `source-classification` flag is re-derived from the SC-13 class table (`classTierForHost`, `src/lib/sources/host-authority.ts`) and the source's own observed item-category distribution via `reDeriveZeroProposalClassification`, instead of sitting open forever; a fresh zero-derivation finding from the proposer is now born already-resolved under its own subtype (`source-classification-no-derivable`, `flags.mjs`), mirroring D15's TAG design. (2) every OPEN `source-drift` flag is now decided by `autoResolveDriftFlag` (adopts the observed distribution as `expected_output` once the sample clears 20 items / 2 distinct dates, else "insufficient sample") -- previously advisory-only with no apply target. (3) `item-anomaly`'s detector (`buildAnomalyFlagRow`) is DELETED from the proposer; any surviving open row is retired unconditionally by `retireAnomalyFlag` ("advisory retired under the ADR-030 rider"). Both new `sources`/`integrity_flags` writes go through the SAME guarded `updateSource`/`resolveFlag` deps this row already registers -- no new writer file. | apply-classifications.mjs (builders from propose-classifications.mjs, core logic + `reDeriveZeroProposalClassification`/`autoResolveDriftFlag`/`retireAnomalyFlag` from `scripts/classification/apply-classifications.mjs`) |
| `flywheel-signal:shared_regulation_identifier` / `:shared_title_entity` | `scripts/connections/analyze-corpus.mjs` (--signals; lane L4, 2026-09-03) | **AMENDED 2026-09-12 (Part 7 task 7.2 / ADR-030 rider)**: an undecided candidate no longer sits open indefinitely -- every open flag this pass touches gets a terminal disposition (`decisive`/`undecided`/`stale`, `src/lib/connections/signal-confidence.mjs`'s `planSignalFlagResolutions`), and a brand-new candidate with no existing flag row is inserted ALREADY RESOLVED (`buildPreResolvedSignalFlagRow`). Second writer of the SAME namespace and decision path: `scripts/maintenance/resolve-signals.mjs` (the standalone MAINT dispatch runtime for a backlog drain that would otherwise only run bundled inside a full `corpus-turn.yml` run) -- imports the same pure functions unmodified, never a second copy of the write logic. | analyze-corpus.mjs (lines in the "Signal-candidate handling (L4)" block); resolve-signals.mjs; signal-confidence.mjs (`planSignalFlagResolutions`, `buildPreResolvedSignalFlagRow`) |
...
| `scripts/maintenance/census-off-vertical.mjs` (Lane MAINT, 2026-09-02; archive path added lane RULINGS-EXEC, 2026-09-05) | UPDATE — `arg=archive` (ruling R-A) archives every `would_mint`, not-yet-archived row the shared relevance screen calls `off_vertical`: `is_archived=true`, `archive_reason='off_vertical'` via `scripts/lib/db.mjs`'s table-generic `archivePatch()` (migration 308 added the columns; no new archive-write helper). Idempotent (`applyMatch` re-checks `dryrun_disposition='would_mint' AND is_archived=false` per chunk). `arg=park` is a no-op (nothing to write). | `guardedUpdateByIds("census_worklist", ids, archivePatch("census_worklist", "off_vertical"), { cite: CITE, applyMatch })` |
...
Migration 266. **RESOLVED (lane W71-WIRE, 2026-09-05, plan §W7.1): ONE writer, not two.**
`src/lib/research/theme-brief.mjs` is READ-ONLY — its own header says so verbatim ("READ-ONLY, $0, NO
GENERATION... This module never writes, never clusters, never calls an LLM") and it carries no
...
2026-09-05): `theme_briefs` carries 9 rows, all `generated_by='session-executor'` — the exact value
`generate-theme-brief.mjs`'s `validateAgainstLiveMembers` sets — dated 2026-08-21, proving this script
already ran `--write ... --execute` in a prior session, through the guarded path, before this lane gave it
a CI dispatch root.
...
- **`item_cross_references`** — `scripts/entities/backfill-lineage-edges.mjs` (WO-28 Phase D lineage-edge backfill; its own header notes it feeds a capability "never connected to a producer that runs unmetered"). **RESOLVED (lane W71-WIRE, 2026-09-05, plan §W7.1):** now wired as a `maintenance.yml` step (dry by default, `--apply` to write; `arg` optionally sets `--limit N` for a bounded pilot run). Confirmed distinct from DAG-AUTHOR's `backfill-derivation-edges.mjs`, which writes a different table (`derivation_edges`, wired into `propagation-drain.yml`'s `backfill_and_statutory` checkbox) — the two are not duplicates despite similar names, both are kept, both wired, on their own tables.
...
1. **`integrity_flags` / `created_by ∈ {intake-seek-study, intake-relevance}`** — producer
   ...
   route closes these today. **RESOLVED (lane W71-WIRE, 2026-09-05):** confirmed `scripts/connections/
   ratify-flag-to-census.mjs` (now wired, see `theme_briefs`/`census_worklist` sections above) does NOT
   close this gap — it requires an operator-supplied `source_id`/`url` neither namespace's flags carry by
   construction, so it is a generic ratification vehicle, not a resolver for these two namespaces. This
   leak remains open.
...
4. **`theme_briefs`** — RESOLVED (lane W71-WIRE, 2026-09-05): only one real writer
   (`scripts/connections/generate-theme-brief.mjs`, now CI-dispatched); `src/lib/research/theme-brief.mjs`
