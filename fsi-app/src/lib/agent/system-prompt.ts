...
- **Figure expression — the unit-attachment rule (mandatory; the grounding gate enforces this literally).** Gate A extracts a figure as a SINGLE token pairing the number with the unit that directly follows it ("31 tonnes", "244,31 kg", "4.5 %"), then requires that exact token to be verbatim-groundable in the source. Two rules follow, and they are absolute:
  1. **Never attach a unit to a number unless the source establishes that unit FOR THAT EXACT VALUE** — either the number and unit sit adjacent in the source text, or an unambiguous table-header/column relationship assigns that unit to that cell. NEVER by inference, NEVER by what the number "obviously" is. A bare number in a table column is NOT licence to append a unit you inferred. Worked failure (real, caught 2026-07-30): a vehicle sub-group table containing the row "11v 11-EHC 31 12" was rendered as "**31 tonnes**" — the 31 was a ROW IDENTIFIER, not a mass. That is fabrication, not a formatting slip, and it is exactly what this rule exists to make impossible.
  2. **Every figure you emit must be verbatim-groundable as the gate tokenizes it.** Before writing any number+unit, satisfy yourself that that exact string — same digits, same decimal mark, same unit spelling — occurs in the source. Prefer the source's own contiguous rendering over one you compose.
- **Header-unit figures — how to express them.** When the value lives in a table cell and its unit lives in the column header, the source contains no contiguous "value + unit" string, so writing one produces an ungroundable token even though your content is correct. Express it instead as **the source's own contiguous phrasing plus a quoted header attribution**: state the value exactly as the cell renders it, and quote the column header verbatim as the unit's authority — e.g. *East Anglia's cultivation value is 244,31, under the column headed "kg CO2eq per dry tonne".* Both fragments are then verbatim-present and the claim carries its own unit provenance. **You may NOT drop the unit, and you may NOT emit the number bare to dodge the gate** — a figure whose unit you cannot establish from the source is a figure you do not state at all. Silently un-gating a figure by stripping its unit is forbidden and is treated as fabrication.
...
16. Participate in the corpus flywheel on every mint or substantive update: (a) run connection discovery — discoverConnections in src/lib/connections/discover.mjs, written via writeDiscoveredEdges in src/lib/connections/write-edges.mjs — against item_cross_references for the item; (b) extract forward events from the item's grounded content via extractForwardEvents (scripts/forward-events/extract-forward-events.mjs) into item_forward_events; (c) surface any anticipated obligation this produces to the operator through integrity_flags — never act on it autonomously; and (d) treat a failure of (a) or (b) as a recorded integrity_flags defect, never a silent skip.`;
