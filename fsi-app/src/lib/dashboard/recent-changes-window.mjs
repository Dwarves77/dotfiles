...
// Hold data there until we start regular updates to data." In build mode (CLAUDE.md rule 16, the
// scrape cadence held at "off") the corpus changes only on explicit dispatches, days apart, so a
...
/**
 * @param {string | null | undefined} cadence the scrape cadence read by readScrapeState ('off' in build mode)
