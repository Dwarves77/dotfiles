...
// sitemap walker and run-ledger-consume.mjs's buildFetchDoc both import — reuses CHALLENGE_RE/SOFT_404_RE/
// CDN_BLOCK_RE verbatim instead of a second hand-typed copy (CLAUDE.md "one body" rule). No behavior
// change to this module's own detectRoadblock — only the `export` keyword was added to each const.
export const CHALLENGE_RE = /just a moment|performing security verification|enable javascript|access denied|attention required|cloudflare|captcha|ddos protection/i;
...
export const SOFT_404_RE = /\b(page not found|404 not found|404 error|error 404|document not found|no documents (?:found|matching)|page (?:does not|doesn'?t) exist|page (?:is )?(?:no longer available|unavailable)|requested (?:page|document|url)(?: was| could)? not (?:be )?found)\b/i;
...
export const CDN_BLOCK_RE = /the request could not be satisfied|request blocked\b|generated by cloudfront|akamai reference|reference\s*#\s*[0-9a-f]{6}|varnish cache server|error 5\d\d \(|^error 4\d\d\b/i;
