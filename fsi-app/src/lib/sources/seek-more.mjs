 ...
 *  form is the /TXT/HTML/ rendering (the bare /TXT soft-404s for many ids — see primary-fallback). @returns {string[]} */
 ...
 *  lovdata.no would have been found mechanically. @returns {string[]} */
...
export const GAZETTE_RESOLVERS = [
  {
    ...
    test: (j) => /\bireland\b|\birish\b|^ie$/i.test(String(j || "")),
    resolve: (id) => {
...
/** Gazette candidates for a jurisdiction + identifier (extensible registry). @returns {string[]} */
 ...
 *  the HTML returned "Request Access". @returns {string[]} */
 ...
 * @param {{title?:string, identifier?:string, jurisdiction?:string, sourceUrl?:string}} identity
...
export function exhaustionFlagRow(itemId, exhaustionRecord, verdict = {}) {
  ...
  return {
    ...
    recommended_actions: record.map((a) => ({
 ...
 *  (injected; a fake in tests — NEVER a live write during build). @returns {Promise<object>} the row written. */
 ...
 * @param {{id:string, title?:string, identifier?:string, jurisdiction?:string, source_url?:string, sourceUrl?:string}} item
 * @param {{
