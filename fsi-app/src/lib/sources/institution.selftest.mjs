// Moat selftest (pure, no DB): the reg-fact resolver (buildResolver/resolveSpan) is base_tier-ONLY.
// Reputation (effective_tier) can neither RAISE nor LOWER the stamp tier; a NULL base_tier resolves to
// NULL (the latent breach is closed); a per-host tier_override still wins. Run by fitness F12
// (F12-moat-base-tier) — spawn `node institution.selftest.mjs`, passes iff exit 0. This is the
// committed form of the proof; it fails LOUD on the one-line regression (reintroducing
// `?? effective_tier` in tierOfSource) that the corpus audits cannot catch (stamp + audit move
// together through the same resolver).
import { resolve, dirname } from "node:path";
import { fileURLToPath } from "node:url";
import { createJiti } from "jiti";

const HERE = dirname(fileURLToPath(import.meta.url));        // .../fsi-app/src/lib/sources
const ROOT = resolve(HERE, "..", "..", "..");               // fsi-app
const jiti = createJiti(import.meta.url, { interopDefault: true, alias: { "@": resolve(ROOT, "src") } });
const { buildResolver } = await jiti.import("./institution.ts");
