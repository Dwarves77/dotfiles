...
import { sourceRole, congruence, PRIMARY_ARTIFACT_TYPES, STUDY_BACKED_TYPES } from "./source-role.mjs";
...
test("congruence: primary-artifact type on a NEWS source → market_signal (retyped, not dropped)", () => {
  for (const t of PRIMARY_ARTIFACT_TYPES) {
    const c = congruence(t, "https://sfc.org/en/news/release");
...
test("congruence: primary-artifact on a PRIMARY source is unchanged; market_signal on news is unchanged", () => {
  assert.equal(congruence("framework", "https://x/documents/glec.pdf").changed, false);
  assert.equal(congruence("standard", "https://eur-lex.europa.eu/legal-content/x").changed, false);
  assert.equal(congruence("market_signal", "https://sfc.org/news/x").changed, false); // news IS its correct primary
  assert.equal(congruence("regulation", "https://unknown.host/page").changed, false); // 'other' host stays classifier's call
});
