...
export const DERIVATION = Object.freeze({
  // ── STATUTORY: the strongest claim available. Added 2026-08-12 after an external review noted that
  // `calculated` conflated two materially different things: a number WE computed, and a number computed
  // under a formula PRESCRIBED BY LAW. A FuelEU penalty is not our arithmetic, it is the statute's, and
  // a compliance reader needs to see that distinction without reading our method docs. `statutory` is
  // therefore its own class, ordered above `observed`, and it is the only class admissible as the basis
  // of a compliance assertion.
  statutory_fixed: Object.freeze({
    code: "statutory_fixed", label: "Statutory constant", order: 1, contractable: true, statutory: true,
    note: "A value published in the instrument itself (e.g. EUR 2,400 per tonne VLSFOe). Must cite the provision.",
  }),
  statutory_formula: Object.freeze({
    code: "statutory_formula", label: "Statutory formula", order: 2, contractable: true, statutory: true,
    note: "Computed under a formula prescribed by the instrument. Must cite the provision and the formula version.",
  }),
  observed: Object.freeze({
    code: "observed", label: "Observed", order: 3, contractable: true,
    note: "Measured or reported directly by the primary source.",
  }),
  transacted_index: Object.freeze({
    code: "transacted_index", label: "Transacted index", order: 4, contractable: true,
    note: "Mechanical aggregation of reported deals. No judgment. Goes dark in illiquid conditions.",
  }),
  assessed: Object.freeze({
    code: "assessed", label: "Assessed", order: 5, contractable: true,
  ...
  calculated: Object.freeze({
    code: "calculated", label: "Calculated", order: 6, contractable: true,
    note: "Deterministic function of other observed values under OUR named method. Not law: if a statute prescribes the formula, use statutory_formula.",
  }),
  interpolated: Object.freeze({
    code: "interpolated", label: "Interpolated", order: 7, contractable: false,
    note: "Inferred between known points. Not an observation.",
  }),
  modelled: Object.freeze({
    code: "modelled", label: "Modelled", order: 8, contractable: false,
    note: "Model output. Must never share a visual slot with an observed or statutory figure.",
  }),
  estimated: Object.freeze({
    code: "estimated", label: "Estimated", order: 9, contractable: false,
...
/**
 * Is this value's authority the STATUTE rather than our method? The gate for a compliance assertion:
 * only a statutory class may be presented as what the law requires, and only it may carry a penalty
 * figure. Everything else, including our own `calculated`, is our arithmetic and must say so.
 */
export function isStatutory(derivation) {
  return DERIVATION[derivation]?.statutory === true;
}
