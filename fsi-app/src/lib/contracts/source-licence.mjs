...
export const SOURCE_LICENCES = Object.freeze({
  ...
  emsa_thetis_mrv: {
    key: "emsa_thetis_mrv", name: "EMSA THETIS-MRV", redistribution: "permitted",
    licence: "EMSA site notice: reproduction authorised provided the source is acknowledged",
    attribution: "Source: EMSA THETIS-MRV. Reproduced with acknowledgement of source.",
    url: "https://www.emsa.europa.eu/disclaimer.html", verifiedOn: "2026-08-30",
    note: "DISCHARGED 2026-08-30 from conditional to permitted. Operator ruling (Jason): the EMSA "
        + "material is free to all, citing https://www.emsa.europa.eu/thetis-mrv.html. Verified the "
        + "same day against the primary notice at the url above, which states verbatim: "
        + "\"Reproduction is authorised, provided the source is acknowledged, save where otherwise "
        + "stated.\" That is an authorisation, not a condition to discharge: this register defines "
        + "`conditional` as permitted-subject-to-an-act-we-must-perform (notify, register, accredit), "
        + "and acknowledgement is not such an act — every `permitted` entry here already carries an "
        + "attribution string (GeoNames under CC BY 4.0 is the precedent). No commercial carve-out and "
        + "no prior-permission requirement appears in the general case. "
        + "TWO THINGS RECORDED HONESTLY RATHER THAN GLOSSED: (1) the notice is site-wide, not "
        + "dataset-specific, and carries a `save where otherwise stated` tail — the THETIS-MRV page "
        + "itself was read on 2026-08-30 and states nothing to the contrary (it carries no licence "
        + "wording at all), so the site notice governs; if EMSA ever marks the dataset separately, "
        + "this entry must be re-verified. (2) EMSA is an EU agency, so Commission Decision "
        + "2011/833/EU does not apply to it directly — the authorisation above stands on EMSA's own "
        + "notice, not on the Commission reuse decision. The prior entry asked exactly that question "
        + "and it is now moot. Supporting context, not load-bearing: publication is a legal obligation "
        + "under Regulation (EU) 2015/757 Art. 21, and the annual fleet report is produced by DG CLIMA "
        + "(European Commission), which would fall under 2011/833/EU in its own right. "
        + "WHAT THIS UNLOCKS: `verified_operator_avg` (factor-tier rank 2) was structurally empty while "
        + "this sat amber; it is now seedable. THETIS-MRV is also the substitute for Clean Cargo "
        + "carrier factors and the lawful source of IMO numbers, so discharging it clears two red "
        + "entries. Seeding it is separate work and is NOT authorised by this entry alone.",
  },
  worldbank_cppi: {
...
export function renderDataSourceSeedSql() {
  ...
  // Derived, never hardcoded: the register's verification date is the LATEST entry-level
  // verifiedOn, so re-verifying one entry cannot leave this header asserting a stale date.
  const verifiedThrough = SOURCE_KEYS
    .map((k) => SOURCE_LICENCES[k].verifiedOn)
    .filter(Boolean)
    .sort()
    .at(-1) ?? "unverified";
...
-- Register verified through ${verifiedThrough}. ${SOURCE_KEYS.length} sources: ${t.green.length} embeddable, ${t.amber.length} conditional/unverified, ${t.red.length} prohibited.
