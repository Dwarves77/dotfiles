 ...
 * model-based render by lane w10-factcard, 2026-09-20). One shared renderer for
 * all four detail surfaces so the FACT/ANALYSIS/LEGAL -> FactCard mapping
 * (src/lib/detail/fact-paragraphs.ts, extended by src/lib/detail/fact-card-model.ts)
 * is applied exactly once, not reimplemented per surface.
 *
...
import { deriveFactCardModels } from "@/lib/detail/fact-card-model";
  ...
  return (
    <>
      {blocks.map((b, i) => {
        ...
        return deriveFactCardModels(b).map((model, j) => (
          <FactCard key={`${i}-${j}`} model={model} />
        ));
      })}
    </>
  );
