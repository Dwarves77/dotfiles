...
const SUEZ = { origin: "CNSHA", dest: "NLRTM", mode: "sea", routingKey: "suez" };
const CAPE = { origin: "CNSHA", dest: "NLRTM", mode: "sea", routingKey: "cape-of-good-hope" };
...
test("content-addressed, so two independent ingest paths agree with no coordination", () => {
  const fromUpload = corridorId({ origin: "CNSHA", dest: "NLRTM", mode: "sea", routingKey: "suez" });
  const fromRateBoard = corridorId({ origin: " cnsha ", dest: "nlrtm", mode: "SEA", routingKey: " SUEZ " });
  assert.equal(fromUpload, fromRateBoard, "case and whitespace must normalise to one id");
});

// This case used mode "OCEAN" until 2026-08-12 and passed, because corridor-id carried its OWN private
// mode list containing "ocean" while the emission-factor table was being drafted with "sea". One product,
// two names for one mode, and nothing would ever have errored: a corridor keyed on "ocean" simply never
// matches a factor scoped to "sea", so resolution falls through to a worse tier and looks like normal
// operation. The vocabulary now has a single home in vocabularies.mjs, and the alias is refused HERE.
test("a trade-vernacular alias is REFUSED, not silently resolved, so JS cannot diverge from the SQL twin", () => {
  assert.throws(
    () => corridorId({ origin: "CNSHA", dest: "NLRTM", mode: "ocean", routingKey: "suez" }),
    /non-canonical mode "ocean"[\s\S]*normaliseMode\(\)[\s\S]*"sea"/,
    "the error must name both the fix and the canonical token"
  );
  // Why it cannot be resolved here: the SQL twin cl_corridor_id() only lowercases. Resolving the alias in
  // JS would make the two implementations return DIFFERENT keys for the same input, which is the exact
...
test("COLLISION 1b: explicit via-points also discriminate, and their ORDER matters", () => {
  const viaSuezThenMalta = { origin: "CNSHA", dest: "NLRTM", mode: "sea", via: ["EGSUZ", "MTMLA"] };
  const viaMaltaThenSuez = { origin: "CNSHA", dest: "NLRTM", mode: "sea", via: ["MTMLA", "EGSUZ"] };
  ...
  assert.notEqual(corridorId(viaSuezThenMalta), corridorId({ origin: "CNSHA", dest: "NLRTM", mode: "sea" }));
});
...
test("an invalid spec throws instead of minting an id", () => {
  assert.throws(() => corridorId({ dest: "NLRTM", mode: "sea" }), /origin is required/);
  assert.throws(() => corridorId({ origin: "CNSHA", mode: "sea" }), /dest is required/);
...
test("a degenerate corridor is refused, because it is an upstream parse bug", () => {
  ...
  assert.throws(() => corridorId({ origin: "NLRTM", dest: "NLRTM", mode: "sea" }), /degenerate/);
...
test("multi-byte via names hash by BYTE length, matching SQL octet_length", () => {
  ...
  const p = corridorPayload({ origin: "CNSHA", dest: "NLRTM", mode: "sea", via: ["MALMÖ"] });
