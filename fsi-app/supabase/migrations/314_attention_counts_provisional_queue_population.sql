-- subject: Migration 314 (lane counts, 2026-09-08, production click-through audit of /admin): `admin_attention_counts()` counted the provisional queue by a narrower predicate than the table it labels, so one screen showed three numbers for one queue - the Sources tab badge 489, the issues-queue row 489, and the table header directly above the rows "491 PENDING". The function now counts the queue AS RENDERED, 491 = 489 pending_review + 2 needs_more_data. **APPLIED TO PRODUCTION AND VERIFIED BY THE COORDINATOR BEFORE THE FOLD** (the schema half of the migration two-track policy, CLAUDE.md standing rule 3): the file is in the tree for this inventory and must NOT be re-applied. Inventory row added at FOLD-61, where the C3 consistency check found the file on disk and unlisted. | applied 2026-09-08 | lane counts / coordinator
