-- subject: Migration 145 (D1 floor render-derive): validate_item_provenance criterion-3 authority floor DERIVES the FACT tier INLINE from the claim's resolved source (section_claim_provenance.source_id → sources.COALESCE(tier_override, base_tier)) instead of reading the stored source_tier_at_grounding — base_tier-only + sanctioned override, never effective_tier (moat-pure), read live (drift-proof). Criteria 1/2/4/5 + source_span checks + legal-line guard + label-variant match BYTE-IDENTICAL to 143. Zero blast radius (proven read-only before apply: 0 drift on the source_id basis; 145 output == 143 output for all 658 items — the lone confirm-script "flip" was a stale-stored-status false positive, 143 quarantines it identically). claims-tier audit re-pointed to derivation-consistency (lane green). APPLIED 2026-06-29. [glyph:verbatim]
-- Migration 145: validate_item_provenance — Criterion-3 authority floor RENDER-DERIVES the FACT tier.
--
-- D1 (source_tier_at_grounding = CACHE, render-derive). The authority floor (criterion 3) previously read
-- the STORED scp.source_tier_at_grounding — a cache that can drift from the live source registry. It now
-- DERIVES the FACT tier INLINE from the claim's resolved source: section_claim_provenance.source_id ->
-- sources.COALESCE(tier_override, base_tier). This is:
--   * MOAT-PURE by construction — base_tier (the static authority-origin) with the per-host tier_override
--     (the only sanctioned escape); NEVER effective_tier. With A1's grounding-SELECT drop, the entire
--     reg-fact-tier path is base_tier-only end-to-end (resolver stamp, floor, and audit all derive the
--     same way; reputation can never confer reg-fact eligibility).
--   * DRIFT-PROOF — the floor reads the live source tier each validation, so it cannot diverge from the
--     registry the way a stored cache does (the store-and-drift class this kills at source).
--   * ZERO blast radius — proven read-only before apply: stored stamp == COALESCE(source_id ->
--     tier_override, base_tier) for all 6,263 FACT claims (0 drift on the source_id basis), and 0 items
--     flip the floor in either direction. The re-stamp backfill is moot (nothing to re-stamp).
--
-- The stored source_tier_at_grounding column is now a pure cache the floor no longer consumes; the
-- claims-tier audit is re-pointed to derivation-consistency (stored == derived-from-source_id), and the
-- column is slated for drop in cleanup (Phase 7).
--
-- Criteria 1/2/4/5, the criterion-3 source_span checks, the legal-line guard, the label-variant match,
-- and everything else are BYTE-IDENTICAL to migration 143. Only criterion-3's tier source changes
-- (stored stamp -> inline COALESCE(tier_override, base_tier) via source_id). STABLE / read-only.
