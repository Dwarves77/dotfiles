-- subject: Stage audit `docs/audits/stage-audit-2026-09-18/s6-gates-harness.md` findings 8/9; build plan `docs/plans/complete-system-build-plan-2026-09-04.md` section 6.1, lane M9 (lane m9c, 2026-09-18). Schema: drops `community_promotion_transitions` (migration 295's audit log for the unwired five-gate `promotion_state` machine, `src/lib/community/promotion.mjs`, zero production importers), retiring promotion mechanism A; `post_promotions` (migration 041) is the one live promotion path (`docs/plans/C6-promote-spec.md`). Verified live before the drop, this lane: 0 rows, 0 foreign keys in, 2 RLS policies and 2 indexes exist only on this table (drop with it), its 4 triggers are internal FK referential-integrity triggers (drop with it). `community_posts.promotion_state` and `.stance` (migration 295's other two changes) are untouched. **APPLIED 2026-09-18** by the coordinator before this lane's code merged (two-track policy). Numbered 329 because 326 and 328 belong to lane L39.
