-- subject: Migration 309 (Lane ATTACH-SOURCES, 2026-09-05, W3.3 — **renumbered from the lane's own 308 by the ASSEMBLE-47 coordinator lane**: 308 collided with lane RULINGS-EXEC's `census_worklist_archive_columns.sql`, which landed first in this train; disjoint tables, no dependency between the two, renumbered to the next free slot rather than colliding). Extends `source_tier_opinions`'s `opinion_source` CHECK constraint (migration 091: `haiku_brief_classifier` / `haiku_verification` / `operator_review`) to add a fourth literal, `host_class_table` — the first deterministic, $0, no-LLM upstream for this table: `scripts/maintenance/tier-opinions.mjs` (same lane) compares every `sources.base_tier` against `src/lib/sources/host-authority.ts`'s `classTierForHost(hostOf(url))` (the SAME SC-13 class table `heal-provenance.mjs`'s STEP SOURCE and `institution-canonicalize.mjs`'s Part C already use) and records a disagreement via `recordTierOpinion` (`src/lib/sources/tier-opinion-writer.ts`, extended with an optional `opinionSource` parameter, default unchanged). SAFE BY CONSTRUCTION: `source_tier_opinions` has 0 rows (confirmed live, Supabase MCP, 2026-09-05) and adding a CHECK literal cannot invalidate an existing row regardless. Pre-check confirms the target constraint's name/count; DROP CONSTRAINT + re-`ADD CONSTRAINT` (same pattern migration 307 established for an equivalent index rename, applied here to a CHECK's literal set, since Postgres has no `ALTER CONSTRAINT` for a CHECK expression); post-check confirms all four literals are present. **APPLIED LIVE 2026-09-05 19:30:19 UTC** by the coordinator via Supabase MCP, before the first `tier-opinions --mode apply` dispatch (post-check confirmed all four CHECK literals present; `schema_migrations` version `20260905193019`). Was: written, not applied — this lane held no DB write credentials. Reversible: `ALTER TABLE source_tier_opinions DROP CONSTRAINT source_tier_opinions_opinion_source_check; ALTER TABLE source_tier_opinions ADD CONSTRAINT source_tier_opinions_opinion_source_check CHECK (opinion_source IN ('haiku_brief_classifier','haiku_verification','operator_review'));` (safe only while no `host_class_table` row exists yet — check row count first). [glyph:verbatim]
-- 309 — source_tier_opinions: add 'host_class_table' to the opinion_source CHECK constraint.
--
...
BEGIN
  ...
  END IF;
  RAISE NOTICE 'migration 309 pre-check OK: source_tier_opinions has % row(s); target constraint present as expected', n_rows;
END $$;
...
COMMENT ON CONSTRAINT source_tier_opinions_opinion_source_check ON public.source_tier_opinions IS
  'opinion_source enum, migration 091 + 309. host_class_table (309, Lane ATTACH-SOURCES W3.3) is the '
...
BEGIN
  ...
  END IF;
  RAISE NOTICE 'migration 309 OK: opinion_source CHECK now allows host_class_table alongside the original three';
END $$;
