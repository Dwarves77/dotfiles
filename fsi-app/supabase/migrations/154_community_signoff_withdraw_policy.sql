-- subject: Migration 154 (Template 11 sign-off wiring follow-on, 2026-07-07). ADDITIVE to 153, adds ONE RLS policy `signoff_withdraw_own` on `community_post_signoff_requests`; touches none of 153's table/index/policies. WHY: 153's only UPDATE policy (signoff_decide) is active-verifier/admin-only, so a plain requester had NO RLS path to withdraw their own still-open request (community-schema-mapping.md section 3.1 names withdraw as part of the lifecycle). The new policy is deliberately narrow: requester may move ONLY their OWN request, ONLY while `status='pending'`, and ONLY to `status='withdrawn'` (using pins the pre-image to the requester's own pending row; with_check pins the post-image to the withdrawn shape). Never overlaps signoff_decide. Backs `POST /api/community/signoff/[id]/withdraw`. Until applied, that route matches 0 rows and returns 404 (safe pre-apply); create + verifier-decide work against 153 immediately. Reversible (DROP POLICY). APPLIED + verified 2026-07-07.
