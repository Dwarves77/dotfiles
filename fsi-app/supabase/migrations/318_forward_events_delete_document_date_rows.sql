-- subject: Defect D10, `docs/plans/defect-fix-plan-2026-09-12.md` (lane L6, 2026-09-13). Data-only cleanup: `DELETE FROM item_forward_events WHERE source_kind = 'section' AND obligation_text LIKE 'In force as of %' AND source_span = event_date::text` -- the exact fabricated shape the section-side forward-events extractor produced before this lane's fix (an "In force as of [date]." status sentence promoted to an event dated the RUN date, not a date the instrument states). 2 rows expected per the plan (one confirmed as item 252f0ecf); this lane held no live DB read credentials to independently reconfirm the exact count or the second item id, so the coordinator asserts both before applying. Lands with the same commit series as the extractor fix (`src/lib/forward-events/extract-forward-events.mjs`'s reference-date/status-only refusals, `read-and-extract.mjs`'s referenceDates + verbatim-in-a-FACT-claim class fix) per standing rule 3 (data migration commits with consumer code, runs after merge). NOT applied by this lane. Reversible: no -- a DELETE of fabricated rows has no honest inverse; recovery, if the predicate is ever found to have matched a genuine row, is re-extraction via `read-and-extract.mjs`, never a manual re-INSERT.
