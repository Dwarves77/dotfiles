-- subject: Migration 315 (lane duenext, 2026-09-08, production defect "DUE NEXT: 0 ITEMS"): new `get_workspace_due_next(p_org_id uuid, p_limit integer DEFAULT 24)`. The Due-next card was fed `get_workspace_intelligence_dashboard`, a slice ordered by PRIORITY with LIMIT 50, which answers a different question from the one the card asks; measured live, 45 of the workspace's 1,433 active verified items carry a future binding date and that slice held 23 of them and none of the 5 whose only future date is `compliance_deadline`. This function orders by the binding date itself, computed as `LEAST(compliance_deadline when >= current_date, MIN(item_timelines.milestone_date when >= current_date))`, which is exactly the rule the UI's `dueInfo` applies; `entry_into_force` and `next_review_date` are deliberately excluded because no Resource field carries them and the UI cannot reproduce them (see the file header). Bounded: p_limit clamped to [1,100] in-function (F38/F39). Workspace-scoped through `public._workspace_active_items(p_org_id)`, so the membership assert and the customer read gate are unchanged. Column list byte-identical to the dashboard RPC's plus one trailing `next_binding_date`, so `mapWorkspaceItemRows` maps its rows with no change. Idempotent (DROP IF EXISTS on both arities, then CREATE, then explicit GRANTs); reversible by the DROP alone, since it alters nothing that existed before it. **APPLIED LIVE 2026-09-08 by the lane during verification** (post-check run: 24 rows, first next_binding_date 2026-09-08, then 2026-09-30 x3, 2026-10-01, 2026-11-18, 2026-11-21, 2026-11-29, 2026-11-30, 2026-12-30); re-applying is safe and idempotent. | applied 2026-09-08 | lane duenext
