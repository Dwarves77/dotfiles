-- subject: Migration 202 (operator ruling 2026-07-13). Scopes the `standard` authority floor to the item's OWN authoring body: `validate_item_provenance` grounds a `standard` item's FACT at the standards-body tier (4) ONLY when the fact's source shares the item's own-source `institution_id` (migration-122 institution SSOT); a same-tier UNRELATED host stays at the reg-family floor 2. Monotonic + standard-only (can add verifications, never remove any); all other item types/criteria byte-identical to the prior function. Mirrored in JS by `authorityFloorForFact` (source-blocks.mjs; accept/reject golden) + invariant SC-14 + source-credibility-model floor-vocab anchor. **APPLIED 2026-07-13** via the direct postgres pooler; verified non-regressive (30/30 verified-live still valid; non-standard controls o5/o13 unchanged; c5/c8/green-building correctly stay quarantined); recovered c3 (GRI) + c4 (ISO 14083) to verified at $0. Reversible (re-apply the captured prior function body).
-- 202 — STANDARD floor scoped to the authoring body (operator ruling 2026-07-13).
--
-- FLOOR VOCAB: for item_type='standard', the authority floor is the standards-body class (tier 4, the tier the
-- registry assigns ISO/GRI/USGBC-type bodies) — but SCOPED to the item's OWN authoring body's authoritative
-- text, NOT any same-tier host generically. Mechanized via the institution SSOT (migration 122): a FACT whose
-- source shares the item's own-source institution_id grounds at floor 4; every other host stays at the
-- reg-family floor 2. "A matched span in a same-tier UNRELATED host is not the item's primary" (moat).
--
-- Blast radius is BOUNDED + MONOTONIC: only item_type='standard' is affected, and only LOOSENED (2 -> 4) for
-- own-body facts — it can add verifications, never remove any. All other item types + criteria are byte-identical
-- to the prior function. Schema/function migration: applied via MCP before the JS mirror commits.
--
-- (This replaces validate_item_provenance in full — plpgsql has no partial patch. The ONLY changes vs the prior
--  body: two new DECLAREs, the item-institution lookup, src.institution_id in the criterion-3 select, and the
--  per-fact scoped floor block. Everything else is preserved verbatim.)
...
BEGIN
  ...
  IF v_has_sections THEN
    ...
    LOOP
      ...
      -- 202: per-fact effective floor. Default = the item-type floor (2 for the reg family). For a STANDARD
      -- item, a FACT on the item's OWN authoring body (same institution as the item's canonical source) grounds
      -- at the standards-body tier (<= 4) — the standard's own authoritative text IS its primary. A same-tier
      -- UNRELATED host does NOT qualify; it stays at floor 2.
      v_fact_floor := v_floor_max;
      IF v_item.item_type = 'standard'
      ...
      IF v_floor_armed
        ...
        v_failures := v_failures || jsonb_build_object(
          ...
          'floor_scope', CASE WHEN v_fact_floor = 4 AND v_item.item_type = 'standard'
                              THEN 'standard_own_body' ELSE 'default' END,
