-- subject: Migration 204 (operator ruling 2026-07-15, from the build-phase spend-regime ledger findings). Adds `agent_runs.model text` (nullable) so per-MODEL actuals are FIRST-CLASS — the build-phase MEASUREMENT requirement (spend-regime: actuals per item/class/**model**). The model was already known at the spend chokepoint (`recordSpendCall`) but stashed in `errors[].telemetry.model` (documented NO-DDL workaround); this lands the queued column and `spend-client.ts::recordSpendCall` now writes it. Cost-0 marker/aggregate rows may leave it null. Additive + idempotent (`ADD COLUMN IF NOT EXISTS`). **APPLIED 2026-07-15** via apply_migration. Reversible (DROP COLUMN model). [glyph:verbatim]
