-- subject: Migration 191 (Wave-α Track D d2; master-gap P2, 2026-07-11). AUTHOR-ONLY — operator DDL window. The DB half of the ban re-join block: BEFORE INSERT (and UPDATE OF user_id/org_id) trigger `org_memberships_ban_guard_trigger` on `org_memberships` raises 42501 when the (org_id, user_id) has an `org_member_bans` row. SECURITY DEFINER so the ban lookup is not blinded by the owner/admin-scoped SELECT policy on org_member_bans (INVOKER guard would read 0 rows for a non-admin caller and silently pass — the F10 class). Backstops the two app-layer checks that already exist (accept_invitation guard from mig 156; members-route PUT add-by-email check) so no writer — service-role, user-JWT RLS-path, or a future forgetful caller — can bypass the block. Reversible (DROP TRIGGER + FUNCTION; app checks remain). APPLIED 2026-07-11 (wave-alpha). [glyph:verbatim]
