-- subject: Migration 200 (Wave-α Track C8 / RD-5). Adds `intelligence_items.canonical_instrument_key text` + `derive_canonical_instrument_key(instr, src_url)` (IMMUTABLE — bare CELEX from a CELEX token or ELI reg/dir/dec path in instrument_identifier or source_url; NULL when not confidently derivable, never guesses R/L/D from a bare YYYY/N) + BEFORE INSERT/UPDATE trigger (derivation wins, preserves a non-derivable manual value) + partial UNIQUE index `WHERE key IS NOT NULL AND provenance_status='verified' AND is_archived IS NOT TRUE` (twin-defect guard, correctly excludes archived tombstones). Ships with `backfill-canonical-keys.mjs` (RD-5, 20/21 set; 1 unverified skipped by guard_provenance_flip, key immaterial) + `canonical-key-uniqueness.mjs` (invariant EP-11). Proof: 4 derive cases pass; uniqueness audit PASS (175 verified-live, 0 collisions). Reversible (rollbacks/200 drops index+trigger+both fns+column). APPLIED 2026-07-11 (wave-alpha). **CORRECTION (lane W71-C, 2026-09-05):** `scripts/_wave-alpha/backfill-canonical-keys.mjs` was DELETED — the backfill it ran is applied history (20/21 set, above), and F25 module-liveness flagged the script itself unwired (zero non-test importers, no dispatch). Removed from `.discipline/governance/skill-contract-map.mjs`'s `PINNED_MANIFEST` citingFiles in the same commit. [glyph:verbatim]
