-- subject: Migration 316 (lane brieffields, 2026-09-11, W9 brief-chain-build-plan Part 2 task 2.1). Adds `intelligence_items.cost_mechanism text`, `penalty_range text`, `enforcement_body text`, `requirement_trajectory jsonb` (who pays / penalties / the regulation's per-year requirement path -- fields the brief contract is meant to carry but no column did; `requirement_trajectory` is deliberately NOT `trajectory_points`, migration 107's numeric price series with its own CHECK and its own renderer) plus `intelligence_items_requirement_trajectory_check` (`requirement_trajectory IS NULL OR jsonb_typeof(requirement_trajectory->'steps') = 'array'`). All four nullable, no backfill; task 2.2 (parser + single write site) and task 2.4 (`format_type` catch-up) populate them. Projects the four columns as trailing columns on the SAME 11-function set migration 310 (`item_grade`) established as this codebase's "every customer RPC" for a new item-level column: the shared helper `_workspace_active_items` plus the org-scoped + `_public` pair for slim, listings, market, operations, research. DROP FUNCTION + CREATE OR REPLACE on all 11 (CREATE OR REPLACE alone cannot widen a RETURNS TABLE, 42P13, per 272/310), re-GRANT EXECUTE to anon/authenticated/service_role on each (DROP does not preserve grants). Deliberately EXCLUDED, same reasoning 310 used: `get_workspace_intelligence_dashboard` (already strips narrative fields for the card, migration 064), `get_workspace_due_next` (315, states its own byte-identical-to-dashboard invariant), `get_technology_items` (no `_public` sibling; outside the five-surface model), the bare `get_workspace_intelligence` (zero live callers), and the two `jsonb`/narrow-shape aggregate RPCs. The single-item DETAIL read (`fetchIntelligenceItemUncached`, all four detail routes) is a direct `SELECT "*"` against `intelligence_items`, not an RPC, and needs no migration change -- it gets the four columns for free once the ALTER TABLE applies; task 2.3's mapper-only file list corroborates this split. Re-verified the 6 callers of `_workspace_active_items` beyond the 10 in-scope RPCs (2 more than existed at 310's time: `get_workspace_intelligence_dashboard` and 315's `get_workspace_due_next`) all select an explicit column list off the helper, never `ii.*`, so widening its RETURNS TABLE is safe for all six. Guarded pre-check (exact live md5 per function, captured via Supabase MCP `pg_get_functiondef`, 2026-09-11) and post-check `DO $$ ... $$` block: presence (all 11 defs mention all four columns), execution (calls the five `_public` functions, would raise 42703 on a mistyped column), the slim_public/listings_public row-count parity invariant (310's own check), and an ADVERSARIAL proof against one real row that the CHECK rejects a non-array `steps`, accepts a valid array, and accepts NULL, per rule 15 ("attack, don't assert presence") -- no permanent data change. Coordinator finding (2026-09-11, live probe on kwrsbpiseruzbfwjpvsp): the proof originally used bare `SAVEPOINT`/`ROLLBACK TO SAVEPOINT` statements, which PL/pgSQL does not support (`42601 syntax error at or near "TO"`) and would have aborted the whole migration at apply time; rewritten same day to use PL/pgSQL's own `BEGIN ... EXCEPTION ... END` block-subtransaction semantics instead, no SAVEPOINT statements anywhere in the file. Written, not applied -- this lane's Supabase MCP access was read-only `execute_sql` SELECT throughout (schema state, function bodies, grants, constraint names/column absence all confirmed live before drafting); the coordinator applies live before task 2.2/2.3's dependent code lands, per CLAUDE.md rule 3 (two-track policy). Reversible: `ALTER TABLE intelligence_items DROP COLUMN cost_mechanism, DROP COLUMN penalty_range, DROP COLUMN enforcement_body, DROP COLUMN requirement_trajectory;` (drops the CHECK with the column), then `DROP FUNCTION` each of the 11 post-migration signatures and `CREATE OR REPLACE FUNCTION` each back to its pre-migration body (captured verbatim in the file's pre-check comment block), then re-GRANT EXECUTE to anon/authenticated/service_role on each.
