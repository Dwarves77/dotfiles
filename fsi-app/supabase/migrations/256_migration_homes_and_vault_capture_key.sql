-- subject: Migration 256 (migration homes complete + vault the capture JWT, 2026-08-11). Writes the LAST FIVE out-of-repo database objects into the migration tree verbatim from their live definitions captured via pg_get_functiondef (gate_a_health_cache table, gate_a_health_compute, gate_a_health_refresh -- deliberately UNSCHEDULED per operator ruling 2026-08-10, the 30-minute staleness gate makes the dormancy visible by design --, gate_a_health, next_uncensused_portal_candidates), all CREATE OR REPLACE / IF NOT EXISTS no-ops against live. ONE deliberate change: capture_worker_fetch's hardcoded anon-role JWT literal moves to Supabase Vault (secret capture_worker_anon_key, created idempotently), read at call time with a fail-loud if absent -- key rotation becomes one vault update instead of a silent break inside a SECURITY DEFINER body no repo-side scan can see. The anon key is PUBLIC by design; vaulting is rotation visibility, not secrecy. Post-check RAISEs if the function still embeds a JWT literal ('eyJ'). Completes the arc db-layer-census -> migration 254 (drop 17) -> this (backfill 5): F24's NO_MIGRATION_HOME allowlist 22 -> 5 -> EMPTY, now audited as the permanent target state; F24 NET_EGRESS_SANCTIONED updated (JWT vaulted). **APPLIED LIVE 2026-08-11** via apply_migration; self-checks green. Reversible per object (DROP), though reverting the vault change would re-embed the literal. **Migration home of record for `next_uncensused_portal_candidates` [coordinator ruling, task 5.3, 2026-09-11]**: item 5 above IS this function's migration home; PR #370's own stale `223_next_uncensused_portal_candidates.sql` was RETIRED here (`git rm`, not renumbered to 241) once re-verified live (`md5(pg_get_functiondef(...))` = `879c7fa78d59e79da5321a67f2fa284d`, unchanged since this migration's 2026-08-11 capture) -- a byte-identical second migration file for the same function is dead weight, not an audit trail. PR #370's portal-harvest.ts consumer code and its npmtest.mjs golden landed independently of this migration's DDL, since the DDL was already home here.
