-- subject: Migration 161 (P2-6 change detection, chrome-audit S1-10). check-sources wrote `change_detected: false` HARDCODED — zero change rows ever; "monitoring" was accessibility-only. Adds two additive nullable columns: `sources.last_content_hash` (sha256 of the last successful render's normalized text — the worker fingerprints the SAME Browserless render the accessibility check already pays for, zero extra units vs the 20k/mo budget) + `sources.last_content_changed_at` (stamped on fingerprint difference — the queryable change signal). Pure half = content-change.mjs (200ch error-page floor so outages never read as change; first observation only seeds). Downstream auto-action deliberately NOT wired (rides the loop flip). APPLIED + verified 2026-07-08 (operator "Proceed"; ledger row 161 same transaction; read-back 2 columns present). Reversible (DROP COLUMN x2). [glyph:verbatim]
