-- subject: Migration 240 (Layer C insert gate, 2026-08-08). Creates `guard_data_audit_block()` + `guard_data_audit_block_trg` BEFORE INSERT ON `intelligence_items`: rejects any insert (ERRCODE check_violation, message prefix `layer-c-insert-gate:`) while an open data-audit block exists (`integrity_flags` category=`data_integrity`, subject_ref=`data-audit-lane`, status=`open`) lacking a valid dated waiver (`recommended_actions` element `{action:"waiver", until}` with until's UTC midnight >= now -- exact SQL mirror of the unit-tested `audit-gate-core.mjs` `hasValidWaiver`; keep in sync). Closes the fleet-bypass gap root-caused 2026-08-02: the app-layer gate lived only in `preflightStep` (generate-brief workflow) + funded-pass, which the recurring fleet never invokes -- 500+ fleet-authored rows with zero `agent_runs` rows never consulted the open block (e.g. cfb1799a, waiver expired 2026-07-28). Same fix shape as mig 115/118/201: the invariant moves into the DB where every writer passes through it (BYPASSRLS does not skip triggers). Deliberate-act exemption: transaction-local GUC `app.data_audit_override` (mig-201 marker precedent), RAISE WARNING on use, not advertised in the rejection message. Fail-closed superset: ALL open block rows must be waived. No open block = inserts untouched. Additive + idempotent (CREATE OR REPLACE / DROP IF EXISTS); the app-layer checks stay as defense in depth. Proof harness (red-then-green, rollback-only, live rows never durably touched): `scripts/verify/layer-c-insert-gate-proof.mjs`. **NOT APPLIED** -- schema DDL: apply via Supabase BEFORE the branch merges (two-track policy). Reversible (drop trigger + function). Insert-path proof (task 5.3, 2026-09-11): the canonical generate path (canonical-pipeline.ts / generate-brief.ts) performs zero INSERTs into intelligence_items (SELECT/UPDATE only) so the trigger cannot affect it; the record-mint path (run-mint-batch.mjs -> apply-mint-batch.mjs, direct guardedInsert at apply-mint-batch.mjs:480) and applyStagedUpdate new_item (-> mintIntelligenceItem, INSERT at mint-item.ts:256-260) both insert through the trigger, and the live data-audit-lane block state was confirmed OPEN=0 rows at authoring time, so landing this migration changes nothing for any current legitimate insert; it only starts enforcing the intended block during a future RED lane. PRE/POST-CHECK md5+existence blocks added to the migration file.
