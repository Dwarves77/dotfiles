-- subject: Migration 308 (lane RULINGS-EXEC, 2026-09-05, W2.2, ruling R-A: the census-off-vertical archive path). Additive: `census_worklist.is_archived BOOLEAN NOT NULL DEFAULT FALSE` + `census_worklist.archive_reason TEXT`, mirroring `intelligence_items`' own pair (migration 004) verbatim, no CHECK pairing them, matching that precedent; `db.mjs`'s table-generic `archivePatch()` needs no change to serve this table. Orthogonal to `dryrun_disposition`/`enumeration_status` (migration 221's two existing lifecycles), `is_archived` is a separate terminal flag, not a widened disposition CHECK. Partial index `idx_census_worklist_archived` (`WHERE is_archived = TRUE`) mirrors `intelligence_items`' `idx_items_archived`. Unblocks `scripts/maintenance/census-off-vertical.mjs`'s `arg=archive` path (previously NOT RUNNABLE, see that file's own header pre-2026-09-05), built in the same commit; every live reader of the `would_mint` pool (`export-census-rows.mjs`'s live read + its `selectCensusRows` pure filter) excludes `is_archived=true` in the same commit so an archived row can never re-enter export. **APPLIED LIVE 2026-09-05 19:29:58 UTC** by the coordinator via Supabase MCP (post-check DO block passed: column types/defaults, index presence, 0 archived rows immediately after; `schema_migrations` version `20260905192958`). Was: written, not applied, this lane held no DB write credential (Supabase MCP read-only). Reversible: `ALTER TABLE census_worklist DROP COLUMN is_archived, DROP COLUMN archive_reason;` (drops the partial index with the column).
