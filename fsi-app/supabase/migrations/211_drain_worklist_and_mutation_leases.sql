-- subject: Migration 211 (operator ruling 2026-07-16, PARALLEL DRAIN). Adds two tables for the two-writer drain: `drain_worklist` (the DB-resident lane split — Lane A judgment / Lane B mechanical, one row per quarantined item, upserted by the classifier) and `mutation_leases` (per-item H5 lease: one LIVE lease per item, row exists only while leased). Lease functions mirror the proven funded_pass_runlock (mig 205) atomic acquire-or-stale-takeover-or-fail: `acquire_mutation_lease` (insert-or-stale-takeover-or-refuse, returns acquired/takeover + incumbent holder), `heartbeat_mutation_lease` (still_held; empty = taken over), `release_mutation_lease` (holder-only delete). A live lease held by the other session blocks acquisition so no item is ever worked twice; heartbeat past 300s is claimable. Consumed by `scripts/lib/mutation-lease.mjs` (acquireLease/heartbeat/release/withLease). Golden `scripts/verify/mutation-lease.golden.mjs` (10/10: refuse-on-held, incumbent-named, holder-only heartbeat/release, re-acquire-after-release, stale-takeover). Closes the H5 residual named in RD-38. Additive tables + fns. **APPLIED 2026-07-16** via apply_migration. Reversible (DROP the 3 fns + 2 tables). [glyph:verbatim]
