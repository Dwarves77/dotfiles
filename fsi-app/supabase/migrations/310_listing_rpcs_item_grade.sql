-- subject: Migration 310 (lane CHIPS, 2026-09-05, W3.4 — **renumbered from the lane's own 308 by the ASSEMBLE-47 coordinator lane**: 308 collided with lane RULINGS-EXEC's `census_worklist_archive_columns.sql`; 309 taken by lane ATTACH-SOURCES's renumbered migration; disjoint objects, no dependency among any of the three, renumbered to the next free slot). Projects `intelligence_items.item_grade` (migration 278) as a new trailing `RETURNS TABLE` column on 11 functions: the shared org-scoped helper `_workspace_active_items` and the 10 listing/category-routed RPCs (org-scoped + org-independent `_public` pair each, for slim, listings, market, operations, research). Root cause `[CONFIRMED]`, live `pg_get_functiondef` reads 2026-09-05: `RecordGradeBadge.tsx` has been wired into `RegulationsLedger.tsx`'s row and all three detail surfaces since 2026-09-01/04, and `src/lib/supabase-server.ts`'s mappers already read `row.item_grade` defensively — but every one of these 11 RPCs omitted `ii.item_grade` from its SELECT list, so the badge rendered nothing for its entire live population (1,101 non-archived verified items carry `item_grade='record'`, 1,095 of them domain=1/Regulations). Every SELECT list, WHERE clause, JOIN and ORDER BY is otherwise byte-identical to the live definitions (captured verbatim in the file's own pre-check comment block); this migration adds one column and changes no routing/filtering logic — verified this does not desync the org-scoped/public pairs, since both halves of each pair are extended identically in the same statement. Code side needs no change (the mappers' existing `row.item_grade === "record" ? ... : undefined` passthrough lights up once this is live) — landed alongside the lane's PR per the two-track policy (CLAUDE.md rule 3: DDL before code). Guarded pre-check (exact live md5 per function, captured 2026-09-05) and post-check `DO $$ … $$` block (asserts all 11 function bodies mention `item_grade`, executes `get_workspace_intelligence_listings_public(1)` live and asserts `count(*) FILTER (WHERE item_grade='record') > 0`, and asserts slim_public/listings_public row-count parity is unchanged). Standalone; does not depend on 305/306/307. **REWRITTEN by lane MIG310-FIX, 2026-09-05 (coordinator, live, 20:20 UTC found the defect)**: the file's original `CREATE OR REPLACE FUNCTION`-only shape fails on all 11 with `[CONFIRMED] ERROR 42P13: cannot change return type of existing function` (reproduced in a BEGIN/ROLLBACK against the live project, nothing applied; live md5s of all 11 bodies still equal this file's pre-check table) — `CREATE OR REPLACE` cannot widen a `RETURNS TABLE` shape, the identical error class migration 272 already hit and fixed for a different eight functions. The file now `DROP FUNCTION IF EXISTS`s all 11 (dependents of `_workspace_active_items` first, for readability only — Postgres does not require it, plpgsql bodies are not dependency-tracked) immediately before their `CREATE OR REPLACE FUNCTION` statements in the same transaction, then explicitly `GRANT EXECUTE ... TO anon, authenticated, service_role` on each — DROP does not preserve grants, and `CREATE FUNCTION` grants only `PUBLIC` by default. This replaces the file's former line asserting "CREATE OR REPLACE preserves grants... nothing to re-grant," which was true only for the plain-CREATE-OR-REPLACE shape the file no longer uses. Dependency check (`git grep` across every migration file for all 11 identities, this lane): no view, trigger, or other function references any of the 11 outside their own prior redefinitions; `active_intelligence_items` (migration 116) reads `intelligence_items` directly and is not a dependent. Verified this lane against a scratch Postgres 16 cluster seeded with the exact pre-310 (272/305/306-shaped) signatures: the original shape reproduces the identical 42P13/HINT; the rewritten DROP+CREATE+GRANT file applies cleanly end-to-end in one transaction (11 DROP, 11 CREATE, 11 GRANT, the file's own post-check DO block passes) and the resulting `pg_proc.proacl` matches the live ACL byte-for-byte. Code side re-confirmed by reading `src/lib/supabase-server.ts`: both mapper sites still read `row.item_grade` defensively, no change needed. **APPLIED LIVE 2026-09-05 19:47:46 UTC** by the coordinator via Supabase MCP, using the rewritten file (its own outer `BEGIN;`/`COMMIT;` stripped, redundant with the MCP's own transaction; post-check DO block passed; `schema_migrations` version `20260905194746`). Was: written, not applied — this lane's Supabase MCP access was read-only SELECT. Reversible: `DROP FUNCTION` each of the 11 post-fix signatures, `CREATE OR REPLACE FUNCTION` each back to its pre-migration body (captured verbatim in the file's pre-check comment block, and in migrations 077/117/269/272/303/305/306 for the functions that originated there), then re-`GRANT EXECUTE` to anon/authenticated/service_role — a bare `CREATE OR REPLACE FUNCTION` rollback hits the same 42P13 in reverse. [glyph:verbatim]
