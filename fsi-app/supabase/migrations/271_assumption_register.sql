-- subject: Migration 271 (`assumption_register`, WO-20 assumption register, docs/plans/wo20-assumption-register-spec.md, 2026-08-30). New table, natural key `assumption_key text UNIQUE NOT NULL` (dot-namespaced `<subsystem>.<mechanism>.<parameter>`, e.g. `connections-scorer.weight.shared_source`), the register for modelling constants THIS PRODUCT chose (a connection-scorer weight, an idf coefficient, a score floor, a bias-tag confidence cutoff, an urgency-score mapping, a pedigree floor), distinct from `emission_factors` (WO-12/18), which registers numbers the WORLD published. Hand-written columns (11): `id` uuid PK, `assumption_key`, `subsystem` (denormalized first key-segment, free text v1, spec section 7 Q2), `label`, `rationale`, `code_location` (the drift-detectable file:line pointer), `governing_decision` (ADR id or NULL, NULL is an honest "no ratified decision governs this value today", never a placeholder), `status` (+CHECK active/superseded/retired, default active), `superseded_by` (self-referential FK, append-only-supersession posture), `created_at`/`updated_at`. Plus the envelope, NARROWED (spec section 3): `value_numeric, unit, derivation +CHECK, origin_class +CHECK, source_key FK->data_sources, source_ref, n_observations +positive-CHECK, method_version, as_at_date`, 9 of the full 11-column envelope, `currency` and `reference_period` DELIBERATELY EXCLUDED (none of the 10 catalogued rows are a monetary rate or a period aggregate), 20 columns total. DDL is GENERATED by `src/lib/contracts/provenance-envelope.mjs` via `scripts/gen/migration-271-assumption-register.mjs` (the migration-258/267/268 codegen pattern, adapted to a narrowed `columns` list); an anti-drift test (`src/__tests__/contracts-assumption-register-migration.test.mjs`) asserts the emitted origin_class/derivation CHECKs are BYTE-IDENTICAL to 258's, that `currency`/`reference_period` are absent from the DDL, and that `assumption_key` is the SOLE UNIQUE constraint. ADDITIVE ONLY, no other table touched (asserted by the same test). RLS enabled, read-only to authenticated, no write policy (service-role/guarded-path only, same posture as 258/268). SCHEMA-ONLY, ZERO seed rows in this migration, the 10-row backfill (spec section 5 step 4, one row per section 2 catalogued constant, every `code_location` re-verified this session against the live file, no correction needed on any of the 10) is a separate, later, guarded-path pass (`scripts/gen/assumption-register-seed.mjs` + fixture `scripts/gen/fixtures/assumption-register/wo20-catalogued-assumptions-2026-08-30.json`), not run by this migration and NOT RUN THIS SESSION (dry-run only, no DB access, no service-role credentials; a `--apply` run against a pre-migration database fails loudly on the missing table by design). NUMBERING CORRECTION against the spec, named in the generator's own header: spec section 5 names this migration "269"; 269 and 270 landed as unrelated migrations (`269_routing_rpcs_use_surface_of`, `270_widen_org_watchlist_market_series`) the same day between the spec being written and this file being generated, so 271 is the real next-free number, the spec's DDL content (section 3) is unaffected, only the file number moved. **APPLIED LIVE (confirmed 2026-09-05 by lane W71-WIRE via read-only SQL against the live project: migration version `20260830201604` present in `list_migrations`; `assumption_register` carries 20 live columns, 0 rows).** This corrects the "NOT YET APPLIED" status this row previously carried, the coordinator applied it (two-track policy, CLAUDE.md standing rule 3) at some point between this row being written and 2026-09-05; the exact apply date was not separately recorded and is not re-derivable from `list_migrations` alone (it reports version/name only). **CORRECTION (lane W71-WIRE, 2026-09-05, plan section W7.1):** `scripts/gen/migration-271-assumption-register.mjs` and its anti-drift test (`src/__tests__/contracts-assumption-register-migration.test.mjs`) were DELETED, same one-shot-generator-already-applied reasoning as migrations 267/268 above; this lane byte-compared `renderMigration()`'s fresh output against the live `271_assumption_register.sql` before deleting, exact match. The migration SQL file is untouched. The seed script (`scripts/gen/assumption-register-seed.mjs`) is now wired as a `maintenance.yml` step (dry/apply), see this lane's report; still 0 rows live as of 2026-09-05, so seeding remains a separate, later, coordinator-dispatched `apply`. The admin-panel reader spec section 4 named as not-yet-built is now built, see this lane's report for `src/app/admin/page.tsx` / `AdminDashboard.tsx`. Reversible: `DROP TABLE public.assumption_register` (now has one dependent object, the admin-panel reader, a drop must be paired with removing that panel).
