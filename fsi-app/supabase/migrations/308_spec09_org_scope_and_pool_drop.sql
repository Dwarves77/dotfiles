-- 308 — spec 09 customer-data tables: org-scope the six CSV-upload tables (surcharge_audits,
...
COMMENT ON TABLE public.surcharge_audits IS
  ...
  '1 leaves unmade) were DROPPED by migration 308 along with carrier_compliance_pools itself, 0 rows '
  'confirmed live — see that migration''s header for the full reasoning. org_id (migration 308): every row '
...
COMMENT ON COLUMN public.surcharge_audits.org_id IS 'organizations(id) — the uploading customer org. NOT NULL (migration 308). RLS-enforced via user_belongs_to_org(org_id).';
COMMENT ON COLUMN public.tce_data_quality.org_id IS 'organizations(id) — the uploading customer org. NOT NULL (migration 308). RLS-enforced via user_belongs_to_org(org_id).';
COMMENT ON COLUMN public.auxiliary_energy_profiles.org_id IS 'organizations(id) — the uploading customer org. NOT NULL (migration 308). RLS-enforced via user_belongs_to_org(org_id).';
COMMENT ON COLUMN public.eudr_plot_claims.org_id IS 'organizations(id) — the uploading customer org. NOT NULL (migration 308). RLS-enforced via user_belongs_to_org(org_id).';
COMMENT ON COLUMN public.custody_chains.org_id IS 'organizations(id) — the uploading customer org. NOT NULL (migration 308). RLS-enforced via user_belongs_to_org(org_id).';
COMMENT ON COLUMN public.indexation_clauses.org_id IS 'organizations(id) — the uploading customer org. NOT NULL (migration 308). RLS-enforced via user_belongs_to_org(org_id).';
...
BEGIN
  ...
  VALUES
    ('cl:corridor:0000000000000308', 'cl:organisation:0000000000000308', 'selftest line', 100, 80, 'selftest basis', org_a)
  ...
  RAISE NOTICE 'migration 308 OK: carrier_compliance_pools dropped, 6 tables org-scoped, adversarial cross-org RLS proof passed (org_b denied org_a''s row), 0 rows at rest';
END $$;
