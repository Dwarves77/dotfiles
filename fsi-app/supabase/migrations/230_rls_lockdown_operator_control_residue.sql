-- subject: Migration 230 (RLS lockdown of the anon-writable operator-control residue, P2 structural gate, 2026-07-28). RLS residue audit finding: 8 public tables had RLS DISABLED **and** full DELETE/INSERT/UPDATE/TRUNCATE/SELECT grants to `anon` + `authenticated` → **anon-key writable via PostgREST** (the anon key ships in the client bundle). Operator-control tables exposed: `funded_pass_runlock` (spend runlock), `disposition_ledger` (pipeline audit trail), `mutation_leases` (lease coordination), plus `corpus_census`, `coverage_gap_candidates`, `coverage_gap_census_findings`, `drain_worklist`, `claim_versions`. Fix: `ENABLE ROW LEVEL SECURITY` with NO policies = deny-all to anon/authenticated; `service_role` (BYPASSRLS — every internal write, script, agent runtime via `SUPABASE_SERVICE_ROLE_KEY`) unaffected. Same posture as `census_worklist`. Verified no legitimate anon path first (7/8 zero src refs; `claim_versions` written only by the service-role agent runtime). **APPLIED 2026-07-28**; post-apply verified 0/8 still RLS-disabled. Reversible (`ALTER TABLE … DISABLE ROW LEVEL SECURITY`). [glyph:verbatim]
