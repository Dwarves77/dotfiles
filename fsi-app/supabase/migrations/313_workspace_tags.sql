-- subject: Migration 313 (lane uitags, 2026-09-07, README "Workspace tags" / ruling R6): `workspace_tags` (id, org_id, name, name_key GENERATED lower(trim(name)), created_by, created_at; UNIQUE org_id+name_key) and `item_workspace_tags` (tag_id, intelligence_item_id, org_id, created_by, created_at; PRIMARY KEY tag_id+intelligence_item_id, both FKs ON DELETE CASCADE so a tag or item removal drops its links outright rather than soft-hiding, keeping facet counts a live COUNT(*)). RLS mirrors migration 077's org_watchlist pattern: org-scoped read/insert/delete via `public.user_belongs_to_org(org_id)` OR `auth.role() = 'service_role'`, INSERT additionally requiring `created_by = auth.uid()`. Idempotent (IF NOT EXISTS / OR REPLACE throughout). **APPLIED LIVE 2026-09-07** by the coordinator via Supabase MCP (2 tables, 6 policies confirmed); the file was updated post-apply to match exactly what ran, the redundant `workspace_tags_org_name_idx` (duplicating the unique index on the same org_id/name_key columns) dropped, and the SQL comments' em dashes replaced with commas, both per the coordinator's own applied DDL.
