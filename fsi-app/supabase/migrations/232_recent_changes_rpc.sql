-- subject: Migration 232 (window-scoped What-changed feed, 2026-08-01). Creates `get_workspace_recent_changes(p_org_id, p_days default 7)` (STABLE SECURITY DEFINER, `_assert_org_membership` gated) — all verified live items with `added_date` inside the trailing window, priority-ordered, defensive LIMIT 500. Exists because the dashboard RPC (064) is LIMIT 50 by priority: once the corpus outgrew that slice, in-window additions fell outside it and the home This-week section rendered "nothing" while 216 items had been added in 7 days (2026-08-01 defect). Consumed by fetchDashboardData → WhatChanged (PR #395); render stays capped at 5 via the #393 show-all toggle, so the feed is window-complete and the render is bounded. **APPLIED 2026-08-01** via apply_migration before the consumer code merged (two-track policy). Reversible (`DROP FUNCTION public.get_workspace_recent_changes(uuid, integer)`). [glyph:verbatim]
