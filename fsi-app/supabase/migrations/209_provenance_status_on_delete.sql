-- subject: Migration 209 (operator ruling 2026-07-16, drain root-gap fix). The `set_provenance_status` trigger fired only on AFTER INSERT/UPDATE, never DELETE — so versioning a gate-FAILING claim OUT of the live ledger (the drain prior-junk clearance / hold-loop via `eraseClaimWithProof`) removed the blocker but did NOT recompute `provenance_status`, leaving the item quarantined with a valid ledger until an unrelated write nudged it. Fix: extend `set_provenance_status()` to handle `TG_OP='DELETE'` (derive item id from OLD, RETURN OLD) + add an `AFTER DELETE` trigger on `section_claim_provenance`. A CASCADE delete of the item itself is a safe no-op (the NOT-FOUND guard). DB-layer, so it covers ALL writers by construction (executor-universality ruling). Pure trigger/function DDL, no row rewrites, no backfill; idempotent. **APPLIED 2026-07-16** via apply_migration. Reversible (restore the 139-revision function body + DROP the DELETE trigger). [glyph:verbatim]
