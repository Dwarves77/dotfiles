...
DECLARE
  v_func_src TEXT;
BEGIN
  -- Read the current function source to check for the guard marker.
  SELECT pg_get_functiondef('public.validate_item_provenance(uuid)'::regprocedure) INTO v_func_src;

  IF v_func_src LIKE '%c_own_body_types constant%' THEN
    RAISE NOTICE '207 already applied: c_own_body_types found in validate_item_provenance.';
    RETURN;
  END IF;

  -- Apply the patch: fetch the current function, inject the constant and update the floor logic.
  ...
  DECLARE
    ...
    -- 207: voluntary-instrument family eligible for the own-authoring-body floor.
  ...
  BEGIN
    ...
    IF v_has_sections THEN
      ...
      LOOP
        ...
        -- 202/207: per-fact effective floor. Default = the item-type floor (2 for the reg family).
        ...
        -- issuing-body tier (<= 4) — the instrument's own authoritative text IS its primary.
        -- A same-tier UNRELATED host does NOT qualify; it stays at the item-type floor.
  ...
  RAISE NOTICE '207 applied: extended own-body floor to framework and initiative item types.';
END;
$$;
