-- subject: Migration 282 (Lane DP-SPINE, system-completion train, 2026-09-02, [ADR-024](../decisions/ADR-024-decision-propagation.md); spec `docs/specs/08-flywheel-design.md` section 1.1/section 1.2). The entity spine: `entity_kind` enum (11 values: corridor, node, jurisdiction, organisation, asset, instrument, obligation, method, technology, signpost, person), `entities` (`entity_id` text PK shaped `cl:<kind>:<16-hex-sha256>`, `kind`, `canonical_name`, `status` +CHECK active/merged/retired, `merged_into` self-FK, `created_at`; CHECK `id_matches_kind` ties the id prefix to `kind`; CHECK `merged_has_target` ties `status='merged'` to a non-null `merged_into`), `entity_identifiers` (the crosswalk: `entity_id` FK, `scheme` text, no CHECK, deliberately byte-faithful to spec section 1.1, `value`, `scheme_version`, `asserted_by`, `asserted_at`; PK `(entity_id, scheme, value)`; UNIQUE INDEX `one_entity_per_identifier` on `(scheme, value, coalesce(scheme_version,''))` so the same external id can never resolve to two entities), `entity_scope` (spec section 1.2: `subject_id`/`scope_id` FKs, `relation`, `confidence numeric(3,2)`, `attributed_to`; PK `(subject_id, scope_id, relation)`). RLS on all three: world-readable (`FOR SELECT USING (true)`, sources/regions posture), no write policy (service-role/guarded-path only, same posture as 258/271/277). Idempotent (`IF NOT EXISTS` / `DO $$` enum guard); self-check `DO` block asserts enum cardinality, column counts, the unique index, and probes the id-shape CHECK. Zero rows by construction, backfilled by `scripts/entities/backfill-entities.mjs` (separate, guarded, `--dry`-by-default pass, same commit). Empirically verified against a local Postgres 16 fixture schema (apply clean, idempotent re-run clean, `id_matches_kind` correctly rejects a mismatched-kind id), no live DB access this lane. **APPLIED 2026-09-02** by the coordinator through the Supabase MCP after PR #517 merged, in order 281→287, each self-check green. Reversible: `DROP TABLE public.entity_scope, public.entity_identifiers, public.entities; DROP TYPE public.entity_kind;` (no dependent object outside this migration and 283, which must be reverted first).
