-- subject: Defect D23 part (b), `docs/plans/defect-fix-plan-2026-09-12.md` (lane L15, 2026-09-13): `get_workspace_recent_changes` (migration 232) only ever returned items with `added_date` in the window, i.e. newly minted items, so a week of brief regeneration (which mints nothing) left the dashboard's "What changed" card empty. Widens the RPC to UNION the existing new-item branch with an updated-item branch that reads `item_changelog` rows (`change_type='UPDATED'`) dated in the window, adding two trailing columns `change_kind` ('new' or 'updated') and `change_date`; an item both newly added and changed in the same window reads 'new' only, since `updated_items` excludes ids already selected by `new_items`, so no duplicate rows. DROPs and recreates the function because CREATE OR REPLACE cannot widen a RETURNS TABLE (error 42P13, per 272/310/316's own precedent); no GRANT is repeated since 232 carried none. Additive and backward compatible: old code reading only the original six columns is unaffected. Idempotent (DROP IF EXISTS then CREATE). Two-track policy (standing rule 3): this is schema DDL, applied by the coordinator BEFORE this lane's code merges.
