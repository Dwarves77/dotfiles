-- subject: Migration 119: validate_item_provenance FAIL-CLOSE — a 0-section item no longer vacuously passes criteria 2-5; records no_section_content → quarantined. Sprint 4 Block 1. [glyph:verbatim]
...
COMMENT ON FUNCTION public.validate_item_provenance(uuid) IS
  'Sprint 4 source-provenance invariant gate (design-doc section 3b), migration 119 fail-close revision. Walks the six criteria for an intelligence_items row and returns (valid, failures jsonb[], recommended_status). FIX vs 114: a 0-section item no longer vacuously passes criteria 2-5 — it records a no_section_content failure and routes to quarantined (the prior skip certified empty shells as verified). Sectioned items are validated on 2-5 unchanged. READ-ONLY (STABLE).';
