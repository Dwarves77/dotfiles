...
`intersection_summary` is a short markdown string (≤ 800 chars) describing how this item interacts with the linked items: overlapping requirements, conflicting timelines, sequential compliance dependencies, or operational coupling. Sourced; cite the linked items inline by title.
...
- `intersection_summary` — short markdown string (≤800 chars) describing how this item interacts with the linked items. Sourced; cite linked items inline by title. Empty string OR null when no intersections were identified.
