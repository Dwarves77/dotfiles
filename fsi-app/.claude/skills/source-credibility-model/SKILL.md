...
**SC-13 class-table extension (operator ruling 2026-07-13, 124-host batch).** The codified rule assigns only the floor-passing tiers (legal 1 / gov 2); the register-at-grounding step (`classTierForHost` / `decidePoolHostRegistration`) extends it with the ruled class table so a host that classifies to a ruled class auto-registers at its class tier, and an unrecognized host stays null → worklist. The ruled table: `verifier` (accredited conformity-assessment bodies — DNV/ClassNK/SGS/TÜV/Intertek/Verifavia/Bureau Veritas class; NOT Big-4/advisory) → T4; `academic` (.edu/.ac institutions) → T4; `association` (industry-body allowlist, cer.be precedent) → T4; `analysis` (think-tank / Research-feedstock) → T6; `lawfirm` and `news`/trade-press → T7. The T4 classes demand a high-confidence signal (they can pass the research floor); the sub-floor classes (T6/T7) never pass a floor, so a mis-fire only under-credits. The PERMANENT WORKLIST class — encyclopedia (wikipedia), aggregator (policycommons), DOI-resolver, legal-aggregator (justia/legiscan), and off-domain/unknown — is never auto-registered: a span attributing to one is a re-attribution instruction (re-home to the official publisher or relabel FACT→ANALYSIS), the same family the moat protects. The extension is still deterministic (pattern table, no LLM guess, no default), so it rides the SC-13 invariant, not a new one.
