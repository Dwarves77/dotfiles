...
**The standard floor is the authoring body, scoped by institution (SC-14).** For `item_type='standard'`, the authority floor is the standards-body class tier (T4 — the tier the registry assigns ISO/GRI/USGBC-type bodies), but SCOPED to the item's OWN authoring body's authoritative text, not any same-tier host generically. A FACT whose source shares the item's own-source `institution_id` (the migration-122 institution SSOT) grounds at T4; every other host stays at the reg-family floor T2. Enforced by `validate_item_provenance` (migration 202, the `standard` + own-institution branch) and mirrored in JS by `authorityFloorForFact` (source-blocks.mjs). Rationale: a published standard's own authoritative text (ISO 14083 on iso.org, GRI on globalreporting.org) IS its primary instrument and legitimately grounds at the standards-body tier — but a matched span on a same-tier UNRELATED host is not the standard's primary and MUST NOT be promoted (the moat: authority is the authoring body's own text, never generic same-tier company). The scope is the institution SSOT, never a host-string match.
...
**SC-13 class-table extension (operator ruling 2026-07-13, 124-host batch; `standards_body` class added 2026-09-04).** The codified rule assigns only the floor-passing tiers (legal 1 / gov 2); the register-at-grounding step (`classTierForHost` / `decidePoolHostRegistration`) extends it with the ruled class table so a host that classifies to a ruled class auto-registers at its class tier, and an unrecognized host stays null → worklist. The ruled table: `verifier` (accredited conformity-assessment bodies — DNV/ClassNK/SGS/TÜV/Intertek/Verifavia/Bureau Veritas class; NOT Big-4/advisory) → T4; `academic` (.edu/.ac institutions) → T4; `association` (industry-body allowlist, cer.be precedent) → T4; `standards_body` (standard/framework-publishing bodies whose OWN text this row's T4 already describes — IFRS Foundation/ISSB, CDP, SBTi, GHG Protocol, ISO, GRI, TNFD — curated allowlist `STANDARDS_BODY_ALLOW`, same posture as `association`, never a fuzzy `.org` rule) → T4; `analysis` (think-tank / Research-feedstock) → T6; `lawfirm` and `news`/trade-press → T7. The T4 classes demand a high-confidence signal (they can pass the research floor); the sub-floor classes (T6/T7) never pass a floor, so a mis-fire only under-credits. The PERMANENT WORKLIST class — encyclopedia (wikipedia), aggregator (policycommons), DOI-resolver, legal-aggregator (justia/legiscan), and off-domain/unknown — is never auto-registered: a span attributing to one is a re-attribution instruction (re-home to the official publisher or relabel FACT→ANALYSIS), the same family the moat protects. The extension is still deterministic (pattern table, no LLM guess, no default), so it rides the SC-13 invariant, not a new one. `standards_body`'s own operator ruling (verbatim, on `ifrs.org` / `cdp.net` / `sciencebasedtargets.org` sitting worklisted despite this row already naming "professional standards bodies" → T4): "you know how to classify, fix it … T4" — `scripts/maintenance/institution-canonicalize.mjs` Part C applies it retroactively to the live rows the same ruling covers (see that file's header and `docs/runbooks/MAINTENANCE-RUNBOOK.md` §8a).

**`company` class (D14 residue ruling, 2026-09-13, defect-fix-plan-2026-09-12.md D14)** -> T7: any host with a stored registry name and no earlier class match, since its own site is a primary only for its own announcements and never passes an authority floor.
