...
export const INVARIANTS = [
...

  {
    ...
    enforcedBy: ['selftest:fsi-app/scripts/verify/mint-gates.golden.mjs', 'selftest:fsi-app/scripts/verify/mint-gates-report-only-posture.golden.mjs'],
    residual: 'mint-gates.mjs (perFactGates + identityCongruenceHolds) goldened 13/13 (each gate positive + clean + non-FACT null). The report-only pipeline wiring at canonical-pipeline.ts (gates run over minted FACTs, log would-have-held, NOTHING else) is proven report-only by mint-gates-report-only-posture.golden.mjs (11/11: report block does no db write / no throw, FACT insert unconditional on gates, evaluator advisory). mint-gate-calibration.mjs (read-only, --representative) produces the flip-gating number. The live-flip and A2 (per-document keying, scoped by this calibration) wait behind the operator ruling.',
  },
