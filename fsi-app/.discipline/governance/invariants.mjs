...
export const INVARIANTS = [
  ...
  },
  {
    ...
    text: 'A CLI entry-point guard under fsi-app/scripts/** or fsi-app/.discipline/** must compare import.meta.url against a URL built from process.argv[1] through Node\'s own pathToFileURL/resolve, never a hand-typed file:// string concatenation. [CONFIRMED, task 0.3b, 2026-09-11, by grep across scripts/** and .discipline/**]: 36 files used a guard that compared import.meta.url, with the === operator, against a template literal built by prefixing the six characters "file://" onto process.argv[1], then calling main() when it matched. On Windows, import.meta.url is a real file:// URL with forward slashes while process.argv[1] is a native backslash path, so the hand-built comparison string never equals import.meta.url and the guard is never true: every one of the 36 CLI scripts silently exited 0 with no output when invoked directly on Windows, a runtime defect caught only because the pre-push suite\'s spawned-CLI tests returned empty stdout on the operator\'s Windows machine while the identical tests passed on Linux CI. (The exact broken idiom is not reproduced verbatim in this text because this file itself falls under F44\'s scanned tree; see F44-broken-main-guard.mjs\'s own header for the literal form.)',
