...
export const INVARIANTS = [
  ...
  {
    id: 'SC-13-register-step-deterministic-tier',
    skill: 'source-credibility-model',
    section: 'Canonical Institutional Tier — register-at-grounding assigns a DETERMINISTIC tier only',
    text: 'The register-at-grounding step assigns base_tier DETERMINISTICALLY ONLY: an existing institution-group (eTLD+1) match inherits the canonical tier, or a codified host-class rule applies (legal-primary -> T1, gov/regulator/intergov -> T2). No LLM tier guesses, no default tier. An AMBIGUOUS host (no institution match, no codified rule) is NOT registered — its FACT span NULL-stamps and walls the floor honestly, and surfaceNullTierHosts aggregates the host into one integrity_flag for one batched operator look; a guessed default tier is fake certification (a guessed T5 hollow-passes the technology floor=5).',
...

  {
    ...
    residual: 'The audit (CI-with-secrets / ops lane, DB creds) is the live-data guard: it splits past-bound into undispositioned (HARD tripwire, fails the lane) vs deferred (standing, reported only), applying isValidDeferral on the read side AND re-checking deferred_until is in the future. The write-time guard scripts/lib/deferral.mjs prevents vague deferrals from ever being written (reason must name blocker + disposition path, named future resolution_event, real owner). The meta-gate proves wiring (audit file git-tracked + skill-cited) in the secret-less pre-push. Self-resurrection on expiry is the anti-silence property — a deferral cannot quietly outlive its own clock. The FLAG side is guarded too: deferral-hygiene-audit.mjs names the integrity_flags disposition_deferred rows that rotted — THREE classes, ALL of which FAIL the lane (exit 1): expired-open (deferred_until passed while status=open), deleted-subject (subject item hard-deleted), and RENEWAL-REPACKAGE (operator ruling 2026-07-13, item 3: an open deferral whose reason matches a sibling deferral\'s reason on the same subject — a clock re-set that recycled the prior blocker instead of naming a new one, the RD-6 silencing). "Never writes" means it does NOT blind-resolve/re-open (that is a later disposition dispatch) — it is NOT report-only in the sense of non-failing: expired-open is a failing condition. The write-time guard now also exports isValidRenewal/assertValidRenewal (deferral.mjs): a renewal MUST carry a NEW blocker finding — repackaging the superseded reason with a fresh date is rejected at write time. Standing backlog: 47 expired-open + 5 deleted-subject await a disposition dispatch (flag-rate is not defect-rate); 0 renewal-repackage live. NOT mechanized: whether the named blocker is genuinely the real blocker (vs a plausible-sounding one) is remediation judgment (RD-1), not a checkable property.',
  },
  {
    id: 'RD-30-flag-age-dwell',
    skill: 'remediation-discipline',
    section: 'Section 2.1/2.2: Quarantine is an open investigation; a flag must not dwell open forever',
    text: 'Every OPEN integrity_flag has a bounded dwell across ALL subject_types (surface/source/system/item), not only quarantined-item flags. A non-exempt open flag past the dwell bound (30d) is enqueued for disposition. EXEMPT (valid long-dwell): RD-28-held flags (hold_class=rd28-resting-state — held-with-named-reopener), disposition_deferred (owned by deferral-hygiene-audit), standing-debt/lane markers, and item-flags whose subject is LIVE-QUARANTINED (owned by quarantine-disposition-audit — the item dwell is enforced there, the flag follows the item; this scopes flag-age to the GAP quarantine-disposition cannot see: non-item flags and flags on non-quarantined items).',
    anchor: 'Quarantine Is an Open Investigation (research-or-erase)',
    enforcedBy: ['audit:fsi-app/scripts/verify/flag-age-audit.mjs'],
...

  {
    ...
    skill: 'remediation-discipline',
    section: 'Section 4 — category 23: Funded-pass run-lock (no concurrent worklist driver)',
...

  {
    ...
    text: 'Every size cap on the capture→synthesis→grounding→judge path is either sized so it never binds in normal operation, or fails LOUD (surfaced wall — recordTruncation → coverage_gap flag + item held with a named reason) when it binds. Silent slicing is forbidden (the GROUND_SECTION_MAX_CHARS=12000 category-2 defect that hid the back of every long section from the grounder). Every path cap is enumerated + classified; a new unregistered/silent cap is RED. EVERY WRITER, not every module: a cap binds on the COLUMN, so it must hold at every process that writes that column — agent_run_searches.result_content_excerpt has two independent writers (the Next.js pipeline and the Deno capture-worker, which cannot import the pipeline\'s config) and the ceiling was live on one of them, which is the same silence one layer out.',
    anchor: 'The size-cap doctrine (no silent slice on the grounding path)',
    enforcedBy: ['fitness:F17', 'fitness:F26', 'selftest:fsi-app/src/lib/agent/section-grounding.test.mjs', 'selftest:fsi-app/.discipline/fitness/functions/F26-storage-ceiling-parity.test.mjs'],
    residual: 'F17 (grep-class, red-then-green: a new unregistered *_MAX_CHARS/*_BUDGET_CHARS/*_CEILING_CHARS on the path is RED; a registry entry marked silent-grounding is RED) gates the STRUCTURAL guarantee — no silent cap slips onto the path unclassified. section-grounding.test.mjs proves the fix red-then-green: a binding fact beyond the old 12KB boundary is invisible to the old slice(0,12000) and reaches the grounder complete post-fix; a pathological over-ceiling section is SURFACED (truncated=true + fullLength), never silent. NAMED RESIDUALS: (1) the classify-path excerpt caps (haiku-classify ~6000, first-fetch ~6000, recommend-tier 4000) are SILENT binders on the CLASSIFICATION path (tier/type/portal), not the fact-grounding path — LOW risk (source nature is evident early), registered in the cap-inventory as REVISIT, not converted now; (2) F17 registers cap CONSTANTS on generation-config + section-grounding — an inline `.slice(0, N)` re-introduced elsewhere on the path is caught by review + the section-grounding selftest, not by F17 grep (the constant-declaration form is the robust signal). Full inventory: docs/design/cap-inventory-2026-07-06.md. SECOND-WRITER HALF (2026-08-17): F17 registers cap constants inside the Next.js tree, so it structurally could not see the Deno capture-worker, and the ADR-016 storage ceiling was enforced on one of the column\'s two writers — three captures landed over it (17,787,345 / 12,579,090 / 10,351,091 chars, all capture-worker:first-fetch, all dated AFTER the 2026-07-21 ruling) with no signal, because the unguarded path had nothing to fire. F26 closes it as PARITY rather than presence: both writers must resolve STORAGE_MAX_CHARS from the same env var with the same fallback literal (a hand-copied constant IS the divergence — cf. the gate_a_* version literal in the 2026-08-11 db-layer census), and a worker bind must be LOUD (the [truncation-guard] warn AND the integrity_flags write, mirroring recordTruncation()). Negative-tested by F26-storage-ceiling-parity.test.mjs (19 assertions, fixture-driven: divergence in BOTH directions, ceiling removed, ceiling hard-coded, wrong env var name, and the silent-ceiling cases that satisfy parity while quietly slicing the pool). NAMED RESIDUAL: F26 is a STATIC source-text check by necessity — the two runtimes cannot share an import, so agreement can only be verified in the text before deploy; a divergence introduced by setting the STORAGE_MAX_CHARS env var to different values in the Vercel and Supabase environments is invisible to it, and is the out-of-repo-boundary class (docs: the env/hooks/DB config boundary manifest), not a code gate.',
  },
  ...
  },
  {
    ...
    enforcedBy: [
      ...
      'selftest:fsi-app/.discipline/rendering/smoke/smoke.test.mjs',
    ],
    residual: 'The portable selftest (in the no-npm `node --test` discipline glob) proves the THREE detectors red-then-green over the REAL app modules: the overflow detector (fitting-vs-overflowing measurement), the F-1 placeholder detector reusing isPlaceholderText + HEADER_LITERALS over the pre-fix vs current stripSourcesSection output, and the V-07 hydration detector reusing the real stableDateLabel (now-independent, green) vs relativeTimeLabel (now-dependent, red). run-rendering-guard.mjs renders the extreme-data fixtures at every app breakpoint tier (380/420/480/560/640/767/768/900/960/1100/1200/1440) in Playwright chromium and measures REAL layout (scrollWidth/clientWidth — impossible in jsdom), feeding the SAME detectors; it runs in the dedicated CI "Rendering guard" job, which is NON-BLOCKING (continue-on-error, the SOFT/data-audit-lane pattern) and does NOT enter the required-check set until 3 consecutive green runs on master (operator policy 2026-07-11 — a flaky required check is a merge embargo). The portable selftest above IS a required gate, so the detection logic gates today; only the real-browser leg is non-blocking while it stabilizes. Anchor is a stable SF section marker; the invariant is owned by this guard mechanism. ASSERTION-CLASS RESIDUAL (day-one catch, 2026-07-11): the guard\'s HARD assertions must prefer STRUCTURAL truths (scrollWidth <= clientWidth, no placeholder literal, no hydration error) over ABSOLUTE pixel widths — cross-OS text measurement always carries a small tolerance band (Linux fallback digits render ~11px wider than Windows), so pixel deltas are INFORMATIONAL, not pass/fail. FIDELITY RESIDUAL, PARTIALLY CLOSED (lane GATES-1, 2026-09-02): the "full-page E2E... not-yet-built extension" named below is now built for four surfaces — .discipline/rendering/smoke/*.mjs bundles the REAL src/components/** module with esbuild (not a facsimile) and mounts it via ReactDOM.createRoot in the same Playwright chromium instance, wired into run-rendering-guard.mjs\'s own run as an "SM smoke specs" pass; smoke.test.mjs proves its pure detector-reuse (guard-assert.mjs) and fixture-builder parts red-then-green in the required no-npm suite, the same posture assertions.test.mjs already has relative to the fixture legs. STILL NAMED RESIDUAL: the browser fixture legs proper (the pre-existing, non-smoke fixtures.mjs pairs) still reproduce each fix\'s layout contract as a facsimile and declare Plus Jakarta Sans without bundling it; the four smoke specs cover watchlist(team)/personal-archive/list-order/notifications only, not every customer surface (see RD-58\'s surface-acceptance register for which surfaces have a rendering_spec at all), and none of the four run under real auth against live data — fixture data only, no network, matching this whole guard\'s $0 posture. Extending the remaining fixture legs to real components, and extending smoke coverage to the surfaces RD-58\'s register currently exempts once they are built, is the still-open piece of NA-1/NA-2.',
  },
  ...
  {
    id: 'RD-9b-producer-composition-proof',
    skill: 'remediation-discipline',
    section: 'Section 4 - category 9 (extension): the seam half-slice - both halves proven, the join proven by nothing',
    text: 'Every producer entry point under fsi-app/scripts/producers/** must have at least ONE proof that imports EVERY first-party seam module its write path imports. Two proofs that each cover half of a producer\'s seam set do not prove the seam between them. This is the half-slice defect one level in from RD-9: RD-9 catches a table with a writer and no reader; this catches a producer whose parts each have a green unit test while the COMPOSITION that actually runs in production is exercised by nothing. Twice-demonstrated, not theoretical: (1) 2026-08-30, run-envelope-producer built rows inline instead of routing through buildEnvelopeRow, writing a NULL into regional_data_facts.value (NOT NULL) - buildEnvelopeRow was fully unit-tested and the suite, tsc, the fitness functions and the discipline engine were all green while the producer was broken; (2) the same day, scripts/producers/market/eu-weekly-oil-bulletin.mjs composed parseEuWeeklyOilBulletinCsv -> planMarketSeriesUpsert with both halves proven separately and the seam proven by nothing but a live apply against production. A proof must assert the composed rows against the DESTINATION TABLE\'s real constraints (the NOT NULL columns above all: regional_data_facts.value, market_series.label), because that is the failure the seam produces.',
    anchor: 'Producer-consumer orphan (the half-slice defect)',
    enforcedBy: ['fitness:F27', 'selftest:fsi-app/.discipline/fitness/functions/F27-producer-seam-proof.test.mjs', 'selftest:fsi-app/src/__tests__/market-producer-composition.test.mjs'],
    residual: 'F27 (filesystem-pure: resolves each producer\'s first-party import specifiers to repo-relative paths and requires one proof file whose own import set is a superset) gates the STRUCTURAL guarantee - a new producer, or a new seam added to an existing one, cannot ship with its composition unexercised. market-producer-composition.test.mjs proves the BEHAVIOUR for the market lane: real fetcher-shaped CSV at the production-verified values of week 2026-08-24 through parse -> plan, asserting every planned row against the live market_series constraints read from the database (series_key format regex, label NOT NULL, derivation and origin_class enums, n_observations > 0 or null, UNIQUE(series_key, reference_period) via the skipped-no-reference-period path) plus idempotency on a second pass. WHAT THIS DOES NOT DO, stated plainly: F27 cannot tell you a fixture matches REALITY. Both Wave 13 defects (the EU block keyed on a legend string; the A1087 "Notes:" cell in the date column) were fixtures faithfully encoding a wrong belief about the source file, and a composition proof over those fixtures would have been green. That class is held by ADR-023 point 4 - first live run is dry, a human reads the plan, then apply - which is what actually caught both, loudly, writing nothing. F27 holds the seam; the dry-run rule holds the reality; neither substitutes for the other. NAMED RESIDUAL: coverage is import-set superset, not semantic - a proof that imports all the seams but exercises them shallowly satisfies the gate, so the assertion quality stays authoring discipline enforced at review.',
  },

  {
    id: 'RD-55-harness-run-integrity',
    skill: 'remediation-discipline',
    section: 'Section 4 — category 31: Harness-run integrity (an iterated harness\'s own run history is a class fix target too)',
    text: 'Every iterated harness (mint, screen, fetch-drain, and any family added later) writes a run artifact validating against scripts/harness-runs/CONVENTION.md\'s schema as part of its OWN execution path, never as a step an operator remembers separately. A harness family\'s governing files (each family\'s own family.json) MUST NOT drift from the harness_version every recorded run in that family names without EITHER a newer run whose own harness_version matches the drifted state, OR a file under that family\'s own pending/ directory naming the change and the run that will supersede it, the mechanical form of "the harness changed without a run recording why," re-derived from the live tree and git range at check time rather than a hash stored in the file (build plan section 6.8 Rule B, lane N3 2026-09-19). A family with ≥2 artifacts additionally carries a LAST-PROPOSER-PASS.md naming its latest run, the machine-checkable half of PROPOSER-RUNBOOK.md\'s "no harness runs its next batch until a proposer has read the full prior record" precondition. Concrete incident this codifies: round 1 of the screen harness (3,661 rows, 349 decided) wrote its raw per-row output to a FIXED path, not a run-numbered one; round 2\'s run against the same --out-basename silently overwrote it, leaving only an aggregate count in a commit message, a summary standing in for a lost full trace, the exact failure class arXiv 2603.28052\'s ablation measured as WORSE than no trace at all (38.7% vs 56.7% for full raw traces).',
    anchor: 'Every harness-run artifact validates against its schema, every registered harness family has at least one recorded run, and a harness family\'s governing files MUST NOT drift from the harness_version its recorded runs name without EITHER a newer run or a pending-run marker recording the change and the run that will supersede it',
    enforcedBy: ['fitness:F28', 'selftest:fsi-app/.discipline/fitness/functions/F28-harness-run-integrity.test.mjs'],
    residual: 'scripts/lib/run-artifact.mjs\'s writeRunArtifact/readRunHistory (Wave MH-1, filesystem-pure) enforce the PER-ARTIFACT schema fail-closed at write time, an invalid artifact is never written to begin with, on the writing side. F28 (Wave MH-2, rewritten lane N3 2026-09-19 per build plan section 6.8 Rule B; filesystem + git, no network/DB/model: reads scripts/harness-runs/**/*.json plus each family\'s own pending/ directory and LAST-PROPOSER-PASS.md, re-hashes GOVERNING_FILES from disk, and reads one git-diff pair against the resolved range) is the READING-side/COMMITTED-TREE half: it catches an artifact hand-edited into invalidity after the fact, a harness whose governing files changed in a range with no run or pending file added in that same range (the range rule, skipped rather than failed when no git range resolves), a family with no artifact at the CURRENT live hash and no pending file (the tree-state rule), and a family with ≥2 runs and no proposer attestation. NAMED RESIDUAL, stated in F28\'s own header: harness_version is a WHOLE-FILE content hash, a comment-only or prose-only edit to a governing file trips the tree-state rule exactly like a behavior change, by CONVENTION.md\'s own deliberate design ("any edit to any hashed file changes it... lets a proposer lane tell the harness changed apart from the input changed without reading a diff"); F28 does not attempt a semantic diff, since that would reintroduce the judgment-call honor system this registry exists to remove. The pending/ escape hatch is itself reverse-audited (F28\'s auditPendingTreeState: an artifact landing at the live hash with pending files still present is itself a violation, "the run happened, delete them") so it cannot become a permanent exemption once its promised run lands, mirroring the F14/F22/F24/F25/F27 shrinking-allowlist idiom. Nothing is stored any more (no hash pinned inside a marker file), which is the specific fix for the prior residual this entry used to name: two lanes each writing a correct pending file to two DIFFERENT filenames never collide, where two lanes each writing a correct hash to the SAME line always did (17 scripted re-pins across four logged days, three lanes colliding on one marker on 2026-09-18). F28 cannot verify a run artifact\'s CONTENT is honest (a fabricated per_item entry, an inflated metric), only that the schema shape and the governing-file/pending-file/attestation coupling hold; content honesty stays authoring discipline, the same residual CONVENTION.md\'s "per_item at scale" rule names for the writer side.',
  },

  {
    id: 'RD-56-derived-values-gate',
    skill: 'remediation-discipline',
    section: 'Section 4 — category 32: Propagation engine gates (a computed value\'s pollution barrier and a statutory formula\'s purity are class fix targets, not per-caller trust)',
    text: 'Spec 08 §3.3\'s pollution barrier, made structural: nothing outside src/lib/propagation/ reads the raw derived_values table directly. Migration 285 denies raw-table SELECT via RLS and grants only derived_values_admissible (the view, already excluding stale/falsified/obsolete rows); F31 (derived-values-gate) is the SECOND enforcement point the spec names — a service-role client, which bypasses RLS entirely, could otherwise read derived_values directly and see ungated rows with no admissibleFor() check applied. F31 fails CI on a literal .from("derived_values") call site (any quote style) anywhere outside src/lib/propagation/; derived_values_admissible never matches (a different string), so reading the view — already-admitted data — is unaffected anywhere in the repo.',
    anchor: 'Nothing outside `src/lib/propagation/` MUST read the raw `derived_values` table directly — every consumer reads through `derived_values_admissible` (RLS-granted) or calls `admissibleFor()` (the pollution barrier)',
    enforcedBy: ['fitness:F31', 'selftest:fsi-app/.discipline/fitness/functions/F31-derived-values-gate.test.mjs'],
    residual: 'F31 is filesystem-pure (globs fsi-app/src/**/*.{ts,tsx,mjs,js} and fsi-app/scripts/**/*.mjs, excluding test/story files) and per-file, matching F21\'s single-grounding-entry shape: a per-line regex over each file\'s content, sanctioned by directory prefix rather than a fixed file list (src/lib/propagation/ is expected to GROW as later lanes add method modules, unlike F21\'s closed SANCTIONED set). It catches the ONE bypass shape named in the task brief and this invariant (.from("derived_values") in application/script code); a raw SQL string issuing the same read from outside a migration file is a materially different, currently-unheld residual — named, not silently narrowed, in F31\'s own header. RLS (migration 285) remains the FIRST enforcement point (a role with no grant on the base table cannot read it regardless of what F31 catches); F31 backstops the service-role/bypass-RLS case specifically.',
  },

  {
    id: 'RD-57-statutory-purity',
    skill: 'remediation-discipline',
    section: 'Section 4 — category 32: Propagation engine gates (a computed value\'s pollution barrier and a statutory formula\'s purity are class fix targets, not per-caller trust)',
    text: 'Spec 08 §4 Layer 3: a legally-prescribed statutory figure must never quietly absorb a model\'s guess or an unvalidated estimate as one of its own inputs. Migration 286\'s assert_statutory_purity() trigger inspects a statutory_computations row\'s OWN declared inputs jsonb array directly (adapted from spec\'s own transitive-DAG-walk body — statutory_computations is never a to_value_id in derivation_edges, so there is no DAG edge to walk from this table; the migration\'s own header names this deliberate deviation) and refuses any element naming a live estimated_values row, or a derived_values row whose derivation is modelled/estimated/interpolated — proven live by that migration\'s own self-check (a real estimated_values row, a real rejected INSERT). F32 (statutory-purity) is the filesystem-only backstop: it fails if the repository copy of migration 286 no longer defines assert_statutory_purity() or no longer attaches it as a BEFORE INSERT OR UPDATE trigger on statutory_computations, and ships a tested, pure JS mirror (assertStatutoryPurity) transcribed byte-for-byte from the SQL trigger\'s own two-EXISTS-query logic for a future JS-side pre-flight caller.',
    anchor: 'A statutory computation\'s declared inputs MUST NOT cite a live `estimated_values` row or a `derived_values` row whose derivation is modelled/estimated/interpolated — a legally-prescribed figure never quietly absorbs a model\'s guess',
    enforcedBy: ['fitness:F32', 'selftest:fsi-app/.discipline/fitness/functions/F32-statutory-purity.test.mjs'],
    residual: 'F32 CANNOT run the live DB proof itself (filesystem-only, no database, matching every other holistic function in this registry) — the actual write-time rejection is proven by migration 286\'s own embedded self-check DO block (run against a local scratch Postgres for this lane, not the live project — named as an [UNCONFIRMED]-against-production item in this lane\'s own REPORT). F32\'s structural-presence regex is a REGRESSION guard (the trigger disappearing or being downgraded to AFTER/moved to another table), not a re-proof of correctness on every commit. The JS mirror\'s NAMED residual (migration 286\'s own header, restated here): the check is SHALLOW (a row\'s own inputs column) rather than a transitive graph walk, which is safe today because Layer 2\'s type barrier (StatutoryInput, types.ts) is the only sanctioned path into this table and a bypass caller\'s inputs column is exactly what both the trigger and the mirror inspect — a future multi-hop statutory composition (a statutory result cited as another statutory computation\'s own input) would need the schema widening the migration\'s header already names, not a fix to this invariant\'s enforcement alone.',
  },

  {
    id: 'RD-58-surface-acceptance-register',
    skill: 'remediation-discipline',
    section: 'Section 4 — category 33: Surface-acceptance register (a customer surface\'s own existence is a class fix target, not just its rendering fidelity)',
    text: 'Every customer-facing surface named in docs/specs/01..11-*.md must be either PRESENT (a register entry naming a real route, a real data_path, and a real rendering_spec, each verified against the actual import graph — reachability, not a name that merely looks plausible) or EXEMPTED (a register entry naming reason, ruled_by, and date). A surface silently missing from both the register and the spec text is the gap this closes: SF-10 proved that a BUILT surface renders its data extremes cleanly; this proves a surface the specs OBLIGATE the product to have was not quietly dropped between planning and delivery, and turns "is this built yet" from a manual per-release audit into a self-reporting register a reviewer can diff. This is the same class as RD-9/RD-9b (a promised half with nothing proving the other half exists) applied one level up, to the surface itself rather than to a table\'s writer/reader pair.',
    anchor: '## Section 4: Remediation Strategy by Category',
    enforcedBy: ['fitness:F33', 'selftest:fsi-app/.discipline/fitness/functions/F33-surface-acceptance.test.mjs'],
    residual: 'Anchor is a stable skill-section marker (SF-8/SF-9\'s precedent: no single existing line in remediation-discipline/SKILL.md names surface acceptance, since category 33 did not exist before this lane) — the invariant itself is owned by F33 and this registry entry, not by prose already in the skill file. F33 (filesystem-pure: reads surface-acceptance-register.json, hardcodes the spec-derived surface list with basis citations in its own source rather than re-parsing the spec prose, and reuses buildImportGraph — already the F25-module-liveness reachability engine, not a second implementation — to verify a register entry\'s route/data_path actually resolve and connect in the real import graph) catches an entry that NAMES a path nothing imports, and a spec-named surface present in neither the register nor an exemption. NAMED RESIDUAL: F33 cannot verify a rendering_spec path is the CORRECT smoke/rendering proof for that surface, only that the path exists and is reachable — the semantic match between a surface and its cited proof stays authoring discipline enforced at review, the same posture F27 states for producer-seam import supersets. F33 also cannot detect a spec being edited to remove an obligation instead of building or honestly exempting it — the register is honest only as of the humans who wrote docs/specs/01..11-*.md; a silently narrowed spec is a doctrine-authoring failure outside a fitness function\'s reach.',
  },
  {
    id: 'RD-59-bundle-safe-module-evaluation',
    skill: 'remediation-discipline',
    section: 'Section 4 — category 34: Bundle-safe module evaluation (a module that runs on import must not depend on files the deployed bundle does not carry)',
  ...
  {
    id: 'RD-63-unbounded-supabase-read',
    skill: 'remediation-discipline',
    section: 'Section 4 — category 38: A capped read that claims to fetch "the whole set" pages, it does not lean on a bigger .limit() (PostgREST\'s db-max-rows silently truncates any single request past 1,000 rows, no matter what N a caller asks for)',
    text: 'A `.limit(N)` call with N > 1,000, or a full-set read with no `.range()`/`.limit()` at all where the code path assumes it received every row, is forbidden outside the shared fetchAllRows/exactCount helpers (src/lib/db/paginate.mjs) — Supabase PostgREST enforces a server-side db-max-rows=1000 cap that a larger requested limit does not override, so any such call silently returns at most the first 1,000 rows in whatever order the query happens to produce, and any "total" derived from that array\'s .length is not a count, it is a truncated sample wearing a count\'s clothes. [CONFIRMED, live, 2026-09-05, lane CAP-1000, "two defects one cause"] carosledge.com\'s [slug] prerender coverage: PERF-13\'s getPublicSurfaceSlugs called the public listings RPC with `.limit(BUILD_TIME_SLUG_ENUM_LIMIT = 20000)` against a live 1,312+-row regulations corpus — slug index 900 was a prerender HIT, 1000/1050/1200/1311 were all MISS, exactly the first 1,000 enumerated. The SAME bug class independently produced: the obligations register masthead reading "60 of 1000 obligations" against a live 1,141-row table (read-register.mjs\'s OVERFETCH_CAP=2000 JS-side overfetch-then-filter, whose displayed "total" was the capped fetch\'s array length, not a real count); community/directory/page.tsx\'s profiles aggregate silently dropping rows past 5,000 (found live by this invariant\'s own fitness function\'s whole-tree test, not named in the original dispatch); and run-change-detection.mjs\'s readPendingDrainRows deriving its backlog "overflow beyond --drain-limit" figure from a capped array\'s length, silently reading 0 overflow once the true staged_updates backlog exceeded 1,000 rows no matter how deep it actually ran.',
    anchor: '### Section 4 — category 38: A capped read that claims to fetch "the whole set" pages, it does not lean on a bigger .limit()',
    enforcedBy: ['fitness:F38', 'selftest:fsi-app/.discipline/fitness/functions/F38-unbounded-supabase-read.test.mjs'],
    residual: 'F38 is a LEXICAL scanner over `fsi-app/src/**/*.{ts,tsx,mjs}` and `fsi-app/scripts/**/*.mjs` (test files and `/_archive/` excluded): it flags a `.limit(<literal>)` call with a literal > 1,000, or `.limit(<SAME-FILE ALL_CAPS CONST>)` where the const\'s own declared value exceeds 1,000, and REDS a registered ALLOWLIST entry once its named `expiry` train has passed (per F25\'s `latestTrainWave`, reused rather than re-implemented). By deliberate scope decision (stated in F38\'s own file header) it does NOT attempt generic detection of an unranged `.rpc()`/`.select()` call with no `.limit()`/`.range()` at all — a codebase-wide scan for that shape would false-positive on the many legitimate single-row/small-result RPCs (a lookup by primary key, an aggregate RPC that returns one row, etc.), so a genuinely new full-table read with no limit clause of any kind is NOT mechanically caught by this invariant; it would need to be found the way this lane found the community/directory instance — by a human or a fitness function\'s own live end-to-end test discovering the symptom, not by the static scanner naming the site directly. The ALLOWLIST is not empty by construction: `scripts/verify/mint-gate-calibration.mjs`\'s `.limit(8000)` is a deliberate calibration-sample width, not a claimed full-set read, and is named with a reason and an expiry (train 46) rather than silently exempted.',
  },
  {
    id: 'RD-64-unbounded-in-filter',
    skill: 'remediation-discipline',
    section: 'Section 4 — category 39: a PostgREST `.in(col, list)` filter URL-encodes its whole list into the GET request; a runtime id list whose size follows the data, not a literal or a provably capped slice, eventually exceeds the gateway\'s URL-length limit and the request itself 400s',
    text: 'A `.in(column, list)` call whose `list` is a runtime value (a variable, a property access, a method-chain expression) is forbidden outside the shared readAllByIds/guardedUpdateByIds/fetchAllByIdChunks helpers (scripts/lib/db.mjs, src/lib/db/paginate.mjs) unless the call site carries a same-line or preceding-line `// fitness-allow: F39 (reason)` marker naming why the list is bounded — PostgREST URL-encodes an `.in()` filter\'s entire list into the request line, and past roughly 2,000 UUIDs (about 80KB) or any sufficiently long id list the gateway answers 400 Bad Request or an HTML error page instead of the JSON the caller expects, independent of and in addition to the row-count truncation RD-63/F38 already covers (RD-63 is about a RESPONSE silently truncated past 1,000 rows; this is about a REQUEST that never completes at all, because the URL itself is too long). [CONFIRMED twice, lane INCLAUSE-CLASS, 2026-09-06]: review-apply wrappers (run 34045479342, 911 ids) and census-off-vertical.mjs (Maintenance run 34046850770, 1,655 ids, raw `<!DOCTYPE html>` error page returned in place of JSON) — in the second case the error surfaced only on the POST-write read-back, after the chunked write itself had already succeeded, live-confirming 1,655 rows correctly archived despite the read-back\'s own failure.',
    anchor: '### Section 4 — category 39: a PostgREST `.in(col, list)` filter URL-encodes its whole list into the GET request',
    enforcedBy: ['fitness:F39', 'selftest:fsi-app/.discipline/fitness/functions/F39-unbounded-in-filter.test.mjs'],
    residual: 'F39 is a LEXICAL scanner over `fsi-app/src/**` and `fsi-app/scripts/**` (test files and `/_archive/` excluded): it flags a `.in(col, X)` call whose `X` is not an array literal, a string/template literal, or a SCREAMING_SNAKE_CASE constant reference — i.e. a shape that reads as bounded by construction versus one that reads as a runtime-sized value. Unlike F38, F39 carries NO allowlist-with-expiry escape hatch by deliberate design (this lane\'s explicit dispatch constraint): a site that cannot be proven bounded gets fixed (routed through the shared chunking helpers) or, when investigation shows it genuinely is bounded by a mechanism the lexical scanner cannot see (a `.limit()` clause earlier in the same function, a request-body length validated upstream, a singleton/per-item/per-user scope), gets an inline `// fitness-allow: F39 (reason)` marker stating that bound at the site itself, permanently, not in a separate file with a countdown. F39 cannot verify that a marker\'s stated reason is actually true — it only verifies a reason was written down; a false or stale marker is a governance-discipline failure the same way a false `[CONFIRMED]` label would be, not a gap F39 itself closes. Like F38, F39 does not attempt generic dataflow analysis to prove a variable\'s bound automatically — it recognizes bounded SHAPES at the call site, not bounded VALUES traced through arbitrary prior code.',
  },
  {
    id: 'RD-65-authed-api-fetch',
    skill: 'remediation-discipline',
    section: 'Section 4 — category 40: a browser call to a requireAuth-guarded route attaches the session token in ONE place, never at the call site',
    text: 'A browser module calling a requireAuth-guarded `/api/` route is forbidden from attaching the session token itself: it goes through the one shared authenticated fetcher (`authedFetch`/`authHeaders`, src/lib/api/authed-fetch.ts), and no module under src/ outside that file and the two server modules that build a Supabase client FROM an already-verified token (src/lib/api/auth.ts, src/lib/api/community-auth.ts, plus the OAuth callback route) may write an `Authorization: Bearer` header. requireAuth reads that header and nothing else, so a caller that sends a cookie instead, or a bearer carrying no token, gets a 401 that fails soft and is invisible to every test that mounts components against canned fixtures. [CONFIRMED, production, click-through audit 2026-09-08]: src/lib/tags/client.ts and useWorkspaceTagsFacet.ts sent `credentials: "include"` and a Content-Type, so all seven workspace-tags requests 401d for every signed-in user from the day the feature landed and the + Tag popover, the detail tag row and the list rail facet were dead on every page — 23 GET /api/workspace/tags 401 in three hours on a signed-in session while /api/auth/identity, /api/workspace/bootstrap and /api/admin/attention returned 200 on the same page loads. A census then found 24 further files hand-rolling the header four ways, 21 of them interpolating the token unconditionally (`Bearer ${session?.access_token || ""}`, or literally `Bearer undefined`), which clears requireAuths startsWith check and then 401s on any read that fires before the session resolves.',
  ...
  },
  {
    id: 'RD-68',
    skill: 'remediation-discipline',
    section: 'Section 4 - category 44: a CLI main guard built from a hand-typed file:// string breaks on every Windows machine',
    text: 'A CLI entry-point guard under fsi-app/scripts/** or fsi-app/.discipline/** must compare import.meta.url against a URL built from process.argv[1] through Node\'s own pathToFileURL/resolve, never a hand-typed file:// string concatenation. [CONFIRMED, task 0.3b, 2026-09-11, by grep across scripts/** and .discipline/**]: 36 files used a guard that compared import.meta.url, with the === operator, against a template literal built by prefixing the six characters "file://" onto process.argv[1], then calling main() when it matched. On Windows, import.meta.url is a real file:// URL with forward slashes while process.argv[1] is a native backslash path, so the hand-built comparison string never equals import.meta.url and the guard is never true. [CORRECTED, fix round 1, reviewer-confirmed]: 31 of the 36 files had no fallback and silently exited 0 with no output when invoked directly on Windows, a runtime defect caught only because the pre-push suite\'s spawned-CLI tests returned empty stdout on the operator\'s Windows machine while the identical tests passed on Linux CI; the other 5 (skill-map.mjs, skill-contract-map.mjs, bootstrap-test1.mjs, defect-signature-scan.mjs, no-generic-source-audit.mjs) carried a working `|| process.argv[1]?.endsWith(...)` fallback naming their own filename, which still called main() correctly despite the broken primary comparison, fragile but not silently broken. (The exact broken idiom is not reproduced verbatim in this text because this file itself falls under F44\'s scanned tree; see F44-broken-main-guard.mjs\'s own header for the literal form.)',
    anchor: '### Section 4 - category 44: a CLI main guard built from a hand-typed file:// string breaks on every Windows machine',
    enforcedBy: [
      'fitness:F44',
      'selftest:fsi-app/.discipline/fitness/functions/F44-broken-main-guard.test.mjs',
