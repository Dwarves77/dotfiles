...
export const INVARIANTS = [
...

  {
    ...
    residual: 'scripts/lib/run-artifact.mjs\'s writeRunArtifact/readRunHistory (Wave MH-1, filesystem-pure) enforce the PER-ARTIFACT schema fail-closed at write time — an invalid artifact is never written to begin with, on the writing side. F28 (Wave MH-2, filesystem-pure: reads scripts/harness-runs/**/*.json plus each family\'s PENDING-RUN.md/LAST-PROPOSER-PASS.md, re-hashes GOVERNING_FILES from disk) is the READING-side/COMMITTED-TREE half: it catches an artifact hand-edited into invalidity after the fact, a family with zero run history, a harness whose governing files changed without ANY run or marker recording why, and a family with ≥2 runs and no proposer attestation. NAMED RESIDUAL, stated in F28\'s own header ("KEEP IT HONEST"): harness_version is a WHOLE-FILE content hash — a comment-only or prose-only edit to a governing file trips the staleness rule exactly like a behavior change, by CONVENTION.md\'s own deliberate design ("any edit to any hashed file changes it... lets a proposer lane tell the harness changed apart from the input changed without reading a diff"); F28 does not attempt a semantic diff, since that would reintroduce the judgment-call honor system this registry exists to remove. The PENDING-RUN.md escape hatch is itself reverse-audited (F28\'s auditStalenessCoupling) so it cannot become a permanent exemption once its promised run lands, mirroring the F14/F22/F24/F25/F27 shrinking-allowlist idiom. F28 cannot verify a run artifact\'s CONTENT is honest (a fabricated per_item entry, an inflated metric) — only that the schema shape and the governing-file/marker/attestation coupling hold; content honesty stays authoring discipline, the same residual CONVENTION.md\'s "per_item at scale" rule names for the writer side.',
  },
