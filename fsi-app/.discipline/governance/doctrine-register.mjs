...
export const DOCTRINES = [
  ...
  },
  {
    ...
    statement:
      'Data is SAVED, and ERASED only when it is inaccurate. A current claim is removed ONLY when a grounding PROVES it wrong against the primary (its span contradicts the enacted text, or its source is superseded) — never because a regeneration failed to reproduce it. Erasure carries its proof: the single erase path (eraseClaimWithProof) requires a proof, archives the claim\'s final state WITH that proof, and only then deletes the current row — fail-closed, so a claim and its proof are never lost. Grounding cannot erase anything; erasure is a deliberate, proof-carrying action. ADDENDUM (operator ruling 2026-07-16, the over-clear incident): PROOF means EVERY condition of the erase ruling is EVIDENCED per claim — a mechanical PROXY for a judgment condition is NOT proof. The drain prior-junk clearance requires BOTH (a) the span is absent from the item\'s verified primary AND (b) the claim text NAMES a foreign instrument identifier (positive cross-instrument evidence), and only on a primary that is target-match CONFIRMED via an instrument identifier (not subject-overlap); span-absence ALONE is a proxy, not proof (a wrong/incomplete declared primary makes a same-subject claim look absent). Automation may EXECUTE a proven clearance; it may NOT INFER the proof. A claim failing either condition is relabeled (true-but-secondary) or routed to per-claim review, never auto-erased.',
    source: 'operator RULING — preserve-not-overwrite (2026-07-16) + addendum (the over-clear incident, 2026-07-16): proof is per-condition evidence, not a proxy; automation executes a proven clearance, never infers it. TAXONOMY ADDENDUM (2026-07-16): erase-only-on-proven-inaccuracy now carries EXACTLY THREE sanctioned live-ledger EXITS, each mechanically proven and recoverable from claim_versions — (1) proven_inaccuracy (cross-instrument: span absent from an id-confirmed primary AND the claim names a foreign instrument id; evidenced); (2) orphaned_no_prose_referent (functionless: claim text verbatim-absent from EVERY section prose AND, if it covers a required slot, that slot is ALSO covered by a FACT/GAP — never version out a slot\'s SOLE coverage); (3) relabel-to-ANALYSIS (true-but-secondary: the claim STAYS LIVE, its prose gets the recognized label — not an exit, a repair). Nothing else removes a claim from the live ledger. An orphan is proven FUNCTIONLESS (certifies nothing customer-facing), NOT proven wrong — distinct supersede_reason so the taxonomy stays honest (migration 210).',
    enforcedBy: ['RD-45-erase-only-on-proven-inaccuracy'],
    residual:
      'eraseClaimWithProof (ledger-apply.mjs) refuses without a proof and archives-before-deletes fail-closed; migration 208 constraint claim_versions_proof_required enforces the proof at the DB. Golden case 5 proves refuse-without-proof + fail-closed-retention. The proof-not-proxy addendum is enforced in the drain clearance by the two-condition gate (span-absent AND foreign-instrument-id AND id-confirmed primary), goldened by drain-clear-two-condition.golden.mjs (the 4ff5cf56 same-subject/wrong-primary pattern must NOT auto-clear; the 55f90df0 foreign-instrument pattern must). INCIDENT-PROVEN 2026-07-16: a batch drain-clear that inferred cross-instrument from span-absence over-cleared 48 claims on 4ff5cf56/ad4cc6c6; all restored from claim_versions (restore-overclear.mjs) and the gate tightened. NAMED RESIDUAL: the automatic proven-inaccurate DETECTOR (contradiction judge) is not built — grounding conservatively keeps not-reproduced claims.',
  },
  {
    id: 'primary-text-is-permanent',
    statement:
      'The captured primary text is PERMANENT. Every primary document fetched at grounding time is retained permanently, per-capture, immutable — because change detection is DOCUMENT-against-document (the ground truth), not ledger-against-ledger (the extraction, one level removed). raw_fetches is append-only by construction for ANY caller (the CC executor or the metered pipeline): the enforcement lives in the shared capture function writeSnapshot, whose storage key and row identity are CONTENT-ADDRESSED, so a CHANGED re-capture is a new versioned snapshot that can never overwrite the prior, and an IDENTICAL re-capture is idempotent. There is no prune or delete path. This is the document-level twin of grounding-is-non-destructive: non-destructive preserves the CLAIM ledger, this preserves the SOURCE DOCUMENT the ledger grounds against — and a hash-identical re-capture stands as a cheap, customer-valuable proof of "unchanged as of [date]".',
    source: 'operator ADDENDUM — document-baseline doctrine, retention permanent + document-against-document change model (2026-07-16)',
    enforcedBy: ['RD-46-primary-text-permanent'],
    residual:
      'raw_fetches append-only by content-addressing (writeSnapshot key `${source_id}/${day}/${sha256}.html.gz` + UNIQUE(source_id, content_hash), migration 052); no prune path (grep-clean; only FK ON DELETE SET NULL + source cascade). Enforcement in the SHARED writeSnapshot so both executors inherit it. Golden primary-text-permanent.golden.mjs (7/7) proves the content-addressing the UNIQUE index rests on. NAMED RESIDUAL: the golden proves the key-derivation at the pure layer; the DB UNIQUE index + the no-delete-path structure are the runtime enforcement. A durable append-only-over-time retention audit is the future strengthening.',
  },
  {
    id: 'doctrine-binds-to-pipeline-not-executor',
    statement:
      'Every integrity doctrine binds to the PIPELINE (the shared code), not to the EXECUTOR. The build-phase driver (Claude Code via Chrome, subscription, $0) and the steady-state driver (the metered model path) MUST be interchangeable drivers of ONE pipeline: they meet at the single grounding chokepoint and collapse into one claim ledger at the extraction pivot, after which the data carries no driver identity. The two diverge ONLY at allowlisted skip-points where the free driver skips a paid/model step it does not need — never in JUDGMENT — and the judgment core (verbatim kept-filter, mint gates, tier-stamp, non-destructive apply) is DRIVER-BLIND: it cannot branch on which driver produced the ledger, so parity is structural, by construction. A doctrine enforced in the executor rather than the pipeline would be silently dropped when executors swap; the enforcement LOCATION is therefore audited, and every chokepoint must be byte-identical across drivers.',
    source: 'operator ADDENDUM — executor-agnostic enforcement, doctrine binds to the pipeline not the executor (2026-07-16); FORWARD EXTENSION (executor-universality ruling 2026-07-16): every FUTURE content-collection mechanism (new scraper, cron acquirer, API integration, any capture/grounding driver added for the expansion or after) MUST enter through the same chokepoint (groundBrief and below) or bind at the DB layer, and the RD-47 structural golden MUST be extended to cover each new driver as it is added — a driver that reaches the ledger without passing the chokepoint is a golden failure and a CI fail. The migration-209 DELETE trigger is the DB-layer form (covers all writers by construction).',
    enforcedBy: ['RD-47-doctrine-binds-to-pipeline-not-executor'],
    residual:
      'executor-parity.golden.mjs (13/13) proves it structurally over the real canonical-pipeline.ts: the driver-identity var is referenced EXACTLY 4x in code (1 decl + 3 allowlisted divergences by code signature), the drivers unify at the extraction pivot, and the judgment core carries at most the one allowlisted dominance-guard reference — a fourth divergence or any core branch on driver identity is RED. Seam: the injectedLedger param in groundBrief + scripts/_reground/executor-ground.mjs. NAMED RESIDUAL: the golden is a static source-scan proving the pipeline cannot branch on driver identity; the two RUNNERS end-to-end are proven operationally (o9 + w4_ca_sb261 via the CC runner, metered path unchanged), and a full end-to-end fixture-parity harness is the future strengthening.',
  },
  {
    id: 'capture-must-be-the-items-own-instrument',
    statement:
      'A captured primary MUST be the item\'s OWN instrument before it grounds. officialnessOf confirms a capture is AN official instrument; it does NOT confirm it is the CORRECT one for the item (eu_clean_trucking captured the CSRD directive for the HDV CO2 regulation and scored "official" — a fact would have ground verbatim against the wrong law). The target-match gate holds a MISMATCH (the fetched pool bears a different instrument identifier and not this item\'s, with no matching block) rather than grounding it; it never over-holds when the right instrument is also present, and it never hard-holds an item that merely references other instruments. Absent any identifier signal it falls back to subject overlap at a raised threshold. This binds to the SHARED pipeline chokepoint before extraction, so both executors inherit it.',
    source: 'drain-loop finding 2026-07-16 (officialness confirms an official doc, not the correct instrument — add target-match before grounding)',
    enforcedBy: ['RD-48-target-instrument-match'],
    residual:
      'verifyTargetMatch/verifyPoolTargetMatch (target-match.mjs, pure, reusing identifier-variants.mjs) wired at the groundBrief fetched-pool chokepoint before the extraction pivot. Golden target-match.golden.mjs (18/18): RED eu_clean_trucking mismatch + GREEN own-id/raw-id/subject matches + raised-threshold fallback + wiring proof. Proven on real data (target-match-probe.mjs, 30 staged captures: eu_clean_trucking sole mismatch, 6 held unverified, reference-only items not false-held). NAMED RESIDUAL: items lacking their own canonical_instrument_key get only subject-overlap (an A2 data gap, surfaced); UNVERIFIED is soft-flagged not held (conservative first-wiring calibration, tighten after A2 keying backfill).',
  },
  {
    id: 'model-tier-rule',
    statement:
  ...
  // ─────────────────────────────── Product / surface intent ───────────────────────────────
  {
    ...
    enforcedBy: ['RD-20-staged-transit-disposition'],
    residual:
      'RD-20 (staged-transit-audit) enforces no-human-finish-of-intake for the intake transit path — a research-role item is machine-ingested and cannot rest in a human-approval queue, which is precisely why an editorial/curation QUEUE on Research is forbidden (a queue is a human finish of the pipeline). The POSITIONING half ("Research = horizon-scan" page intent) is product-framing carried by the platform-intent SKILL.md (binding; drift requires operator-stated correction, same class as the five-surface model / PI-1). A surface-level fitness function scanning the /research surface for curation affordances (editor-picked / featured-by / operator-approval controls) is BUILDABLE but semantic/low-signal; it is NOT built under this relay — reported to the operator as the enforcement option, not silently claimed here. FEEDSTOCK-GAP RESIDUAL (named per the dormant-systems audit G-6, 2026-07-18; ADR-015): the entry\'s core clause — "feedstock is AUTONOMOUS intake from research-role sources, machine-ingested" — describes a capability that CANNOT currently run. No autonomous research-source intake exists, is scheduled, or is reachable without code + config + env change (the discovery layer is frozen; ADR-015 restores it). This is a KNOWN GAP with a named landing point — the crawl rebuild\'s wave three (research-source monitoring for the horizon feedstock: standards-body drafts, ICAO/IMO committee outputs, working papers) discharges it — same enforcement-to-build class as analysis-follows-page-intent. The entry grows an enforcedBy (the research-feedstock intake wired to the awareness tick) when that wave lands; until then the absence is a registered gap, not a violation.',
  },
  {
  ...
  },
  {
    ...
    residual:
      'disposition-content-gate.golden.mjs (18/18) proves the gate structurally over tombstone-delete.mjs. Proven live: 198 verified-disposition removals (174+16+5 provably-empty shells + 3 confirmed-survivor duplicates), 198 tombstones, archived 419->221, zero live item touched. The 110 content-bearing/unconfirmed route to the per-item review lane (RESTORE / CONFIRM-archive-with-reason / HOLD-with-evidence). SECOND-ORDER ADDENDUM (content-is-not-nature, operator ruling 2026-07-17): a mechanical content floor (brief present, claims real) is NECESSARY but NOT SUFFICIENT to decide a RESTORE — it cannot determine whether the content CONSTITUTES an intelligence item vs merely DESCRIBES a source (a portal / publisher / register / org-overview: EUR-Lex-the-database is the archetype; Kansas Register, NY Senate Legislation Portal the class). That discrimination (item-vs-source nature) is JUDGMENT and stays in the review lane PERMANENTLY — never mechanized. Restore only what a freight customer reads as intelligence (a reg/standard/framework/program/finding with decision value); confirm-archive what describes an access point (its content survives as the source row the reclassification correctly created). And label-is-not-proof binds OPERATOR labels too: the operator-named RESTORE presumption for "DEFRA" was overridden by the content read (the row is "UK DEFRA: Organizational Overview" — source-shaped), CONFIRM-archived on evidence. Discovered at scale: of 80 content-bearing reclassified rows, ~half are source-descriptions correctly archived.',
  },
  {
    id: 'every-decline-names-the-five-contracts',
    statement:
      'A scope decision that DECLINES or PARKS a candidate (source, data feed, or instrument) MUST test it against all FIVE customer-surface contracts and record a per-surface verdict + a one-line reason for each, before the decision stands. The five contracts (verbatim): Regulations = a compliance-action text brief (what is binding, when, what it costs, what to do); Operations = structured jurisdictional cost / feasibility intelligence; Market Intel = comparative and numerical signal (deltas, trajectories, lead-time); Research = a structured horizon assessment (distance, maturity, credibility, assumption-shift); Community = human-operated peer surface, outside machine intake by construction. A decline that reasons against ONE surface and never names the other four is the failure class this kills — a candidate that fails the Regulations contract can still be an Operations cost feed, and dropping it whole silently loses that value. Three instances 2026-07-17: (a) cost/price/labor data feeds declined despite Operations = cost intelligence; (b) Market Intel source-discovery omitted; (c) Research source-discovery omitted. Fourth, self-referential: the dispatch that ordered this gate had itself declined Clean Truck Check whole when the five-surface test yields Operations = IN (per-vehicle compliance-testing costs on California lanes) — the gate catching its own author.',
    source: 'operator dispatch + combined ruling (2026-07-17) — SURFACE-CONTRACT SCOPE GATE',
    enforcedBy: ['PI-5-every-decline-names-the-five-contracts'],
    residual:
