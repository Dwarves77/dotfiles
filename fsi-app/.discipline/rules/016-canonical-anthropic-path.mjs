...
const PERMITTED = [
  ...
  // first-fetch-classify.ts REMOVED (Lane SPEND, system-completion train, 2026-09-02): it no longer makes
  // a direct Anthropic call — it routes through spend-client.ts's spendMessage (already PERMITTED below),
  // the sanctioned Rule-016 + F15 chokepoint. A vestigial PERMITTED entry for a file with no direct call
  // is a hole this rule doesn't need punched anymore (found during that lane's own fitness sweep, fixed
