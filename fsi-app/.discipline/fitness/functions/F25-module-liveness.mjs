...
export const LEGACY_ALLOWLIST = [
  ...
  ...(() => {
    ...
    return [
      ...
      d('scripts/mint/lib/gate-a-match.mjs', 'B1 Ranked Gap #10: compatibility re-export nothing in production uses — every real Gate-A caller imports src/lib/agent/gate-a-match.mjs directly', 46,
        'BUILT-NOT-WIRED (B1 Appendix A): only importer is src/lib/intake/record-facts.npmtest.mjs; not load-bearing despite being named in governing-files.mjs (a hash-input string, not proof of an import).'),
      ...
      o('.discipline/governance/worktree-isolation-hook.mjs',
        ...
        'Zero non-test importers, no workflow/package.json dispatch. Predates B1\'s window.'),
      w('.discipline/lib/adr-loader.mjs', 'operator/coordinator ruling: wire into whatever surface should read docs/decisions/ at runtime, or delete if no such surface is planned', 46,
        'Zero non-test importers, no workflow/package.json dispatch. A generic ADR-frontmatter reader with no found caller — genuinely unclear whether anything is meant to consume it yet, unlike the out-of-repo-boundary tools above.'),
