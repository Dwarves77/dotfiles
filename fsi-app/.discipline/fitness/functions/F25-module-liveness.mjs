  ...
  ...(() => {
    ...
    return [
      ...
      w('scripts/spec09/surcharge-audit-producer.mjs', 'plan §W5.1 (re-granted train 44: trains 38–44 were consumed by the speed emergency the operator declared 2026-09-04 18:05; expiry moved to 46 so it fails at T46 (the operator\'s full-system validation, "every item is used") if still unwired/undeleted)', 46,
        'BUILT-NOT-WIRED (B1 Appendix A): DESIGNED-ONLY by SOURCES.md\'s own admission; only importer is its own .test.mjs. [CONFIRMED against this tree, 2026-09-05: lane SPEC09-A wired grid-queue-producer.mjs and oem-roadmap-producer.mjs into maintenance.yml (spec09-grid-queue/spec09-oem-roadmap steps) and removed their allowlist entries; reroute-producer.mjs was already CI-dispatched (maintenance.yml spec09-reroute). The remaining three spec09 producers here (indexation, surcharge-audit, and — see entries below — auxiliary-energy/dqi/eudr-custody) are still BUILT-NOT-WIRED, out of SPEC09-A\'s write set (its brief named reroute_events/grid_connection_queues/oem_tech_roadmaps only).]'),
