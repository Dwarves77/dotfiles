...
**B9. Never commit in the main checkout, and never instruct the operator to (RD-19).** `docs/doctrine/worktree-isolation.md`, status **enforced**: *"Agent branch/checkout/merge operations occur ONLY in that agent's assigned worktree. The main checkout is the orchestrator's exclusive surface. An agent that finds itself in the main checkout stops and reports, it does not operate there."* It governs branch / checkout / switch / merge / rebase / worktree-add **and commit**. Detection: `git rev-parse --git-dir` differs from `--git-common-dir` in a linked worktree; worktrees live under `.claude/worktrees/`. The `pre-commit` hook is the real block and fires inside sub-agents, so a violation arrives as a hard abort, not a warning.

The sanctioned path for an agent with work to land is a worktree branch and a PR. **Never reach for `--no-verify`.** This extends to advice: a copy-paste block telling the operator to `git commit` in the main checkout is the same violation laundered through a human. That happened on 2026-08-17 and was caught by the hook, not by the agent that wrote the instructions.

**B9 corollary — do not run `git` over the Cowork device bridge at all.** On the mounted filesystem `git` cannot unlink its own `.git/index.lock` ("Operation not permitted"), so **every** git command run through `device_bash` — including a read-only `git status` — leaves a stale lock that wedges the operator's next git command. Read repo state from a cloud clone (`git show origin/master:<path>`), write files over the bridge if you must, and leave every git operation to the operator or to a Claude Code session on their machine.

**B10. Never assert a capability limit you have not tested this session.** Do not tell the operator something is impossible because a tool description implies it, because it failed before, or because of an assumption about the product. **Attempt it and report the actual error text.** A tested "no" costs one tool call; an untested "no" costs the operator a round trip and is usually wrong. On 2026-08-17 this agent told the operator four separate times that an action was outside its reach; deleting and installing an account skill were both done minutes later through the connected Chrome, and writing to his disk through the Cowork bridge had already been done. When the operator says a capability exists, he is describing his own configuration and is more likely to be right than your model of it. **Test his claim first, argue second.**

Known-good paths, verified 2026-08-17: account skills are managed at `claude.ai/customize/skills` in the connected browser (three-dot menu -> Remove; Add -> Upload a skill + `file_upload`), and `file_upload` reads from `/mnt/user-data/outputs/`, not from arbitrary container paths. Known-real limits, each verified by its own failure: the cloud container cannot **push** to this repo (git proxy 403, "not in this session's authorized repository set" - fixable by adding the repo to the session's sources), `device_commit_files` refuses paths under `.claude/` (the `device_bash` shell does not), and `device_bash` cannot delete files.
...
- **Main-checkout commit instruction, 2026-08-17.** I staged three files in the main checkout via the Cowork device bridge and then handed the operator a copy-paste block ending in `git commit` there. RD-19 has been enforced since June; the `pre-commit` hook blocked it, and a Claude Code session landed the identical work through a worktree as PR #466 instead. I had read `docs/decisions/` for the column question and never read `docs/doctrine/` for the git question. **B2's habit was applied to the wrong directory.** Rule B9 exists because of this.
