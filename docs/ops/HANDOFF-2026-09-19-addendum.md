...
| W10-A | `wt-landdocs-0911`, `lane/w10-a-f49-impact-meter-2026-09-18`, commits ending `6f5b942f` | NOT pushed. The queue stopped on it as expected (five bookkeeping conflicts); resolved by hand (`resolve-w10a.mjs`, durable copy beside the ledger): skill categories 46 and 47 both kept, manifest F48/F50/F49, invariants RD-72/RD-74/RD-73 with the marker baseline at 58, skill pin re-measured, F45 measured 6,121 on the combined tree. 223 tests pass, skill-contract check clean, worktree clean. CORRECTION 2026-09-19 (I first stamped this "10:45 EDT" and said the queue started at 10:45; both were estimates, the third time I wrote a clock time from memory; the queue log says it started at 10:38:06): the refusal described next is [REFUTED]; it was my measurement error. I ran the gate from INSIDE `fsi-app/`; the gate passes its pathspecs to `git diff` relative to the current directory, so from there the per-lane file's diff came back empty. Run from the repo root (where the push hook and CI run it) it prints "UX compliance gate OK". W10-A was not refused by anything; it and M2 went into the push queue at 10:38:06 (M2 first; T1 #736 had merged at 10:35). The queue HALTED on M2 at 10:40:37, see M2's row; W10-A was then queued alone at 10:42:30 and was inside its gate run when this line was written. The latent defect is real and belongs to lane N0: the gate's git calls should not depend on the caller's directory. This is the second time in two days I measured from inside `fsi-app/` and reported a wrong result (the first was a false "zero uses" count); run repo-wide measurements from the repo root. What I first wrote, kept for the record: ONE OPEN REFUSAL [first labelled CONFIRMED, cause NOT established]: `node fsi-app/.discipline/governance/memory-gate.mjs --range=origin/master..HEAD` reports the UX compliance gate failing, although the lane's own `docs/ops/session-log.d/2026-09-18-w10a.md` carries `### UX compliance (W10-A)` and `git diff origin/master..HEAD` on that file shows the added line. Diagnose that before pushing (start with how the CLI builds its diff lines for the per-lane file on this tree, D28b #731); do not weaken the gate. Its line is still in `push-queue.txt` as pending: the queue will retry it on its next run |
| M2 | `wt-session-d`, `lane/m2-ledger-consume-apply-2026-09-18`, commits `9dd0a698` + `03925ac6` | rebased, F28 and ledger-consume tests 161 pass, NOT pushed, held OUT of `push-queue.txt`. UPDATE 2026-09-19 10:40:37 (queue log): T1 merged, M2 rebased clean onto it (`3fc27927`), and its push was refused by a SECOND test with the same defect: `fsi-app/scripts/producers/market/ecb-fx-producer.test.mjs` ("--apply with the kill switch ON but no DB creds still refuses"). The producer loads `fsi-app/.env.local` from disk (its line 170), so in this worktree, the only one with an env file, the DB-credentials gate is satisfied and the expected refusal never comes. [CONFIRMED: ten credential-absent test files run in this worktree, only this one fails.] Same cause twice, so by the operator's ruling it gets a CLASS fix, not a second one-off flag: **lane T2** = one shared guarded env-file loader (one home; honours ONE variable that switches the load off), every script's bare `process.loadEnvFile` moved onto it, F48 extended to refuse a bare load, every test that asserts credential-absent behaviour sets the variable; acceptance is the FULL suite green in `wt-session-d` with its env file present. My T1 class check missed this test because I searched for the self-skip phrasing only. M2 is pushed after T2 merges: add the line `wt-session-d|pr-m2.md|m2c` to `push-queue.txt` and run the queue |
