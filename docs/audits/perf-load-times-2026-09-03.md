...
## 17. DETAIL-WATERFALL lane (2026-09-06) — the regulation-detail client fetch waterfall and the
...
instrument section above at the time this lane started, so this is filed as §17 — cross-referenced
here under the same operator finding this section addresses.)

### 17.1 The operator's finding, quoted verbatim (production, 2026-09-06)
...
### 17.2 Re-measurement, foreground, this lane, 2026-09-06 (same Resource Timing method as §7-§9)
...
reads that duplicate data reachable server-side** (§17.3) — that structural defect is real and worth
...
### 17.3 Fetch map — every client-side call this lane found, mounted by the regulation detail page
...
and is the real structural defect regardless of the fetch-count discrepancy in §17.2**: three of
...
from the same page mount — they are prime candidates for one batched round trip per §17.4, item 2, and
none of them is needed for first paint (§17.4, item 3) even though today none of them blocks it either
...
fallback (PERF-10 header, §17 above) did not fire in either capture, which is consistent with its own
...
### 17.4 Build — status against the dispatch's 8-point BUILD list
...
1. **Server loader vs. client re-fetch, non-overlap** — [CONFIRMED] by code read: `loadDetailCore`'s
   ...
   independent and is never re-fetched client-side; nothing in §17.3's fetch list duplicates data the
...
6. **Summary/Full/Complete toggle rebuild** — **DONE this lane**, see §17.5.
...
8. **Re-measure after the fix, foreground, same method** — see §17.6 for the one component this lane
...
### 17.5 The toggle fix — root cause, fix, and the six §0 evidences
...
- **Run** — [CONFIRMED], guarded write with read-back is not applicable (no data write; this is a pure
  ...
  `npx tsc --noEmit` clean on the edited file (§17.7); the coordinator's live re-verification after
  ...
  artifact: a short screen recording or three screenshots (one per state), same as §17.7's evidence
  here.
...
- **Visible** — [CONFIRMED]: rendering guard run in §17.7 (no `.tsx` change here triggers a NEW guard
  requirement beyond what already runs — `run-rendering-guard.mjs` executed, see §17.7 for its output).
...
- **Documented** — this §17.5 entry, plus the in-code comment block replacing the old two-state
  comment (see the file's edited header comment above `SummaryTabBrief`'s `mode` state).

### 17.6 Re-measure after the fix

**Not performed** — the toggle fix is not deployed (this lane cannot push; the coordinator lands), so
there is nothing live to re-measure yet. The pre-fix screenshot in §17.5 is this lane's "before"; the
coordinator's post-deploy click-through (named in §17.5's "Run" evidence) is the "after." The
fetch-waterfall re-measure (dispatch point 8) has no fix to re-measure against at all, since §17.4
point 2 (the batched route, the actual structural change) was not built this lane — re-measuring the
UNCHANGED fetch behavior would just reproduce §17.2's numbers.

### 17.7 Gates run this lane

- `node fsi-app/.discipline/fitness/runner.mjs`: see §17.8 (full lane gate log).
...
- `node fsi-app/.discipline/rendering/run-rendering-guard.mjs`: run because a `.tsx` file changed — see
  §17.8 for its output line.
- Full gate suite (fitness, governance/fitness tests, closure gate, run-test-suite.sh,
  override-check.mjs, rendering guard) run together and logged in §17.8 rather than duplicated here.

### 17.8 UX compliance block (docs/design/ux-laws.md)
...
- **44px targets**: unchanged `Segment` component already enforces `minHeight: 44` with
  ...
  lane (rendering guard in §17.7/§17.8's gate log is the mechanical check for this).
