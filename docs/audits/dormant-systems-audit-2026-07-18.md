...
| Operator-fired script runners (`funded-pass.mjs` with RD-38 run-lock, `regen-quarantined.mjs`, verify/* lanes) | Manual, unscheduled by design | Correctly named; spend-gated by the section 2 stack | keep-and-integrate — **`funded-pass.mjs`'s keep-and-integrate SUPERSEDED (lane ONESHOTS, 2026-09-06, operator ruling, rule 14 corollary — corrected in place, not silently dropped): the file drove PAID Sonnet/Haiku generation passes, which contradicts CLAUDE.md's later standing $0/no-LLM-in-runtime rule, and its own top-level `readFileSync` of `scripts/tmp/funded-pass-worklist.json` (a hand-authored, per-campaign file that does not exist) means the "manual, unscheduled by design" judgment above was never actually runnable without first authoring that worklist by hand — no harness-run artifact anywhere proves it ever completed a dispatch. DELETED, with `scripts/lib/funded-pass-core.mjs` (orphaned by the same deletion) and both files' tests; see `.discipline/fitness/functions/F25-module-liveness.mjs`'s own removal note for the full disposition. `regen-quarantined.mjs` and the `verify/*` lanes are UNCHANGED by this — `regen-quarantined.mjs` is now WIRED (maintenance.yml step `regen-quarantined`), not deleted.** |
