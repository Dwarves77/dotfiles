...
- ~~`GeopoliticalSignals` function~~ → CLOSED 2026-05-22 (post-Build-7 evaluation dispatch, commit `79e508f` on branch `feat/geopolitical-signals-evaluation`). Outcome: STRIP ONLY. The geopolitical-disruption / chokepoint / trade-restriction signaling FUNCTION is already covered by Build 7's Price Signals and Trade tab in `MarketPage.tsx` (heading "Geopolitical and Market Signals", workspace-anchored `market_signal` items via `get_market_intel_items` RPC, Q9 chip mounts via `CitationCountChip` + `RecencyChip`, OBS-18 + OBS-20 closures). Phase-1 `GeopoliticalSignals.tsx` (hardcoded, no workspace coupling, no Q9 chips, non-canonical color tokens, orphan / not imported) stripped. Reuse would have duplicated the existing tab; rebuild would have re-coupled a hardcoded category list parallel to the dynamic `groupByCategory` output. Depth-of-content per category (Hormuz, Suez, Panama, EU CBAM, EU EV tariffs, critical minerals) is content / ingest work feeding the existing Price Signals surface, not a missing UI surface.
