...
2. Before that run, walk this list against your diff; each line is a gate that has refused a lane:
   ...
   - A range that changes a pinned `SKILL.md` under `fsi-app/.claude/skills/`, or adds/removes/moves a `GOVERNING SKILL(S):` citation of a registered skill (`fsi-app/.discipline/governance/skill-contract-map.mjs`'s `PINNED_MANIFEST`), adds its own `fsi-app/.discipline/governance/skill-acks/<date>-<lane>.md` naming the skill (`## Skill` / `## Citing files reviewed`) in the same range. Nothing to re-pin; the manifest stores a `skillPath` only, never a content hash or a citing-file list.
