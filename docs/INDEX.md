...
- [ADR-016-source-contact-ledger](./decisions/ADR-016-source-contact-ledger.md) — Append-only source-contact ledger, one home for every source touch (discovered/evaluated/signed_up/newsletter_inbound/declined/parked/registered), closing the found-then-lost class where source knowledge leaked into session-log paragraphs and the inbox; newsletter-inbound explicitly modeled (the IEA statsnews case). Design accepted 2026-07-19, build queued behind census work, operator-sequenced (accepted)
