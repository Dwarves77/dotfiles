...
|---|---|---|---|---|---|
| 2.1 FactCard (UPDATED 2026-09-20, lane w10-factcard, Amendment 1 pass) | `src/components/ui/FactCard.tsx` v2, one `{ model: FactCardModel }` prop, no variant prop, root element carries `data-part="fact-card"` and `data-kind="<kind slug>"` (Amendment 1 section C). Model built by `src/lib/detail/fact-card-model.ts` (extends `src/lib/detail/fact-paragraphs.ts`'s parser), reached via `src/components/detail/primitives.tsx` (`RecordFactCard` -> `deriveRecordFactCardModel`, regulations record-grade) and `src/components/detail/FactBlocks.tsx` (`deriveFactCardModels`, market/research/operations, and regulations non-record sections) | Kind band, 132px/1fr/150px body grid, Anton-22 figure lead, provenance as a right COLUMN with a 1px left rule, 3px left edge, three FORMS (orange/ink/inference) over the fixed 9-word kind vocabulary, mobile stacking below 768px: all built to spec. RESOLVED this pass: (1) `RecordFactCard`'s record-grade rows now map through a deterministic `RECORD_SLOT_KIND` table (Amendment 1 ruling B.3) - `effective_date`/`primary_deadline`/`due_date` -> DEADLINE, `penalty_summary` -> PENALTY, `jurisdictional_scope` -> SCOPE (explicit); every other slot key (`title`, `operative_provision`, `addressee`, `binding_position`, `corridor_identity`, `in_force_status`, `evidence_agreement_signal`, `source_authority_signal`) falls to SCOPE, unit-tested. STILL UNRESOLVED: (2) `figureSubLabel` is always null - no structured field reaches the pure text-parsing layer. (3) The "count every no-lead card by kind" corpus count (brief item 1) was not run against live data - not this lane's item per Amendment 1 ruling B.4 ("not yours; the coordinator obtains it"). (4) `src/components/operations/RegionDimensionMatrix.tsx`'s `MatrixFactCard`: Amendment 1 ruling B.1 orders it MIGRATED to a `density="matrix"` FactCard variant and the old function deleted, superseding the 2026-09-09 "different anatomy" ruling its header cited. This pass corrected the header comment to record the newer ruling but did NOT execute the migration or add the density variant - the panel carries operator-ruled behaviour (a comparison-index delta, a six-word-headline no-figure branch, five `data-audit` hooks a live spec file reads, a "card height constant" acceptance test) that a text-only pass cannot safely verify without running the app and its spec suite. Reported as an incomplete ruling execution, not a silent drop - see the session-log entry | `regulations/[slug]` (`RecordFactCard`, record-grade, and `FactBlocks` for non-record sections), `market/[slug]`, `research/[slug]`, `operations/[slug]` (all via `FactBlocks`) | `operations` matrix panel (`RegionDimensionMatrix.tsx`'s `MatrixFactCard`, migration ordered but not yet executed, see prior column) | Dashboard "What changed" struck from the consumer list per ruling 4 (`docs/design/handoff-2026-09-07/README.md` "Undrawn cases" item 4): it renders `ListRow`, not FactCard, and stays on `ListRow` |
