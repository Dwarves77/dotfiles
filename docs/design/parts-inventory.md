...
|---|---|---|---|---|---|
| 2.1 FactCard (UPDATED 2026-09-20, lane w10-factcard) | `src/components/ui/FactCard.tsx` v2, one `{ model: FactCardModel }` prop, no variant prop. Model built by `src/lib/detail/fact-card-model.ts` (new pure module, extends `src/lib/detail/fact-paragraphs.ts`'s parser rather than re-parsing content_md), reached via `src/components/detail/primitives.tsx` (`RecordFactCard` -> `deriveRecordFactCardModel`, regulations record-grade) and `src/components/detail/FactBlocks.tsx` (`deriveFactCardModels`, market/research/operations, and regulations non-record sections) | Kind band, 132px/1fr/150px body grid, Anton-22 figure lead, provenance as a right COLUMN with a 1px left rule, 3px left edge, three FORMS (orange/ink/inference) over the fixed 9-word kind vocabulary, mobile stacking below 768px: all built to spec. UNRESOLVED at this update: (1) `RecordFactCard`'s record-grade rows (free-form field label, e.g. "Effective date") have no brief-specified mapping onto the 9-word kind vocabulary - `deriveRecordFactCardModel` defaults every record-grade card to kind SCOPE with the field label as qualifier, flagged for coordinator ruling, not the brief's own rule. (2) `figureSubLabel` is always null - no structured field reaches the pure text-parsing layer this module operates at; the artboard's sub-labels ("RECOVERED", "RECOVERY / RECYCLING") are editorial and not derivable from raw content_md alone. (3) The "count every no-lead card by kind" corpus count (brief item 1) could not be run - no DB credentials in this worktree; `countNoLeadCards()` is built and unit-tested but not run against live data. (4) `src/components/operations/RegionDimensionMatrix.tsx`'s `MatrixFactCard` was NOT migrated: its own header comment documents a 2026-09-09 operator ruling that it is deliberately a DIFFERENT anatomy from `ui/FactCard` (figure inline with its label, no kind band, no quotation marks, no tier chip) - a real brief/code disagreement, reported not resolved, per the lane contract's STOP rule | `regulations/[slug]` (`RecordFactCard`, record-grade, and `FactBlocks` for non-record sections), `market/[slug]`, `research/[slug]`, `operations/[slug]` (all via `FactBlocks`) | `operations` matrix panel (`RegionDimensionMatrix.tsx`'s `MatrixFactCard`, deliberately un-migrated, see prior column) | Dashboard "What changed" struck from the consumer list per ruling 4 (`docs/design/handoff-2026-09-07/README.md` "Undrawn cases" item 4): it renders `ListRow`, not FactCard, and stays on `ListRow` |
