...
- [discipline](../inventories/discipline.md) — the discipline-engine manifest / mechanism roster this ADR defines is tracked in the discipline inventory
- [ADR-003-server-centric-dual-write](./ADR-003-server-centric-dual-write.md) — F8 client-server tier boundary is a Layer-2 fitness function catalogued in ADR-005's discipline architecture
- [ADR-004-auth-pattern-split](./ADR-004-auth-pattern-split.md) — F2 admin-routes-isPlatformAdmin is a surviving Layer-2 fitness function in ADR-005's discipline engine
- [ADR-006-plan-skill-hybrid](./ADR-006-plan-skill-hybrid.md) — this ADR's slim refactor deleted rules 009/010 that ADR-006 documented, deprecating it
- [ADR-009-adr-system-architecture](./ADR-009-adr-system-architecture.md) — explicit related; ADR-009 is Layer 3 (decision protection) of this layered architecture
