...
- [build-8-research-surface](../plans/build-8-research-surface.md) — Dispatch 8.3 surfaces the per-dimension bias tags whose auto-cutoff threshold ADR-007 defines, and hides empty-state rows per ADR-007's threshold…
- [ADR-002-tier-model](./ADR-002-tier-model.md) — same sources table + source-credibility model; base_tier feeds the downstream credibility scoring that ADR-007's bias-tag aggregation respects
- [ADR-008-urgency-score-default](./ADR-008-urgency-score-default.md) — both derive defaults from Haiku classifier output and were retired-to-convention F-functions in the same slim refactor
- [migrations](../inventories/migrations.md) — shared migrations 092 (source bias tags) and 097 (Q4 bias retune Option B) tracked in migrations inventory
- [source-classification-framework-2026-05-10](../plans/source-classification-framework-2026-05-10.md) — same source-classification/Haiku-classifier subsystem this bias-tag threshold tunes
