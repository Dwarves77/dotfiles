...
| Outbox + derivation DAG + governed drain | **SHIPPED, schema + runtime** (migrations 284/285, Lane DP-ENGINE, 2026-09-02); `src/lib/propagation/drain.ts` + `scripts/turns/run-propagation-drain.mjs` implement the two-pass (invalidate, then apply-mode-only recompute) drain over the outbox. **UPDATE, Lane DP-SURF, 2026-09-02: the `methods/index.ts` seam now carries two registered methods** — `carbon_intensity_tkm@1.0.0` (`src/lib/propagation/methods/carbon-intensity.ts`, wraps the pure `src/lib/market/carbon-intensity.mjs`) and `automate_vs_hire@1.0.0` (`src/lib/propagation/methods/automate-vs-hire.ts`, wraps `src/lib/operations/automate-vs-hire.mjs`) — registered via explicit `registerMethod()` calls at the bottom of `index.ts` (a self-registering side-effect-import shape was tried first and broke on a real module-linking TDZ error, `ReferenceError: Cannot access 'REGISTRY' before initialization` — see that file's own header). An apply-mode drain now recomputes both once a value exists to supersede; `scripts/propagation/seed-derived-values.mjs` (`--dry`/`--apply`) is the initial-closure seed that creates the first row each method later supersedes |
| Lifecycle × admissibility state machine; computed decay; the gate | **SHIPPED** (Lane DP-ENGINE, 2026-09-02): `src/lib/propagation/admissible-for.ts` implements `admissibleFor()` against ADR-024's FLOOR values; `src/lib/propagation/effective-confidence.mjs` computes decay per §3.2 (never stored); F31 fitness function pins the import boundary (only `src/lib/propagation/**` may query `derived_values`/`statutory_computations`/`estimated_values` directly). **UPDATE, Lane DP-SURF, 2026-09-02: now wired into real call sites** — `StatutoryFigure`/`EstimatedFigure`/`DerivedFigure` (`src/components/figures/`) each call `admissibleFor()` before rendering a value ("the one gate," spec §3.3); the `admissible-for.ts` LEGACY_ALLOWLIST entry (F25) is removed accordingly, its wiring having landed |
| Statutory/estimate physical + type + DB + component isolation | **SHIPPED, layers 1–3** (migration 286, Lane DP-ENGINE, 2026-09-02): separate physical tables (layer 1), the `assert_statutory_purity()` trigger enforcing the derivation-graph constraint (layer 3), F32 fitness function as a static mirror of the same rule. **UPDATE, Lane DP-SURF, 2026-09-02: layer 2 and layer 4 now shipped.** Layer 2 — `src/lib/statutory/types.ts`'s `Contractable`/`NonContractable`/`StatutoryInput`/`computeStatutory<F>()`, a real compile-time barrier proven by `types.contractable-barrier.check.ts`'s `// @ts-expect-error` case (a `derivation: "modelled"` input fails `tsc --noEmit`, not just a runtime check) and exercised end-to-end by `src/lib/statutory/fueleu-annex-iv.mjs` (FuelEU Maritime, Regulation (EU) 2023/1805, Annex IV — see this lane's own final report for citation-verification status: **[UNCONFIRMED] against a primary EUR-Lex text this session**, no network egress; corroborated against 4 independent secondary/technical sources instead). Layer 4 — `StatutoryFigure`/`EstimatedFigure`/`RecalculationNotice` (`src/components/figures/`), each theme-aware, each calling `admissibleFor()`; `StatutoryFigure` itself has no consuming page in this lane's own scope (a FuelEU filing surface is a later lane's build) and carries its own F25 LEGACY_ALLOWLIST entry accordingly |
| `GET /api/notices` — recalculation notices, org-scoped | **SHIPPED, Lane DP-SURF, 2026-09-02**: `src/app/api/notices/route.ts` reads superseded (old→new) `derived_values` pairs for entities on the caller's org's `org_watchlist`, since `?since=` (default 30 days); the raw `derived_values` read (a stale/superseded row is excluded by `derived_values_admissible` by design, so a notice needs the raw table) lives in `src/lib/propagation/methods/superseded-notices.ts`, inside F31's sanctioned directory tree — the route itself never queries `derived_values` directly. Rendered by `RecalculationNotice.tsx`; consumed today by the Operations calculator's notices rail |
| Operations calculator + Market carbon-intensity block | **SHIPPED, Lane DP-SURF, 2026-09-02**: `src/app/operations/AutomateVsHireCalculator.tsx` — a pure client-side automate-vs-hire preview (no API route; `automateVsHire()` is a zero-dependency function, recomputed on every keystroke) rendered through `EstimatedFigure` with NPV/payback/break-even-wage at equal billing (ADR-024), plus a `RecalculationNotice` rail. `src/components/pages/MarketSignalDetailSurface.tsx`'s corridor-band Drivers tab gained a second, separate "Per-unit carbon intensity" block (`DerivedFigure`) alongside the pre-existing "Carbon cost overlay" block (another lane's work, untouched) — same resolved factor row (`selectModalFactor`), two independent pure conversions |
...
additions — see that lane's own report for the one anticipated F28 meta-harness re-pin); **2800/2801 with
Lane DP-SURF's additions on top** (`.discipline/run-test-suite.sh`, same single pre-existing, anticipated
F28 meta-harness failure — unrelated to this lane's write set, confirmed present on the base commit before
this lane's own changes); 25/26 fitness functions (same single F28 violation), F23
governed-surface-coverage 0 gaps, invariant-coverage meta-gate PASS, `tsc --noEmit` clean.
