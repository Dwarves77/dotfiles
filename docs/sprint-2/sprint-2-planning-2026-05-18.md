...
| ~~D6~~ | ~~Category routing wiring path: option a vs option b~~ | **RESOLVED 2026-05-18.** Build 4 shipped option a (Path 2; src-side TypeScript exception lists) at commit 51d9eeb (merged bd42cd4). Post-Build-4 dispatch resolved D6 by porting the mapping to canonical `sources.category` column (option b) via migration 084 at commit 6d98963 (live DB applied; parity verified 47/110/30 items per surface unchanged; src/lib/supabase-server.ts simplified to drop exception lists). Build 7 and future source additions are now DATA entries on sources records rather than CODE changes. |
...
| ~~D15~~ | ~~Migration 070 file: reconstruct OR accept loss~~ | **RESOLVED 2026-05-18.** Reconstructed verbatim from git history at commit c85982d (merged on `fix/mig-070-reconstruction`). Original 070 file was authored on a feature branch (commit 651ae78, 2026-05-11) but did not survive branch squash to master; blob d51bccf preserved 308-line authoritative source. Discovery-doc inference of "5 RPCs in 070" was incorrect: 070 created 3 RPCs (`get_market_intel_items`, `get_research_items`, `get_operations_items`); the other 2 (`get_workspace_intelligence_dashboard`, `get_workspace_intelligence_listings`) originated in 064 and 066. Reconstruction header documents the lineage. Replay parity preserved: 070 → 071 → 073 produces identical end state to production. OBS-40 closed by this resolution. |
| ~~D16~~ | ~~Migration 063 column shadowing remediation path~~ | **RESOLVED 2026-05-19.** Chose Option B (accept divergence + document) via migration 085 applied to live DB. Investigation showed: sources.tier INT NOT NULL CHECK 1-7 works as-is (all 7 tier values populated across 794 sources; maps cleanly to env-policy 6-level Source Type Hierarchy with tier 7 as overflow); sources.jurisdictions TEXT[] NOT NULL DEFAULT empty array works as-is (792/794 sources populated; empty array equivalent to NULL for "unknown" use case); the 12 OTHER 063 columns (source_role, scope_*, classification_*) all applied cleanly. ALTER to TEXT is lossy and breaks consumers. Migration 085 adds COMMENT ON COLUMN to four affected columns explaining the decision and noting that future skill-amendment-driven schema change would require explicit operator authorization. Build 7 does not depend on D16; proceeds against existing schema. |
