...
| **NEXT** | First live dry run of the three armed producers, then apply, then confirm `market_series` and the `regional_data_facts` envelope are non-empty. These parsers have never met a live endpoint — a fixture proves the parse, not that the endpoint still returns that shape | ADR-023 §4 |
