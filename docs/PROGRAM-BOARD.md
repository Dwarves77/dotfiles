...
| **IN PROGRESS — W7.1-CLOSE (2026-09-05)** | The discipline engine's own closure gate was red (NEVER-RUN 10, STALE-NEXT 7 since train 47) with F25/F38/F14 showing 0 violations only because every expired entry had been re-granted to wave52 by ASSEMBLE-47 (the third re-grant in a row, 41 → 43 → 46 → 52 — a disclosed non-fix, not a hidden one). Split into four lanes on disjoint write sets: **W71-A**, **W71-C**, **W71-D** (closure-gate docs half — the 7 STALE-NEXT board rows + `workflow:inspect-oil-bulletin.yml` — plus two duplicate-module questions, both REFUTED on inspection), **W71-F14**. Closes when: (1) zero F25/F38/F14/closure-gate allowlist entries carry an expiry (every remaining entry is either wired, deleted, or handed to a named future lane with no expiry field at all — an expiry that keeps getting re-granted is the exact defect this train exists to end); (2) NEVER-RUN reaches 0 once the coordinator's `docs/ops/dispatch-ledger.jsonl` rows land for the 9 `maintenance:*` steps still pending a real dispatch | this train; Addendum 85 ps 59; next: coordinator dispatches the 9 pending maintenance steps and records the ledger rows; sibling lanes' F25/F38/F14 dispositions folded by the assembly lane |
