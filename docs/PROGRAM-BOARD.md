...
| **T9** | **Machine-flow close** ("N/8 stages flowing; both orphans through full cycle") | **NOT STARTED  -  no artifact** | none. Only referent: "the two T9 orphans" = grandfathered source-less rows (`source-link-invariant.mjs GRANDFATHERED_SOURCELESS`), **re-sourcing assigned to Unit 3** | **see section 3  -  cannot be closed; premise unmet** |
| **T10** | **Autonomous Disposition Engine (Units 0-5)** | Units 0/0b/0c CLOSED; 1-5 mixed (see section 2) | closeout `:127`; PRs #274-#280 | per-Unit in section 2 |
...
## 1a. Four-questions rebuild (2026-09-25)

Opened by the 2026-09-25 coordinator close, per operator rulings R1-R13. Plan:
[docs/plans/build-plan-2026-09-25.md](./plans/build-plan-2026-09-25.md). Rulings, state, design changes
owed and known bugs: [docs/ops/session-log.d/2026-09-25-coordinator-close.md](./ops/session-log.d/2026-09-25-coordinator-close.md).
...
| ETS-proxy carbon calc (no freight rate) | OPEN | decisions 1/2; spec 02 + spec 07 Amendment 2026-09-25 (Market Intel) |
| Learning loop S→M→L | OPEN | R10; docs/plans/learning-loop-design-2026-09-25.md |
| Research assessment model  -  DESIGN phase | OPEN | decision 4; build after four-question structure |
| ADR-034 naming-only phase | OPEN | R5; multi-industry core, freight forwarding first pack |
| #800 look pass against new boards | OPEN | close addendum "Look-only fixes owed" |
| Market detail raw-dump bug | OPEN | close addendum "Known bugs"; needs its own lane |
| CLOSED today |  -  | #796 identity retry + admin entry; #797 sign-in title + RD-82 guard; #798 id-redirect resolver + audit; #799 ADR-034 |
...
- **T9**  -  distinct thread vs folded-into-Unit-3 is undecided (see section 3).
...
- **Flag-system investigation + rulings (2026-07-13)  -  read-only census → per-mechanism rulings.** The 902/903
  ...
  - **What actually landed → see section 7.** The plan above was executed as **Unit A (#304)** + **Unit B (#306)** +
    the diagnosis paired-fix (#307); section 7 carries the live queue, the Unit-2 lineage, and the REJOIN.
...
real lineage; the section 2 "Unit 2 (flag resolver)" row is the autonomous-engine unit, a different thread  -  do not
conflate the two.
...
**registry-expansion execution**, the **T10 units** (section 2), the **coverage floor**, and the **launch clauses (10)**.
...
| **guardedDelete matchColumn parametrization** | **DONE** | Root-cause fix (2026-07-18, `scripts/lib/db.mjs`): `guardedDelete` hardcoded the delete/snapshot key to `"id"`, so a table keyed on something else (`drain_worklist`, PK `intelligence_item_id`) had no way to reach the guarded path at all: a terminal, fully-resolved worklist row could only park, never close. Fixed by parametrizing `matchColumn` (default `"id"`, every existing caller unaffected, verified via `db.test.mjs`). No bypass path added; tombstone-then-delete and every content gate upstream are unchanged. Proved live: closed India's stale `drain_worklist` row (item archived, `integrity_flags` 963d4450 resolved) same bank. Landed 2026-09-11 (task 5.4) as a residual of PR #341; everything else in that PR was already on master. |
| **B-reassignment queue count correction (21 -> 58)** | **NOTED** | The original restart opener estimated "21 B-reassignments." The live `drain_worklist` count (`assigned_by='session-B'`) was 58, the full accumulated handoff across every Session B bank (o13, the 54-item SW-1 handoff, banks 6-8's further 23, plus items Session B restored under its own holder tag), not a single 21-item batch. Corrected 2026-07-18 (drain bank 7); the queue was worked at full size, per-item, not re-estimated down. Historical note carried forward from PR #341; queue state has since moved, see live `drain_worklist`, not this line. |
...
2. Rule purge on the deferred `source_trust_events` never-emitted event-type narrowing  -  evidence in crawl-spec section 8.1 points to purge (held on merits, not access; lands as a content-gated migration at the ruling).
...
| U5 | L3 anticipatory targeting | BLOCKED | build plan section U5  -  needs B1 contract advance |
...
| **RATIFIED** | Connection redesign + remaining-build scope (operator 2026-08-29 "do so then proceed"): three connection classes, WO-28 lineage typing next, WO-29 deferred w/ revisit trigger, section 4 full-build sequence, section 6a multi-agent Sonnet lane model (disjoint write sets, coordinator-only memory/DB writes, serialized landings) | `docs/plans/connection-redesign-and-build-scope-2026-08-29.md` |
| **DEFECT → WO-28** | `mint-item.ts` dedup-linked edge writes `relationship:'references'`  -  forbidden by the live CHECK, error swallowed; zero such rows ever landed. Fix + guard test ride WO-28 | scope section 3 WO-28; live CHECK read 2026-08-29 |
| **FLAG** | U9 listed not-started but its components are on master and wired (ItemConnectionsCard + view-model + resource-lookup, four surfaces)  -  close-out audit owed, not a build. Stage 4-6 WO texts (10/11/13/14/15/21/22/24/25) exist only in the uncommitted v1 plan  -  spec-from-repo pass owed per WO before Sonnet execution | scope section 4 |
| ⛔ PENDING OPERATOR (unchanged) | WO-5 rulings B1-B4 · WO-19 backfill mapping ratification (WO itself ruled proceed) · DDL window for the WO-12(+19) migration family | scope section 6 |
...
| **UNBLOCKED** | Stage 7 producers WO-16/17/18  -  envelope + origin_class both live, so every producer row lands enveloped and classed from day one. WO-20 (greenfield, blocks nothing) is the spine's last piece | scope section 4 |

## Wave 4: Stage 7 producers built, market spine applied, WO-20 specced (2026-08-30)

Executed as four Sonnet lanes with provably disjoint write sets (scope section 6a wave 3 + the WO-20 doc lane),
...
| **CLOSED (superseded, lane W71-D 2026-09-05)** | The WO-17 reader shipped Wave 5 (row below, "WO-9 layer 2 built"); the spec-from-repo pass landed the same wave (three specs table); ADR-022 was ratified Wave 5; the Node 20 bump on `caros-ledge-backups` closed train `db-backup.yml` v6→v7 (Addendum 68, row 1737). U7 stays metered/operator-priced by design (Backlog bucket 4, row 1639)  -  not a dropped thread | Wave 5 rows below; Addendum 68 |
...
| **FINDING  -  orphaned reads on three surfaces** | `regional_data_facts.status` fetched, typed and threaded through `region-grid.mjs`, rendered by nothing (~all 75 rows). `get_research_source_coverage()` fetched by `ResearchLedger` then discarded (`void sourceCoverage`), 15 live rows. Both are work already paid for and not shown | ops spec section WO-10, research spec section WO-15 |
| **FINDING  -  By-state roster is 4 states wide over 13 states of data** | `state_cost_facts` holds 13 enveloped rows across 13 states; `OperationsLedger.tsx`'s `US_STATE_MATCH` recognizes only CA/NY/NC/TX, and NC has zero rows  -  so at most 2 of 13 sourced facts can render | ops spec section WO-10 |
| **CORRECTION (master plan)** | WO-23 is NOT schema-free. `org_watchlist_item_type_check` AND `user_watchlist_item_type_check` both carry a live 5-value CHECK; adding `market_series` needs a coordinator-applied migration plus 4 shared code files  -  the plan said 5 readers and implied no DDL | market spec section WO-23 |
| **CORRECTION (hypothesis refuted mid-lane)** | `checkMatrixEligibility` was expected to be a drift-prone second implementation of `region-grid.mjs`'s coverage logic. It is not: a DB trigger (`rdf_sync_coverage`) keeps `region_dimension_coverage` in sync on every write, zero live disagreement. WO-21 redirected to a different, confirmed-live bug | ops spec section WO-21 |
| **⛔ NEW GATE  -  WO-24 has no join path** | Zero columns anywhere on `intelligence_items` matching `%corridor%`. There is no route from a Market item to `emission_factors.corridor_id`. WO-24's carbon overlay depends on infrastructure that does not exist, on top of the already-recorded DESNZ UNCONFIRMED gate | market spec section WO-24 |
| **⛔ OPERATOR  -  WO-14 is a reconstruction, not a recovery** | WO-14 has zero text anywhere in the vault outside one sequencing-table row. The spec's WO-14 section is clearly labelled as reconstructed and needs ratification before an executor runs it. The larger comparative-ribbon / corridor-rate-board vision was deliberately NOT built into the spec  -  it exists in no code and is uncosted | market spec section WO-14 |
...
| **CLOSED (superseded, lane W71-D 2026-09-05)** | The ready four (WO-10/11/15/25), WO-21 and WO-13 all shipped Wave 6 (rows below); WO-22 shipped Wave 7; WO-23 closed end-to-end (migration 270 + WatchButton UI half, Addendum 58, row 1671); WO-14 closed absorbed (Addendum 50, row 1642); WO-24 re-scoped and shipped (Addendum 52, row 1658); Node 20 bump closed (Addendum 68). U7 stays metered/operator-priced by design | Waves 6-8 rows below; Addendum 58, 68 |
...
| **OWED  -  serialization point** | The duplicated theme/severity taxonomy (`THEMES` / `THEME_KEYWORDS` / `deriveSeverity`) still exists in BOTH `ResearchLedger.tsx` and `ResearchFindingDetailSurface.tsx`. Both lanes were told not to extract it and both correctly did not. It needs one lane owning both consumers | research spec section 2.4/section 2.7 |
| **CLOSED (superseded, lane W71-D 2026-09-05)** | WO-21 shipped Wave 7 (row below); WO-13 shipped Wave 7; WO-22 shipped Wave 7; WO-23 closed end-to-end (Addendum 58); WO-14 closed absorbed (Addendum 50); WO-24 re-scoped and shipped (Addendum 52). The taxonomy extraction (duplicated `THEMES`/`THEME_KEYWORDS`/`deriveSeverity`) was done the very next wave (row below, "DONE (taxonomy)"). U7 stays metered/operator-priced by design; Node 20 bump closed (Addendum 68) | Wave 7 rows below; Addendum 50, 52, 58, 68 |
...
| **CLOSED (superseded, lane W71-D 2026-09-05)** | WO-23 closed end-to-end (migration 270 + WatchButton, Addendum 58); WO-14 closed absorbed (Addendum 50); WO-24 re-scoped and shipped (Addendum 52); the severity-enum→UI-bucket ruling landed Wave 18 (rows below, Addendum 63, `SEVERITY_TO_OPERATIONS_BUCKET`); `fetchWorkspaceResources`/jurisdictionIso closed end to end (migration 272, Addendum 67). U7 stays metered/operator-priced by design; Node 20 bump closed (Addendum 68) | Wave 18 rows below; Addendum 50, 52, 58, 63, 67, 68 |
...
| **SCOPE LIMIT, STATED** | F27 holds the SEAM; it cannot tell you a fixture matches REALITY. Both Wave 13 defects were fixtures faithfully encoding a wrong belief and would have been green under it. Reality is held by ADR-023 section 4 (dry run → human reads plan → apply), which caught both. Neither substitutes for the other | Addendum 48 |
| **DEFERRED (rule 16, lane W71-D 2026-09-05)** | SERIES_ITEM_MAP ratification and the schedule re-arm are both explicitly build-mode-gated (CLAUDE.md standing rule 16: "Build mode holds the scrape cadence OFF, and no session proposes flipping it"  -  `system_state.scrape_cadence='off'`, ADR-023). Neither is a dropped thread; both are correctly held open until the operator sets a scrape time after build, per rule 16's own text. Genuinely blocked on that operator call, not on any lane | ADR-023; CLAUDE.md rule 16; Addendum 48 |
...
| **WAVE 16 CONSOLIDATED** | Five branches → one landing. Addenda rebuilt in reading order (49-56) rather than finish order. **The disjoint write sets held: 21 code files, zero cross-lane overlap**  -  only the two memory files every lane must touch conflicted, which is the section 6a model working. Integration gates on the MERGED result: 1715/1715 suite, tsc clean, fitness 22/22, watchlist npmtests 16/16 | Addendum 56 |
...
| **DONE** | **All 10 `code_location` pointers re-verified this session, zero corrections needed.** Opened every cited file (`discover.mjs` lines 55/81-85, `pair-view.mjs:83`, both `recommend-classification/route.ts` files, `urgency.mjs:8-22`, `factor-tier.mjs:41,47,54,61,68`) and confirmed the literal is present at the exact cited line. The spec's own section 2 table (itself a "spec-from-repo" pass, re-verified independently) was accurate on every row  -  a genuinely clean result, stated plainly rather than padded with a correction that isn't there | this session, direct file reads |
| **DONE** | **10-row seed fixture built.** `scripts/gen/fixtures/assumption-register/wo20-catalogued-assumptions-2026-08-30.json`, one row per section 2 table entry (not per individual literal  -  see the two spec-tension resolutions below), `as_at_date` stamped `2026-08-30` on every row (the date each pointer was re-verified this session). `governing_decision` is `ADR-019` (row 6) and `ADR-008` (row 9), `NULL` on the other 8  -  row 8 (bias-tag thresholds) deliberately registers the CURRENT code value with `governing_decision=NULL` rather than citing ADR-007, per spec section 7 Q1's own recommendation (ADR-007 ratifies different numbers, implemented in a script confirmed absent from the repo) | fixture file |
...
| **THE orderBy LESSON, APPLIED  -  AND ITS PRECONDITION DID NOT REPRODUCE HERE** | The brief that opened this lane warned, correctly as general practice, that `readAll`'s `orderBy` defaults to `"id"` and that omitting it is fatal when a table's PK isn't literally `id` (today's `emission_factors` story, Addendum 60). Checked directly: `assumption_register`'s PK **is** literally `id` (spec section 3's own `CREATE TABLE`), so the precondition that made the default throw for `emission_factors` does not hold here  -  the default would NOT have thrown. `orderBy: "assumption_key"` is still passed explicitly (not the default, and not bare `"id"` either): defensively, against a future PK rename reintroducing the failure class silently, and because `assumption_key`  -  the register's real natural key  -  orders the dry-run/apply console report by dot-path/subsystem rather than by an opaque random uuid. A dedicated test (`seedAssumptions reads assumption_register ordered by assumption_key, not readAll's default 'id'`) asserts the exact value passed, with a real `readAllFn` spy, not a stub that ignores its arguments | `assumption-register-common.mjs` header + `assumption-register-common.test.mjs` |
| **TWO SPEC-INTERNAL TENSIONS, RESOLVED AND FLAGGED, NOT SILENTLY PICKED** | (1) **Granularity**: spec section 3's own naming example (`urgency.priority-to-score.high`) implies one row per individual numeric literal (13+ rows: row 6 alone packs 3, row 9 packs 8, row 10 packs 5), while spec section 5.4 explicitly commits to "10 rows, one per section 2 entry." Followed section 5.4's more specific, more binding numeric commitment: exactly 10 rows; every packed sub-literal for rows 6/8/9/10 is transcribed in full inside `rationale`/`unit`, not lost, and value_numeric carries the single most consequential literal per row (the headline weight; the auto-apply threshold; the MODERATE/default urgency score, which is literally `urgencyScoreFromPriority`'s own fallback default; the `modal_default` pedigree floor, "THE v1 BASELINE" per `factor-tier.mjs`'s own comment). (2) **Subsystem naming**: spec section 3 says `subsystem` = "first key segment" (a schema rule), but spec section 7 Q2 separately names the 4 subsystem VALUES as hyphenated compounds (`connections-scorer`, `bias-classification`, `emission-factors`) that disagree with section 3's own un-hyphenated example (`connections.scorer...` → first segment `connections`, not `connections-scorer`). Resolved by making `assumption_key`'s first segment the HYPHENATED name from section 7's list, so both spec passages agree, rather than reproducing section 3's own example verbatim | fixture header comment; this addendum |
...
| **NOT DONE, BY EXPLICIT SCOPE** | Migration 271 is **NOT applied** (coordinator-only, two-track policy). The fixture is **NOT seeded** (no `--apply` run, no DB access this session). No admin-panel reader (spec section 4's named minimum first reader) and no drift-check script (spec section 4's named-but-unbuilt `assumption-register-drift.mjs`) were built  -  both explicitly out of this lane's scope, per the brief and per spec section 6's own anti-scope list | brief; spec section 6 |
| **DONE (closed train 43: 271 applied live before train 38 (closure gate, train 43): the row's action happened; the assumption-register seed itself is W2.3 / T45)** | Coordinator applies migration 271 (schema-only, additive, 0 rows  -  safe per the same reasoning 267/268's own headers give). After it lands live, `node scripts/gen/assumption-register-seed.mjs --apply` seeds the 10 rows (still requires a human/coordinator decision, per this WO's own section 5 step 4  -  "a separate, later, ratified pass"). Then spec section 4's admin-panel reader and, later, section 4's drift-check script | scope section 5/section 6 |
...
| **DONE (closed train 43: migration 272 applied live (closure gate, train 43); the TS half is live with it)** | None of the 8 customer-facing RPCs project `ii.jurisdiction_iso`, so the TS half of the jurisdictionIso fix is dormant by design until the RPCs catch up. Lane `lb` left an exact spec: add `jurisdiction_iso text[]` to the `RETURNS TABLE` + `SELECT` of all eight | Addendum 65 |
...
| **DONE (closed train 43: migration 272 applied live (closure gate, train 43))** | Coordinator applies migration 272. Once live, list/ledger surfaces (`DashboardTopPriority`, `RegulationsLedger`, `MapPageView`, `OperationsItemsView`, `OperationsLedger`, `MarketIntelLedger`, `app/community/page.tsx`) start receiving `jurisdictionIso` with zero further TS change  -  lane `lb`'s mapper wiring is already live and dormant | Addendum 63/66 |
...
## System completion train  -  LANDED (#517 `2e1afc76`), MIGRATIONS 281-287 LIVE (2026-09-02)

**Operator rulings:** build every remaining part now, spec 08 included; no deferrals; "if you decide that
it needs to be done, we do it." Plan: `docs/plans/system-completion-plan-2026-09-02.md`. Decisions:
`docs/decisions/ADR-024-decision-propagation.md`.

| State | Item | Evidence |
|---|---|---|
| **BUILT  -  runtimes** | `ledger-consume.yml`, `population-turn.yml` (export → record mint → guarded apply with M4 pre-check → propose-tags dry), `change-detection.yml` (route with bounded limit → reconcile dry/apply → drain), `propagation-drain.yml`; three harness families registered with first-run markers | Addendum 84 |
| **BUILT  -  spec 08 (decision propagation)** | ADR-024; migrations 282-287 (entity spine, progressive FKs, outbox + triggers, DAG + `derived_values` + admissibility view, statutory/estimate isolation, aggregate safeguards section 5.2 a-d + dominance + forward-looking); `admissibleFor`/`FLOOR`; `runPropagationDrain`; F30/F31/F32 | Addendum 84 |
| **BUILT  -  the dependents** | automate-vs-hire (ranges, edges to wage + energy facts), carbon intensity per tonne-km, FuelEU Annex IV penalty (verified on EUR-Lex in the browser), `RecalculationNotice` + `/api/notices`, seed script, `lc_lci_lev` producer, BLS hourly series | Addendum 84 |
| **BUILT  -  producers + surfaces** | `data_sources.ecb` (281) + ecb-fx armed at the code gate; oil-bulletin history backfill; registry parity; Market Intel freshness panel, methodology drawer, promotion chip, honest delta ribbon, obligations strip; `originClass` on Resource | Addendum 84 |
| **FIXED  -  14 joins** | classify outside the spend chokepoint; check-sources limit/response; oil-bulletin fetch-on-import; `estimated_values` PK shape; regions without entities; annual-vs-hourly wage; US/EU fact disjointness; dry `upserted` metric; stale markers; merged allowlist JSON; double telemetry; rule 016 prose match; CONVENTION table; migration 281 inventory | Addendum 84 |
| **GATES** | suite 3,001/3,001 · npm-deps 357/357 · fitness 26/0 · meta-gate PASS · C3/C5 PASS · tsc clean · engine 4/0 | Addendum 84 |
| **LANDED + APPLIED** | PR #517 squash `2e1afc76`; 281-287 applied live in order (two deviations found at apply and written back: 285 self-check event cleanup, 286 purity check by estimate_id) | Addendum 84 postscript 1 |
| **POPULATION  -  run #4 read, gate fixed** | 19 exported (UK/FR), 0/19 valid: the JS mint gate resolved tier by exact URL where the registry (and the live gate) resolve by institution  -  fixed (`institution-key.mjs`, shared with `registerSource`), 19/19 re-validate; record-facts prose guard (menu chrome was a scope FACT); 26 EUR-Lex rows bot-gated → Cellar capture first (`publications.europa.eu/resource/celex/<key>`), EUR-Lex second; mint-run-007/008 landed, PENDING-RUN re-stamped, proposer pass names 008 | Addendum 84 postscript 7 |
| **POPULATION  -  run #5 dry, 45/45** | Cellar measured from the runner (26/26 EU captured), 45/45 valid at `sha256:2d498956fb8c476f`, PENDING-RUN discharged, mint-run-009 landed; legacy-HTML title fix for six older acts | Addendum 84 postscript 8 |
| **POPULATION  -  runs #6-#8 (apply)** | WO-26 stamp: statement timeouts → `guardedUpdateByIds` with halving (491/491 live); run #8 wrote 10 items all `quarantined` (gate A written after the claims; the trigger's last derivation saw no gate) + one bare row (U+0000 refused mid-payload, no per-payload boundary) → gate before claims, row-status outcomes, per-payload failure cleanup, U+0000 dropped at capture, post-apply `rederive-record-provenance.mjs`; bare row deleted with a recorded snapshot | Addendum 84 postscripts 9-10 |
| **POPULATION  -  run #9: 43 minted verified, 53 live** | first record-grade items on the site (`mint-run-011`); reconciliation healed run #8's 10; FAILED status was the rederive self-check reading the pre-trigger tuple (fixed); guillemet span delimiters (kit unicode scan) | Addendum 84 postscript 11 |
| **POPULATION  -  run #10 clean, 92 live; pages read** | 39/39 verified (`mint-run-012`), corpus turn #4 ran the flywheel (2,610 edges, 17 themes, 2 forward events); `/regulations` carries the record items; `/market` renders the train's surfaces on one-observation series (producers not yet applied); `/operations` calculator: unit/ordering defect fixed, reworded to "Capacity investment estimate" (operator ruling); `/research` unchanged, no feeding data path | Addendum 84 postscript 12 |
| **POPULATION  -  DEFECT, MINE: runs #9-#11 minted the unscreened pool** | 122 minted, ~half off-vertical by the 2026-08-31 screen ruling (never stamped on the census; the exporter selected on would_mint alone). Fixed in the runtime: screen gate at the export (`lib/screen-verdict.mjs`), `screen-reconcile-records.mjs` after apply (reversible archive, ambiguous listed); next apply performs the archive | Addendum 84 postscript 13 |
| **PLAN RECONCILIATION (2026-08-31 build plan + 2026-09-01 review section 10)** | Complete: RT, EV, HYG, DOC, CD (built), S1, F1, U7, M0, spec 08, landing. Partial: TAG (proposals unratified), SURF/P1/P2 (producers built, no applies; EUA ruled out), S2 (topics seed never applied, 0 live), F2 (dry only), W1 (register awaits wire-8/delete-8 ruling), M1..Mn (runtime live, screen gate now). Not done: R1 review dossiers (927 provisional sources, 331 canonical candidates, 1,457 portal links), SM smoke tests, DESNZ air/sea fixture, Wave D design debt (community core, spec-09 tables, obligation register, FEU overlay, acceptance gates, taxonomy, 5-axis, dashboard rebalance, credibility chips, W2.F). Rulings open: off-vertical 1,676 archive/park, W1, SERIES_ITEM_MAP, origin_class, EIA secret | Addendum 84 postscript 13 |
| **FINISH PLAN (2026-09-02)** | `docs/plans/finish-plan-2026-09-02.md`. Off-vertical ruling applied to the screen (2 rules + 6 reviewed verdicts on_vertical; live records 71 on / 41 off / 10 ambiguous); reconcile step archives on the next apply dispatch. Wave 1 lanes: WSEQ, MAINT, R1, F2, PROD-FIX, GATES-1, HYG-2. Open rulings R-A…R-H in the plan section 1 | Addendum 84 postscript 14 |
| **WAVE 1 (finish plan)** | Landed as one train: WSEQ (shared `write-item.ts` sequence for both mint tiers; kit check `screen_verdict_missing` / `screen_verdict_not_on_vertical`, mint marker `sha256:9a3e4c77ec4d9342`), MAINT (`maintenance.yml`, 7 dispatch-only steps; tier-opinions NOT RUNNABLE, upstream is the paid brief agent), R1 (digest builder + 4 apply scripts; queues are `sources.status=provisional` and `coverage_gap_candidates.disposition IS NULL`, not the tables the plan named), F2 (`inaccessible-triage.mjs` + `source-monitoring.yml` job `triage`), PROD-FIX (DESNZ air/sea: 7 shells `needs_runner_fetch`, gov.uk assets blocked from the container; `series-item-map.json` pending R-D, `--propose-items` validates 6 payloads), GATES-1 (F33 surface-acceptance + register, 4 SM smoke specs in the rendering guard, W2.F report), HYG-2 (migration 288 `sources.source_type`, taxonomy + backfill, coverage-gaps thin lookup; profile sectors root cause fixed at `layout.tsx` by the coordinator; F25 18/18 cited). Coordinator applies owed: migration 288, backfill dry→apply, DESNZ shells via a runner fetch, R1 digests dispatch | Addendum 84 postscript 15 |
| **WAVE 2 (finish plan) + BUILD-BEFORE-POPULATE** | Operator ruling 2026-09-02: build the tools and the UI before populating; no population dispatch until Waves 2 and 3 land. Nine lanes landed as one train: OBLIG (migration 290 `obligations` + derivation + register on /regulations), CORR (corridor seed on the spine, carbon-cost-per-FEU estimate module + overlay on /market, GAP on every axis today), DASH (five-surface pulse grid on the home page, research credibility chips "Not scored" with GRADE ledger), RSRCH (`source-sweep` subject `research`, research-grade record profile), AXIS (Axis 3/4/5 classifiers + proposer/apply in the TAG pattern; live `sources.jurisdictions` holds a Haiku region vocabulary, review-only), HELD (registered-institution fallback closes the `identity_unmapped_source` class; FR canonical keys), INTAKE (record profile: binding_position, due_date+precision, corridor identity, credibility signals; two FR item types withdrawn by the coordinator), DESNZ (runner-side xlsx fetch + extractor for the seven shells), BUILDGATE (`build-proof.yml` runs the real Turbopack build in CI; eight route files split into `logic.ts`; worktree symlink handled in next.config). Coordinator applies owed: migration 290 → derive-obligations dry/apply; seed-corridors dry/apply; DESNZ producers apply on the runner. Open: the kit requires three new slots the live `item_type_required_slots` table does not (added in the population train with the re-mint). | Addendum 84 postscript 18 |
| **WAVE 3 (finish plan): UX gates + community + spec-09 + mobile** | Operator rulings 2026-09-03: R-H yes, R-G yes; twenty UX laws adopted, and adopted as GATES not prose (operator: "if they are not wired… that is dead code and bloat"). Landed as one train: `ux-assert.mjs` detectors + UX smoke slot (real components at 375 px), F35 `row-ux-coverage` + RD-60, discipline CI "UX compliance" gate, lane contract versioned at `docs/dispatches/lane-common-contract.md`, `sprint-followups-discipline` path fixed; COMMUNITY-A (migrations 293-295, guard, identity, promotion machine, benchmark seeder), COMMUNITY-B (entity-bound composer, pseudonymous chips, discover/directory/benchmarks, no-DM, peers strip), SPEC-09 (migrations 296-298, ten tables empty with $0 gaps named, calculators, seven panels), MOBILE (one row system, seven screenshot defects fixed, law-2 floor). Migrations 293-298 LIVE. PERF-MEASURE audit: server render 0.9-2.1 s, click-through 1.3-1.9 s, cache MISS everywhere, sequential awaits, prefetch disabled after 503 fan-out. Open: PERF fix lane; audit test post deletion (operator); DESNZ producer apply; benchmark seed dispatch | Addendum 85 |
| **PERF (2026-09-03)** | Measured, fixed, re-measured [audit `perf-load-times-2026-09-03.md` section 1-section 9]: skeleton on every click (<2 ms); detail server render market 1,905→432 ms, operations 1,262→329 ms, research 1,597→715 ms, regulations 1,257→647 ms; one cached parallel loader for the four detail pages, index ledgers cached per org, middleware `getClaims()`, cache flush after mint apply. OPEN: a self-healing 503 on router RSC navigations, never in a function log, not reproducible by `fetch()` with RSC headers; hypothesis deploy-transition skew; settle with a quiet-window measurement | Addendum 85 ps 1, 3, 4 |
| **RUNTIME APPLIES (2026-09-03)** | DESNZ seven shells filled (`emission_factors` 13 rows, real 2025 sheet values with row/col provenance); 3 house benchmark instruments seeded for 2026-Q3 (open). Build-before-populate: Waves 2 and 3 landed, the one population pass is next | Addendum 85 ps 4 |
| **503 CLOSED (2026-09-03)** | No production 503 on item navigation: the extension's network reader mislabels completed RSC responses (in-page `responseStatus` 200 and Vercel's edge log 200 for the identical request). Audit section 11; four earlier sections retained as the record of an instrument artefact | Addendum 85 ps 6 |
| **COMMUNITY-C (2026-09-03)** | Benchmark responses have a write path: server-only `organisation_key`, profile + verify routes, respond route (k-anonymous aggregate only), response form and profile page. Operator: provision `COMMUNITY_ORG_SALT` on Vercel | Addendum 85 ps 6 |
| **POPULATION PASS: decision-ready** | `docs/plans/population-pass-2026-09-03.md`: dry pipeline proven, dispatch table, migration 299 written NOT applied (live N = 149 verified items would quarantine), rulings R-A…R-F with live numbers. `population-turn` and `corpus-turn` can dispatch on current master; 299 waits on the operator's choice (leave the DB looser than the kit, or GAP-claim backfill first) | Addendum 85 ps 6 |
| **POPULATION PASS: first pass RAN (2026-09-03)** | Dry #13 then apply #14 on master `20a3c02`, limit 200, capture true: 178 exported, 22 held with evidence, 1,118 off-vertical + 244 ambiguous screened out; 178/178 valid, **177 minted verified, 0 apply failures**, 1 holder refusal (32019R1242 already held since April). Live `intelligence_items` 1,233 → 1,410. Follow-ups on the 177: empty connection tags (propose-tags/apply-tags), `origin_class` null (R-E now 940/1,160). Pool: ~1,300 mintable rows remain (~7 runs at 200). Artifact branches `population/33747655857`, `population/33749140151` (mint-run-015/016) await landing on master. Drain paused on a permission-classifier refusal of the `apply` form; operator to choose: hand-run remaining applies, corpus-turn now, or hold at one pass | Addendum 85 ps 7 |
| **EXPORT-HOLD (2026-09-03)** | The ten EUR-Lex `(01)` rows held `capture_blocked` every run were a URL-encoding defect (`encodeURIComponent` leaves parens literal; Cellar 404s literal, 302s percent-encoded), fixed at the encoder; residual robot-gate holds classify `no_capture_path` and stop re-fetching; exporter excludes key-held rows with M4's own rule (`already_held_by_key`, holder id). Next population dispatch measures the capture gain | Addendum 85 ps 8 |
| **TAG-PROPOSALS (2026-09-03)** | 339 of 619 verified live items untagged and zero tag-proposal flags ever written: the ratification pipeline's first step had no dispatch (population-turn's `propose-tags --dry` writes nothing; header corrected). New MAINT step `tag-proposals` (dry/apply, `untagged`/`since:`/`ids:`), `proposeTags()` core exported, writer registered. Dispatch after the population drain; operator ratifies with `ratify:tags`, then `tag-ratification` applies | Addendum 85 ps 9 |
| **FLYWHEEL TURN RAN (2026-09-03)** | corpus-turn #5 dry / #6 apply since 1970-01-01: 5,455 edge rows written (3,449 new), all 177 new mints connected; 70 forward events on 21 items (`item_forward_events` 973); 18 themes, 19 gaps, 930 signal candidates as open flags for operator review. Artifacts run-003 (orphaned since Sep 2) and run-004 landed with the proposer pass. Findings: record-grade yield 9.6% vs 43% brief-grade (input ceiling); `due_date` slot invisible to the extractor (proposal on file) | Addendum 85 ps 10 |
| **FE-SLOT (2026-09-03)** | Coordinator's hypothesis refuted by the lane: the `due_date` slot claim reaches the extractor (no `slot_key` column exists; `[due_date] ` prefix on claim_text), 34 of 48 are unclassifiable durations or truncated windows. Extractor now slot-aware without kind assumption, finer precision wins, `slot_date_unclassified` skip + `by_skip_reason` metrics; PENDING-RUN at `d47a10728a3cc799` discharged by the next corpus-turn apply. Open: `record-facts.mjs` window/anchor for duration spans (mint governing file, decision owed) | Addendum 85 ps 11 |
| **NO HUMAN GATE (ADR-025, 2026-09-03)** | Tags adopt at `high` (`tag-ratification --arg auto`), decisive signals become edges in `analyze-corpus --signals` apply and their flags resolve, classifications `--auto-adopt`; residue stays a flag. Drain: dispatch `tag-proposals` apply, `tag-ratification auto`, corpus-turn apply, `apply-classifications --auto-adopt` | Addendum 85 ps 12 |
| **PROVENANCE HEAL (2026-09-03)** | Operator: attach the source, do not blame the item. MAINT `provenance-heal` (capture → ground → slots → gate A → re-derive → un-archive) for 97 quarantined live, 58 archived-unreasoned, 135 uncaptured, and `slots-backfill` for the 149 ahead of migration 299. Dispatch dry then apply per selection, then apply 299 | Addendum 85 ps 12 |
| **TAG YIELD (2026-09-03)** | Record-grade tag yield 16 → 72 of 178 with widened input (sections, FACT claims, captured window) + legal-text aliases; wired into `propose-tags` | Addendum 85 ps 12 |
| **PERF-3 (2026-09-03)** | Root cause of the frozen-page navigation: the root layout awaited the Supabase bootstrap before any JSX, blocking the RSC stream and the `loading.tsx` skeletons; skipped on `rsc` navigations. Watchlist 6 → 0 fetches on /market; listings/rest 63% smaller + browser cache/ETag. Vercel logs: zero 503 in 24 h (the extension's label, again). Measure after deploy | Addendum 85 ps 12 |
| **BUGS (2026-09-03)** | Regulations bands honest loading state instead of a false "no matching"; community item links use `itemDetailHref` (market/operations/research went to the bare index). W1 register repaired (R-C closed: rows 8/10/6/2). POPULATION-TURN-RUNBOOK written | Addendum 85 ps 12 |
| **R-D READY (2026-09-03)** | Six oil-bulletin `market_signal` rows as a `rows_file` batch (6/6 validate); dispatch population-turn dry/apply with the batch after substituting the live `sources` id; then `ratify-series-items --apply` and `refresh-published-price-statistics --apply` | Addendum 85 ps 12 |
| **PERF-4 (2026-09-03)** | Document loads stream (shell synchronous, bootstrap behind Suspense via `use()`); detail pages carry watch state from the server; Sidebar prefetch at Next's loading-boundary default. Measure in production after deploy | Addendum 85 ps 12 |
| **R-F CLOSED, SALT OPTIONAL (2026-09-03)** | `EIA_API_KEY` mirrored to GitHub by the operator, registered, producer step added, ENABLED flipped (reviewed change). Community organisation salt derives from `WORKER_SECRET` via HKDF when `COMMUNITY_ORG_SALT` is unset: nothing left for the operator to provision. Dispatch `producers` eia-v2-petroleum-spot dry then apply | Addendum 85 ps 13 |
| **HEAL-2 (2026-09-03)** | First heal pass: 2 of 97 healed; the bulk is tier-3+ or NULL sources on facts (596), gate-A orphans (82), missing analysis labels (219). HEAL-2 re-sources facts to the primary capture, resolves institutions, grounds orphans, relabels, re-kinds unprovable facts. Dispatch `provenance-heal quarantined-live` apply again and measure | Addendum 85 ps 14 |
| **HEAL-3 (2026-09-03)** | HEAL-2 apply: 0 healed; label lookup fixed (normalised), slot claims never re-kinded to ANALYSIS (SLOT-REPAIR), CAPTURE-CITED fetches the sources the brief itself cites. Gate-A orphans are 100% brief prose. Dispatch apply, then decide the residue: keep quarantined or rewrite briefs to what their sources support | Addendum 85 ps 15 |
| **CD-GATE (2026-09-03)** | Change detection #4 apply checked 0 of 959 due sources: the route exits at `isGloballyPaused()` while `scrape_cadence='off'`, the build-mode state. Driver now reads the gate both modes (`readScrapeState`, throwing form; `scrapeWindowOpen`), reports `scrape_gate`/`sources_checkable`, classifies a closed-gate apply `gate_closed_at_route`; an unreadable `system_state` is a run error, never "cadence_off". The gate is not bypassed and the cadence is not a decision: build mode holds it off, stale items are held and kept (standing rule 16) | #555 `59d00aee`; Addendum 85 ps 16 |
| **HEAL-3 MEASURED (2026-09-03)** | #14 apply: 0 of 95 healed; 81 resourced, 36 orphans grounded, 141 cited URLs captured, 232 slot claims back to GAP, 0 relabelled (713 `no_owning_section_found`). Residue: `analysis_missing_label_syntax` 365 (45 items), `gate_a_unproven_or_stale` 88, `fact_below_authority_floor` 81 (3 items), slot 4, brief 1, url 1. Items stay quarantined and kept | Addendum 85 ps 17 |
| **HEAL-4 (2026-09-03)** | Root cause of the 365: RECLASSIFY re-kinded FACT→ANALYSIS without touching `claim_text`, which was never verbatim in the section (criterion 4 is an ILIKE substring of a labelled paragraph): of 365, 4 verbatim, 105 after normalisation, 260 paraphrases. HEAL-4 (`hp4-2026-09-03.1`): owning paragraph by Jaccard token overlap (threshold 0.15), `claim_text` rewritten to the best verbatim sentence with markers stripped, or `reclassify_refused_no_owning_paragraph`; RETROFIT step for the already-re-kinded claims; RELABEL replaces a leading `**FACT:**` marker rather than stacking. 143 tests | lane heal4 `0e437977`; this train |
| **SRC-TIER (2026-09-03)** | The 81 floor failures are a registry defect: "Smart Freight Centre" exists twice in `institutions` (smartfreightcentre.org T4, and amazonaws.com T5 keyed from the GLEC PDF's S3 host), so the standard's own text fails its own-body floor; ifrs.org (standards_body) sits at T5 against the class table's T4. New MAINT step `institution-canonicalize`: Part A merges generic-host duplicates by exact name (12-domain list, no fuzzy rule), Part B canonicalises intra-institution tiers to the own-domain tier (never a tier no row carries; conflicts reported), Part C reports standards-body rows worse than T4 as `ruling_needed` (ADR-002: base_tier changes are operator overrides). 25 tests | lane srctier `a60819a0`; this train |
| **POPULATION  -  runs #15-#17 (2026-09-03)** | #15: 177 minted verified (798 live). #16: 168 (966 live, 1,755 total). #17 dispatched. origin-class-backfill apply (R-E accepted): 1,160 nulls → 211 (168 are #16's mints, 12 no source, 31 no rule) | Addendum 85 ps 17 |
| **HEAL-4 MEASURED (2026-09-04)** | #17 apply: 1 of 95 healed; 223 claims retrofitted to verbatim sentences, 152 refused (no owning paragraph ≥0.15), 164 paragraphs relabelled. Live residue: `analysis_missing_label_syntax` 365 → 148 (38 items), gate-A 88 (brief-prose orphans, the honest end state under $0), floor 81 (3 items), slot 4, brief 1, url 1 | Addendum 85 ps 19 |
| **INSTITUTION-CANONICALIZE APPLIED (2026-09-04)** | #18 dry matched the two named defects exactly; #19 apply: the amazonaws.com "Smart Freight Centre" institution merged into smartfreightcentre.org (1 source moved, duplicate deleted), the GLEC v3 PDF source 5 → 4 (institutions 460, merged ids absent on read-back). Part C `ruling_needed`: ifrs.org (2 rows T5), cdp.net (T5), sciencebasedtargets.org (T5), standards bodies the class table puts at T4  -  operator override under ADR-002 | Addendum 85 ps 19 |
| **CITED-HELD (2026-09-04)** | 67 of 208 cited URLs held on HEAL-4: 60 `capture_blocked` (bot gates on imo.org, iea.org, sciencedirect, meti.go.jp, ilostat…), 5 EUR-Lex OJ-issue references without a CELEX, 2 bare federalregister.gov. HEAL-5 (`hp5-2026-09-04.1`, 157 tests): OJ references resolve to the Publications Office `resource/oj/JOL_<year>_<issue>_R` endpoint; blocked or thin captures fall back to the Wayback Machine (`archive.org/wayback/available` → `web.archive.org/web/<ts>id_/<url>`, raw bytes, same extraction incl. PDF), `result_url` stays the publisher's URL, evidence carries `transport: wayback` + snapshot timestamp; `capture_thin` split from `capture_blocked`. Doctrine point: the archive is transport, never a source; attribution unchanged, every use labelled. Container egress refused archive.org, so the first live measurement is the next heal apply | lane citedheld `26c4783c`; this train |
| **PRODUCERS #21 dry (2026-09-04)** | EIA v2 petroleum spot would create rows (WTI, Brent, gasoline, diesel, jet, propane, weekly back to 2017) with the key; table health all FILLED (market_series 6, emission_factors 13, regional_data_facts 86, published_price_statistics 4). Apply dispatched next | this train |
| **PARALLEL LANES (2026-09-04)** | Operator: lanes run concurrently or the queue is single-threaded. Four launched in one call (REOPEN-STEP, ARTIFACTS-2, MARKET-SURF, ECB-FX); the fourth was refused by the tool-permission layer twice, so lane execution moved to a background workflow (`lanes-wave12`: ECB-FX + URL-BOILER) that runs while the coordinator dispatches through the browser | Addendum 85 ps 20 |
| **REOPEN-STEP (2026-09-04)** | MAINT step `reopen-validation-holds --arg <reason-contains>`: wraps URL-GUIL's re-admission tool (only Actions holds DB credentials); refuses a blank scope in both modes; dry plan / apply read-back; 116 tests. First dispatch: `arg=ungrounded_url` after URL-BOILER lands, or the two UK-SI rows re-fail | lane reopen `cf175d97`; this train |
| **ARTIFACTS-2 (2026-09-04)** | Population #17/#18/#19 landed (mint-run-019/020/021, +156/+152/+141); proposer pass: guillemet truncation confirmed gone (run-020 fails the same row on the FULL `http://eur-lex.europa.eu`), hold-back proven across the run boundary (the row is absent from #19), a second UK SI (2012/2567) hit the identical explanatory-note boilerplate → lane URL-BOILER; mint PENDING-RUN discharged by run-020's hash. Live: 2,204 items, 1,415 live verified, 94 quarantined | lane artifacts2 `adbcbe45`; this train |
| **MARKET-SURF (2026-09-04)** | No change needed [CONFIRMED by trace]: `/market` → `fetchMarketSeriesBoard` (no prefix filter) → `buildSeriesBoard` groups by registry `keyPrefix` → `MarketSeriesBoard`/`MarketComparativeRibbon`; `eia-v2` is `implemented: true` since 2026-09-02, so the 2,727 EIA rows and the 2026-08-31 oil-bulletin week render today. `n_observations` null on oil-bulletin rows because the producer never emits `n_member_states`; the drawer omits the line, nothing gates on it. Stale prose: `SERIES_HISTORY_LIMIT` comment in supabase-server.ts | lane report |
| **R-D READY TO MINT (2026-09-04)** | No `sources` row carries the bulletin URL; `registerSource` dedups by `institutionKey` = energy.ec.europa.eu, already four DG Energy rows (T2, institution 3cc5e02b); the six rows now carry `cb816051` at T2 (the batch file claimed T1, the Commission is a regulator, T2). Dispatch: population-turn `rows_file` dry → apply → `ratify-series-items.mjs --apply` (lane) → producers `refresh-published-price-statistics` | this train |
| **PRODUCERS #22 APPLIED (2026-09-04)** | EIA v2: 2,727 `market_series` rows created (six US spot series, weekly since 2017-12); oil bulletin: 6 rows for 2026-08-31; BLS: 3 inserts. ECB-FX parsed 0 rows: the live feed single-quotes its XML attributes and the parser matches only double quotes [CONFIRMED, live fetch from the Codespace] → lane ECB-FX | Addendum 85 ps 20 |
| **ECB-FX (2026-09-04)** | Producer parsed 0 rows because its `TIME_RE`/`CCY_RE` matched only double-quoted attributes and the live feed single-quotes them [CONFIRMED, Codespace fetch]; parser now reads named attributes quote-agnostically (envelope validation unchanged), refusals log status/content-type/bytes/body head; the same assumption fixed in `fetch-oil-bulletin.mjs`'s history-link regex and hardened in `oil-bulletin-workbook.mjs`; 137 tests. Next producers `ecb-fx` apply | lane ecbfx `f7c50d24`; this train |
| **URL-BOILER (2026-09-04)** | The recurring UK-SI criterion-2 failure is boilerplate ("may be viewed … via the EUR-Lex website at http://eur-lex.europa.eu"), selected by the `jurisdictional_scope` trigger as a FACT; `record-facts.mjs` now rejects a span whose only URL is a bare domain (no path/query) as a non-citation pointer, the slot resolves GAP honestly, both named rows validate with zero failures [CONFIRMED by replaying their real captured text]. Hypothesis (b) refuted: the registered EUR-Lex source is `https://`, the citation `http://`, and `canonicalize_citation_url` deliberately never folds scheme; no migration 301. Mint PENDING-RUN re-pinned at `sha256:8712c28763b44ff2`; the two held rows reopen via `reopen-validation-holds --arg ungrounded_url` | lane urlboiler `872c9c14`; this train |
| **HEAL-5 #20 TIMED OUT (2026-09-04)** | The first archive-fallback heal ran 15m20s and was cancelled by the maintain job's `timeout-minutes: 15`; no artifact. Lane HEAL-BUDGET (background) adds a time budget with checkpointed artifacts, resume-by-ids, and a measured job timeout | Addendum 85 ps 21 |
| **HEAL-BUDGET (2026-09-04)** | `hp5-2026-09-04.2`: run loop checks a time budget before each item (`HEAL_TIME_BUDGET_SECONDS=1500`, injectable clock, never read in dry mode), stops cleanly with `stopped_at_budget`/`items_remaining`, checkpoints `summary.json` atomically after every item, resumes via `ids:`; CAPTURE-CITED gains a run-level URL cache (1 fetch for N citing items, each keeps its own evidence row); maintain job timeout 15 → 30 min with the arithmetic (HEAL-4 6.8 s/item; HEAL-5 adds ~130 paced requests). 171 tests. Dispatch: apply, then `ids:<items_remaining>` until no `stopped_at_budget` | lane healbudget `3c06b1f8`; this train |
| **ARTIFACTS-3 (2026-09-04)** | Population #20 landed (mint-run-022, 140/140 verified); R-D rows_file dry landed (mint-run-023, 6/6 valid), apply held until TANDEM lands (rule 17); change-detection run-005 landed, CD-GATE marker discharged by its hash; gate `cadence_off`, 959 due / 0 checkable, build mode | this train; Addendum 85 ps 23 |
| **HEAL #21 = 0/94 (2026-09-04)** | First budgeted heal finished (11m56s) and healed nothing: 88 items fail criterion 7 `gate_a_unproven_or_stale`, 38 criterion 4 `analysis_missing_label_syntax`; the scanner reads `full_brief`, the heal edits sections. Lane HEAL-6 diagnoses from the live function and rows, then builds the step that moves the brief text; grade changes behind a dry flag for the operator | Addendum 85 ps 23 |
| **ONE WORKFLOW PER LANE (2026-09-04)** | The Workflow tool caps agents per workflow at the container CPU count (2): a five-agent workflow starved three lanes. Every lane now launches as its own background workflow; eight concurrent (TANDEM, HOLLOW-GATE, HOLLOW-SWEEP, CLASSIFY-STEP/Haiku, RECORD-SURFACE, PERF-5, SITEMAP, HEAL-6) | Addendum 85 ps 23 |
| **SITEMAP-PORT (2026-09-04)** | Operator: use the open-source monitor's code where it is better. mreflow/control-center (MIT) wins on discovery order (feeds first), bounded bytes, limits, path scoping, deferred baseline; ours wins on polite fetch, gzip, ledger/snapshot contracts. Lane SITEMAP ports with attribution (THIRD-PARTY-NOTICES.md) and wires the walker into source-sweep → census ledger → change-sweep | Addendum 85 ps 23 |
| **HOLLOW-SWEEP (2026-09-04)** | 551 live record items are title-only (390 initiative, 158 regulation; eur-lex 379, legislation.gov.uk 149, federalregister 21). MAINT step `record-hollow-sweep`: archive `record_hollow` + blank the instrument key/identifier/source_url so the mint re-admits the row (archived holders block M4 otherwise); restore SQL per item in the artifact. Dispatch dry → apply after HOLLOW-GATE lands, so the re-mint carries facts | lane hollowsweep `b8710971`; this train |
| **CLASSIFY-STEP (2026-09-04)** | MAINT `apply-classifications`: propose (classify/drift/anomaly flags) → auto-adopt under the GSIG ruling, dry/apply; the classification scripts had no runtime at all | lane classifystep `d14e8e90`; this train |
| **PERF-5 (2026-09-04)** | Queries are 0.6 to 25 ms live; the cost was sequential round trips (ten awaited timeline chunks, overrides serialized behind the RPC). Parallelized in supabase-server.ts; depth 12 → 2 on the rest chunk. Next lever: `getUser()` in org.ts/auth.ts on every read and route → lane PERF-6 | lane perf5 `94697e07`; Addendum 85 ps 24 |
| **TANDEM (2026-09-04)** | The flywheel is a workflow step now: `run-population-flywheel.mjs` runs section 8/section 9 after every apply (batch-scoped discovery, forward events, recluster, obligations, tags, ratification, outcomes into the mint artifact) and THE GATE refuses a new apply while the last one is unconnected. Runs #15-#20 (~650 items) still owe their pass: the first dispatch under the gate must be told their ids | lane tandem `4afbf7a8`; this train |
| **SITEMAP (2026-09-04)** | Walker `sitemap` in source-sweep: feed first (control-center's discovery order, ported, MIT), sitemap fallback with byte/document/entry bounds, path scoping, deferred baseline; new locs → `portal_link_candidates`, lastmod deltas → `monitoring_queue` (the reconcile pass reads it), feed URL → `sources.rss_feed_url` via the guarded path. First dry: `--host aircargonews.net`, `--host aapa-ports.org` | lane sitemap `340295f2`; this train |
| **RULE 012/019 FALSE POSITIVES (2026-09-04)** | Rule 012 now exempts `scripts/_snapshots/` (captured third-party text, byte-exact by ADR-016); rule 019 no longer trips on a test that restated the source-y reasons (it imports them). Neither rule weakened for code | this train; Addendum 85 ps 25 |
| **HOLLOW-GATE (2026-09-04)** | 375 of 551 hollow records are `initiative` routed to market-signal slots with no triggers; kit now refuses title-only record payloads (`record_hollow`), five EU-act facts attempted on every eurlex capture (11/12 fresh hollow captures gain a fact), `not_in_force` export hold. Residual: bare-domain guard rejects a genuine operative provision (32012D0706(01)) → lane BOILER-2 | lane hollowgate `9ecba401`; this train |
| **HEAL-6 (2026-09-04)** | Criterion 7: heal dropped Gate-B coverage on every apply (fixed, `basis_claim_id` now projected); criterion 4: paragraph search was section-scoped, validator is item-wide (fixed, 100/148 claims find a home). Ceiling: 386 of 824 orphans have no floor-qualifying source; those items need an operator ruling (rebuild brief from grounded facts at record grade, or stay quarantined). Dispatch: provenance-heal dry `quarantined-live`, then apply | lane heal6 `26183439`; this train |
| **RECORD-SURFACE (2026-09-04)** | Record items rendered title-only because the brief-grade heading matcher and numbered section keys never matched record sections; new `parse-record-sections.ts`, record summary on Regulations (facts, key dates, honest gap count, tags, obligations, real connections), badge + facts card on Market/Research; rendering guard 375 px green after two caught regressions | lane recordsurface `2bb2341c`; this train |
| **PERF-6 (2026-09-04)** | `getClaims()` replaces `getUser()` in org.ts (15 callers) and auth.ts (47 callers), org resolution memoized per request (`runViewerScoped` resolved it twice). Same class remains in `server-bootstrap.ts` and `community-auth.ts` → lane PERF-7. Re-measure after deploy | lane perf6 `8665d93b`; this train |
| **RECORD_HOLLOW SCOPE (2026-09-04)** | Kit exempts series-backed `market_signal` payloads (registered series key = the substance); research all-GAP records are refused and held. Both paths of the oil-bulletin item carry the series key as identifier | this train; Addendum 85 ps 27 |
| **PERF-7 (2026-09-04)** | Last `getUser()` round trips gone: server-bootstrap, community-auth (one resolver for cookie/bearer), profile/verify. Document load auth round trips 1 → 0 [INFERRED]; re-measure in the browser after deploy | lane perf7 `730ba69a`; this train |
| **BOILER-2 (2026-09-04)** | Bare-domain guard slot-scoped (URL-bearing slots only; 32012D0706(01) accepted, the two UK-SI rows still refused); list-marker continuation in `jurisdictional_scope`; Cellar RDF-fingerprint captures detected at capture and held at export (`capture_garbled_metadata`, two live rows) | lane boiler2 `d37e8256`; this train |
| **RULE 18 (2026-09-04)** | Operator: "get the source, then rate the source, then publish the data." Lane HEAL-7: SOURCE step registers the cited page with its class-table tier, captures, grounds verbatim; migration 302 turns criterion 3 into a rating; kit mirrors | CLAUDE.md rule 18; Addendum 85 ps 29 |
| **RUNTIMES 2026-09-04** | hollow-sweep apply: 551 archived, 552 census rows back; classifications apply: 1,015 flags, 797 auto-adopted, 218 open; sitemap dry ×3 (one bot wall, two walked), apply on smartfreightcentre.org dispatched | Addendum 85 ps 29 |
| **SITEMAP-2 (2026-09-04)** | Feed-probe trail in the artifact; `bot_wall` outcome + metric for 401/403/429 walls. Open: why smartfreightcentre.org /feed (200) was not recognised, answered by the next dry run's trail | lane sitemap2 `72d98368`; this train |
| **HEAL-7 + MIGRATION 302 (2026-09-04)** | STEP SOURCE registers, rates, captures, grounds each orphan figure; criterion 3 is a rating (`warnings.below_floor_facts`), applied live 05:49 UTC (post md5 `7c4ae6dc…`). Dispatch: provenance-heal dry `quarantined-live` → apply. Drift found: live function carries an own-body-types extension no migration file holds → chore lane | lane heal7 `89388e6d`; this train |
| **TIER-CHIP (2026-09-04)** | Record-fact renderer shows no source tier; lane puts the claim tier (one query) on Regulations/Market/Research fact lines as TierBadge, unrated state honest | running |
| **LEDGER-CONSUME FLIP (2026-09-04)** | 1,838 portal candidates (383 from the sitemap walker) wait on `LEDGER_CONSUME_APPLY_ENABLED` (ADR-023 gate, ~$0.001/candidate classify). Operator decision pending; when flipped, chain source-sweep apply → ledger-consume | Addendum 85 ps 30 |
| **TANDEM-2 (2026-09-04)** | Gate scans every mint artifact; `flywheel_backlog` dispatch connects stale slices oldest-first (cap 2 per run, ~7 runs for 13 artifacts / 1,272 items). Two pre-item_id artifacts (001, 005) block the gate until resolved by CELEX match → lane BACKLOG-LEGACY | lane tandem2 `adf61304`; this train |
| **TIER-CHIP (2026-09-04)** | Record-grade FACT lines show the source tier (`COALESCE(tier_override, base_tier)`, migration 145), one query per page, verbatim claim-line match, honest unrated chip; three surfaces; guard green | lane tierchip `feee2e40`; this train |
| **BACKLOG-LEGACY + DRIFT 207 (2026-09-04)** | mint-run-001/005 ids resolved by canonical key (11/11 live); drift migration 207 records the live own-body extension (schema_migrations 20260801004400) | lanes legacy `b59f79e7`, drift `0e22c1d8`; this train |
| **HEAL #26/#27 (2026-09-04)** | Rule-18 dry: 1,730 orphan tokens groundable (1,543 existing sources, 187 to register, 369 capped, 76 ambiguous); apply dispatched #27. Population #22 backlog dry failed on a boolean-vs-string `if:` (fixed this train), re-dispatch next | Addendum 85 ps 31 |
| **HEAL #27 RESULT (2026-09-04)** | 24 sources registered+rated, 147 figures grounded, 103 retrofits, 5 items verified; 1,477 figures not verbatim in the cited page, 359 no candidate URL, 302 capped → lane HEAL-8 (form-tolerant search, one-hop follow, thin recapture, honest terminal state) | Addendum 85 ps 32 |
| **BACKLOG #23/#24 (2026-09-04)** | 13 stale artifacts / 1,272 items; apply over 004 wrote 6 forward events, 38 themes, 932 signal edges then died on a transient `fetch failed` in db.mjs. LEGACY-2 (keyed entries recoverable) + DB-RETRY (bounded transient retry) this train; re-dispatch next | lanes `1d09f4b7`, `4c8c3b31`; this train |
| **HEAL-8 (2026-09-04)** | 1,477 "not in page": ~26.5% were the heal's own bound charging free lookups; fixed (budget split), plus numeric-tolerant search (span stays byte-exact), one-hop follow within the same institution, thin recapture, `unprovable` with sentence context. Dispatch heal dry → apply. Open lever: cross-host one-hop via DB institution lookup | lane heal8 `f08416c4`; this train |
| **LEGACY-3 + DEDUP (2026-09-04)** | Backlog apply #25 refused mint-run-001 on 2-match keys: live duplicate rows per canonical key (EP-11 violation). Resolver now disambiguates by archive state, artifact time and the verified survivor; lane DEDUP measures corpus-wide and builds `canonical-key-dedup` | this train `21aff2d7`; Addendum 85 ps 34 |
| **DEDUP (2026-09-04)** | `canonical-key-dedup` maintenance step: keep the single verified row per canonical key, archive the others reversibly; keeper rule corrected before landing (a live row carries no `archive_reason`) | this train `96ade298`; Addendum 85 ps 35; dispatch dry → apply next |
| **HEAL-9 (2026-09-04)** | Heal dry #28 ran 29m36s and was cancelled by the 30-min backstop: the time budget was armed in apply mode only; now binds both modes | this train; Addendum 85 ps 36 |
| **FIRSTPAGE + SLIM-ORDER (2026-09-04)** | /regulations first paint was 100% MODERATE (outer PostgREST `.order()` replaced the RPC's priority order); `buildWorkspaceItemsQuery` paginates with `.range()` alone, honest "Loading N…" band state; migration 303 (`ii.id ASC` on the slim RPC) APPLIED LIVE, /operations and /market allowlisted | this train; Addendum 85 ps 36 |
| **FWD-TEXT (2026-09-04)** | Upcoming Obligations text started mid-word with markdown/URL residue (667/979 rows): `clauseStart` snap, `normalizeObligationText`, content-gated `dedupeEvents`; maintenance step `forward-events-retext` for the existing rows (dispatch dry → apply next) | this train; Addendum 85 ps 36 |
| **IN-CHUNK (2026-09-04)** | Backlog applies #24/#26 died in analyze-corpus: one `.in("id", <1,317 ids>)` GET exceeded the gateway URL limit (`TypeError: fetch failed`, not transient); four id-list writers now go through `guardedUpdateByIds` at 100 ids per request | this train; Addendum 85 ps 37; re-dispatch backlog apply |
| **IN-CHUNK-2 (2026-09-04)** | Backlog apply #27 passed analyze-corpus and died in tag-proposals: the driver's hand-built `db` namespace lacked `guardedUpdateByIds` (coordinator's miss); fixed with a source-reading contract test | this train; Addendum 85 ps 38; re-dispatch backlog apply |
| **LEGACY-4 + backlog #28 (2026-09-04)** | mint-run-001 and -004 connected end to end (first time); resolver now handles `CELEX:`-prefixed keys (005) and bare uuid ids (008/009/010/015); artifact branches for runs 006-009 folded in; DEDUPE-PLUMB carries the extractor's dedupe drops into metrics | this train; Addendum 85 ps 39; backlog apply continues from 005 |
| **FWD-TEXT-2 (2026-09-04)** | Retext dry #32 showed the first normaliser unfinished (316/654 clause starts, 161 unterminated, non-idempotent); extractor `fe1-2026-09-04.2` requires a sentence start, marks fragments "…", strips labels anywhere, keeps the date cell of a table row, idempotent over all 654 rows | this train; Addendum 85 ps 39; dispatch retext dry → apply |
| **HEAL-10 (2026-09-04)** | Per-item cost cut (capture index cache; ~1,280 full-text passes on one item), per-item backstop, job timeout 35 min with measured arithmetic; STEP BRIEF-HONEST built (strip the sentence carrying an unprovable figure, Gate-A-checked, restore statement), writes only with `+strip-unprovable` ⛔ operator gate | this train; Addendum 85 ps 40; dispatch heal dry, then apply with the token on the operator's yes |
| **BRANCH-BASE (2026-09-04)** | Backlog applies #26/#29 lost their artifact stamps: a workflow-file train landing mid-run makes GitHub refuse the artifact branch; all six artifact-pushing workflows now rebase onto current master before pushing | this train; Addendum 85 ps 41; re-dispatch backlog apply |
| **BRANCH-BASE-2 (2026-09-04)** | Backlog #30 ran six artifacts clean; the rebase refused on unstaged tracked files; `--autostash` on all six workflows | this train; Addendum 85 ps 42; re-dispatch backlog apply |
| **GATE-A-TOKENS + single source (2026-09-04)** | Scanner harvested stamps, boilerplate, ordinals, citation numbers and nested spans as figures: five rule-based skips, 594 → 504 orphans live, 0 regressed; the fix had landed in the kit COPY, so the two copies were collapsed into `src/lib/agent/` with kit re-exports, F28 governs the real files | this train; Addendum 85 ps 44; heal dry → apply under the new scanner; strip still undispatched |
| **THE PLAN (2026-09-04)** | `docs/plans/complete-system-build-plan-2026-09-04.md`: section 0 definition of done (reachable, run, populated, visible, gated, documented); W0-W8; trains T37-T46. **Population stopped until T46 (full-system validation) passes.** Nine of twelve lanes landed in train 38; PERF-8, SITEMAP-3, FWD-TEXT-4 in flight (T38b) | train 37; Addendum 85 ps 48/49 |
| **WIRING AUDIT (2026-09-04)** | `docs/audits/wiring-audit-2026-09-04.md`: built yes / wired mostly / used half; intake open in front of mint; propagation inert; speed defect measured | this train; Addendum 85 ps 48 |
| **RULINGS 2026-09-04 (afternoon)** | no schedules at all (crons disarmed this train); standards bodies T4; ledger-consume flipped on the $0 path; grounding via browser lanes; R-D six ratified (`series-item-map.mjs`, six verified market_signal items) | this train |
| **POPULATION APPLIES #34-#38 (2026-09-04)** | First applies through the open gate: 1 + 120 + 103 + 89 + 104 minted verified ($0, record grade); hollow-swept rows re-hold ~35%/slice; R-D siblings blocked by M4 URL identity → RD-M4/M4b (`instrument_identity.mjs`, one predicate for apply + export) | this train; Addendum 85 ps 47; next: R-D `rows_file` re-apply, ratify, price stats; keep slicing |
| **GOV-SINGLE (2026-09-04)** | Runner copies of F28 GOVERNING_FILES drifted (mint 8 vs 10): artifacts stamped a hash the gate never computes; `scripts/harness-runs/governing-files.mjs` is now the one table for F28 and all runners; PROPOSER-8's false [CONFIRMED] corrected on record | this train `af4739cb`; Addendum 85 ps 47 |
| **CANONICAL-AUTOVERIFY (2026-09-06, folded train 51)** | Operator ruling 2026-09-06: replacement source locations are verified by the pipeline, never a human queue. The pipeline rules reachability, page class, content proof via `locateSpanInText`, and authority via host-authority; ambiguous hosts register provisional at `defaultTierForHost`'s default tier and `tier-opinions.mjs` refines from there on its next dispatch with no code change needed (it already scans the whole `sources` table with no status filter). 16 pending rows dry-verified: 6 approved / 10 rejected. First dispatch pending | this train (folded `46773631`+`751a251c`, no conflicts); next: canonical-autoverify dry then apply, then tier-opinions dry then apply |
| **TRAIN 51 / ASSEMBLE-51 (2026-09-06)** | Folded lane/canonical-autoverify-2026-09-06 (`1a9e45d1`, `a2819e8f`) onto train 50: `scripts/maintenance/canonical-autoverify.mjs` + test (automatic verification of replacement source locations, no `needs_individual_review` human outcome  -  only `approved`/`rejected`/`deferred` for a transient fetch error), a `tier-opinions.mjs` doc-comment + test noting it already covers provisional rows with no code change, a `maintenance.yml` option + step, runbook section 14 and section 38, `canonical-candidates.mjs` recommendation text + test, `CanonicalSourceReview.tsx` one-line copy change, and `docs/inventories/shared-dataset-ownership.md`. Clean cherry-pick, zero conflicts, `grep -rn '<<<<<<<'` clean. `coverage-report.json` regenerated (canonical-autoverify.mjs + test now governed surface): 849 governed files, 814 COVERED, 35 EXEMPT, 0 GAPS. Dispatch ledger: four rows appended (#66-69)  -  `review-apply-portal-links` dry then apply (301 groups, 53,718 rows rejected, read-back rows_named_in_ruling 57,469 = rows_now_live 57,469 through the chunked path, CONFIRMED by SQL: rejected 53,718, candidate 3,751, promoted 3  -  first dispatch after the readAllByIds/fetchAllByIdChunks fix from train 50); `attach-found-sources` dry then apply (176 worklist rows, 82 grounded via worklist  -  38 source_registered_and_grounded, 1 grounded_on_existing_source, 43 gate_a_written, 6 reclassified_rewritten  -  359 of the original 441 orphan figures remain). `assemble-train --fold --propose --ledger`: folded 0 (26 already folded from train 50, 0 conflicts), 0 new proposer briefs, 0 derived ledger rows. UX: `CanonicalSourceReview.tsx` changed one line of copy, no layout change [CONFIRMED by `git diff`] | this train; Addendum 86 ps 5; next: source-role-cleanup, downstream-chain's first real run, backfill-derivation-edges apply, the 16 canonical-candidate individual reviews as a ruling digest for the operator, the map smoke spec, a THETIS-MRV row, retiring `docs/design/redesign/`, UI-DELTA (action row, workspace tags with migration, provisional popover) to land as train 52; land via browser transport |
| **TRAIN 52 / ASSEMBLE-52 (2026-09-07)** | Cherry-picked `lane/canonical-autoverify-3-2026-09-07` (`e2e91409`, zero conflicts) onto train 51: `canonical-autoverify.mjs`'s `buildDeps` fetch path swapped from `browserlessFetch` (paid, no `BROWSERLESS_API_KEY` in CI by the $0 rule) to the same $0 path `provenance-heal.mjs` already uses for heal  -  `makePoliteFetch({fetchImpl:fetch})` at 1 req/s through `followUpgradingRedirects` into `captureDocument`, reshaped by new `makeCanonicalFetchCandidate` into the `{status,text,error,host,path}` contract `decideRow`/`classifyReachability`/`detectAccessWall` already depend on, unchanged. 5 new tests (200/404/network-failure/redirect-following/no-Browserless-reference); existing 52 pass unchanged. `coverage-report.json` regenerated: 849 governed files, 814 COVERED, 35 EXEMPT, 0 GAPS  -  no drift, already current. Dispatch ledger: one row appended (#70, dry, run 34069709848) recording that the first canonical-autoverify dispatch deferred all 16 pending rows on `BrowserlessError: BROWSERLESS_API_KEY not configured`, root-caused to `buildDeps`, fixed by this lane. `assemble-train --fold --propose --ledger`: folded 0 (26 already folded from train 51, 0 conflicts), 0 new proposer briefs, 0 derived ledger rows. No `.tsx`/`.css` touched this train  -  no UX compliance block required | this train; next: canonical-autoverify dry then apply (re-run against the fixed fetch path), tier-opinions dry then apply, UI train 53 |
| **TRAIN 53 / ASSEMBLE-53 (2026-09-07)** | Master went **RED** at PR #601 (train/wave52 landing): 14 F25 (`module-liveness`) allowlist entries the ASSEMBLE-47 coordinator lane re-granted to `expiry:52` on 2026-09-05 expired the instant `latestTrainWave()` read `wave52` off `origin/master`'s own git log. Per binding operator ruling (never re-grant an F25 expiry), lane `lane/f25-wave52-2026-09-07` (`de0eeffe`) disposed all 14 for real: **10 WIRED** (`admin-phrase-scan.mjs`, `defect-signature-scan.mjs`, `surface-visibility-audit.mjs` into `run-data-audit-lane.mjs`'s `AUDITS` list, SOFT/nightly; `remediate-orphan-sources.mjs` into a new `maintenance.yml` step via a `scripts/maintenance/` subprocess wrapper, runbook section 39; six `scripts/_reground/*.mjs` operator tools  -  `executor-ground`, `free-pass-run`, `id-stamp`, `lease`, `target-match-probe`, `tombstone-delete`  -  registered in `OUT-OF-REPO-BOUNDARY.md`'s Operator-CLI register, F25's `findDispatchRoots` Source 7 widened to recognize a backticked `_reground/*.mjs` row the same way it already recognizes `governance/*.mjs`/`dispatch/*.mjs`/`install-hooks.mjs`) and **4 DELETED** (`restore-overclear.mjs`  -  DEAD-HISTORICAL, one named 2026-07-16 incident already closed; `cleanup-dup-sources.mjs` and `remediate-reclassify-proposal.mjs`  -  each a one-time batch already executed against the live corpus; `stale-verified-audit.mjs`  -  superseded by the now-wired `defect-signature-scan` + `surface-visibility` audits, `skill-contract-map.mjs`'s `citingFiles` updated to drop the reference). Full per-file table: `docs/audits/f25-wave52-dispositions-2026-09-07.md`. **Zero expiry-carrying F25 allowlist entries remain**  -  verified by reading the live allowlist object in `F25-module-liveness.mjs` after the fold, not merely by the fitness runner going green. **Correction to this board's own prior "zero expiries" claims** (trains 50/51/52): those were true readings of an allowlist that was, at the time, genuinely empty, but each carried entries re-granted at a *future* wave (`expiry:52` granted while master was still on an earlier wave)  -  the claim held only until the wave counter caught up, which is exactly what happened here. "Zero expiring entries today" and "zero entries that will ever expire" are different claims; this train's fold is the first to leave the allowlist with no `expiry:` field pointing at any wave, past or future, at all. `coverage-report.json` regenerated: 849 governed files, 814 COVERED, 35 EXEMPT, 0 GAPS  -  no drift (file adds/deletes accounted for). `assemble-train --fold --propose --ledger`: folded 0 (26 already folded from train 52, 0 conflicts), 0 new proposer briefs, 0 derived ledger rows. No `.tsx`/`.css` touched this train  -  no UX compliance block required | this train; Addendum 86 ps 7; next: canonical-autoverify dry then apply, tier-opinions dry then apply, UI train 54; land via browser transport |
| **TRAIN 55 / ASSEMBLE-55 (2026-09-07)** | Folded the last three UI-delta lanes onto train 54, completing the UI system + fourteen UI lanes on the September 6 design package: workspace tags (migration 313, coordinator-applied live via Supabase MCP, file matched to the applied DDL, no redundant index, comma prose), the shared detail action row (Export/Share/Watch/+Tag, Summary \| Full brief depth switch, the ActionRow +Tag trigger wired to the SAME TagPopover DetailTagRow opens on all four detail surfaces, one popover, one lifted open state), dated per-page 375px rendering-guard exemptions (`exemptions-375.mjs`, six entries, expiry wave 58), and Notifications as its own Settings section. Three-way DetailShell/detail-surface conflicts (uitags tagRow vs uiactions askPlaceholder/SummaryDepth/SectionIndex trailing) resolved additively; uisettings2's second em-dash fix to `canonical-autoverify.mjs`/its test dropped in favor of uiactions' version (already identical, carries the regression test); DEVIATION-LOG.md unioned chronologically, one RULINGS section. `coverage-report.json` regenerated: 863 governed files, 827 COVERED, 36 EXEMPT, 0 GAPS. Dispatch ledger: three rows appended (#72 canonical-autoverify dry on the wall-defer fix, run `34081920378`; #73 canonical-autoverify apply, first apply on the $0 fetch path, 13 rows written, run `34082118998`; #74 tier-opinions dry, identical to host_class_table, apply not dispatched, run `34082329193`). `assemble-train --fold --propose --ledger`: folded 0 (27 already folded, 0 conflicts), 0 new proposer briefs, 0 derived ledger rows | this train; Addendum 86 ps 9; next: land via browser transport; mobile 390 artboards deferred to train 56 (files not yet in the repo) |
| **TRAIN 59 / FOLD-59 (2026-09-08)** | The programme's biggest fold: **eleven** lane branches, 39 cherry-picks, onto master `3230c7d2`. The compose branches were cut from train 58's tip, whose TREE equals master but which is not its ancestor, so every branch was landed by `git cherry-pick -x` (content) rather than merge (ancestry). Order: the three fix lanes (navfoot59  -  the nav footer is ONE user row per the operator ruling superseding R2; dashrow59  -  ListRow cell containment and masthead text-wrap balance; hydration59  -  render-now, BandTile hrefs, the dashboard's server-side brief-rows selection, matrix pending counts), then compose-lists, comp-06/08/11/oblig, compose-dashboard-details, compose-other, comp-admin. **Conflicts and their resolutions** are in session-log Addendum 86 postscript 14; every one was resolved by keeping BOTH sides' additive changes. Four dedup decisions: the band-card foot strip (built twice, by comp-06 and comp-08) kept comp-06's more complete implementation  -  it carries the per-band `sectionFoot` slot  -  extended with comp-08's named `transitionLabel` helper, on the canonical `--page` token, under ONE `data-audit="band-foot"` marker with compose-08's spec repointed to it; `SectionRule.coverage.npmtest.mjs` was rewritten by THREE lanes for the same red assertion, unioned; `run-audit.mjs`'s textMatch fix was already deduped in train 58 and stayed one copy; `DashboardBrief` now uses the comp-11 shared `SectionHeading`/`CardFoot` AND hydration59's server rows, with comp-11's re-introduced local caps dropped as dead code. **The fold's own defect**: `list-surface-helpers.ts` carried one literal NUL byte (the search haystack separator), so git classified the module BINARY, and a binary file has no line-level merge  -  comp-06's Window additions silently REPLACED comp-08's `countLabel` addition instead of conflicting, and only `tsc` caught it. Both restored; all three literal NULs under `src/` rewritten as escapes (runtime byte-identical, files now text). Also restored: comp-admin had removed the `compose-account` AUDIT_MOUNTS block, and the mounts interleave dropped `COMPOSE_MAP_ENTRY`/`COMPOSE_COMMUNITY_ENTRY`  -  every one of the 60 specs now names a mount that exists, verified mechanically. **Audit**: `npm run audit:design` **60 specs / 990 checks / 990 MATCH  -  0 MISMATCH, 0 NOT BUILT, 0 NOT IN SPEC** (incl. compose-01..17). Reaching it needed two harness repairs, both harness defects and not product defects: the `page-frame-1440` mount still fed `<DashboardBrief/>` the pre-HYDRATION-59 contract, so it threw and 33 checks across four specs read NOT BUILT against correct pages (the fixture now runs its `Resource[]` through the real `buildDueNextRows`/`buildChangedRows`); and compose-01's "never a fabricated 0/12" forbid was a plain substring that also matches the legitimate "10/12", reporting a passing product as NOT IN SPEC  -  `textMatch` gained a `re:<regex>` form (one shared `textFilter` for targets and forbids) and the forbid uses a boundary pattern. **C4** was failing on `origin/master` itself here and is now fixed rather than recorded again: the check exempts `.worktrees/` and `.claude/worktrees/` as ephemeral, and `work/lanes/`  -  the convention TRAIN-ASSEMBLY-RUNBOOK.md prescribes  -  postdates the rule, so all eleven lanes read as untracked; the three conventions now live in one exported predicate with the adversarial test it never had, and the stale `wt-audit` orphan row is removed. **Visual pass**: all 17 side-by-sides regenerated from the folded code and read by eye. The compositor was extracted from one lane's script (`compose-composite.mjs`) because eight pages' side-by-sides were being assembled BY HAND; the lists capture THREW on a lost `artboard` key; the dashboard capture rendered BLANK on the same stale contract; the four detail captures had no artboard beside them, injected raw instead of compiled CSS (so the mobile TopBar showed at 1440), and clipped against AppShell's 100vh frame. One real product defect found by eye: `<BandTile/>` rendered its count raw, so a four-digit band read "1135" where artboard 01 draws "1,135"  -  now through `formatNumber`, with the bandtile mount raised to a four-digit count and a spec row that goes MISMATCH when the fix is reverted. Remaining per-page differences are listed verbatim in the session-log addendum. Gates: audit 990/990; rendering guard PASS (11 fixtures, 431 checks, 7 SM + 12 UX smoke incl. hydration and masthead-balance); `tsc` clean; fitness 33/33, 0 violations; `node --test` discipline globs 121/121; closure-gate PASS on all four; `run-test-suite.sh` 5912 pass / **0 fail** / 5 skipped, exit 0 (the known kill-switch-without-creds failure did not reproduce); `runner.mjs --mode=ci --range=origin/master..HEAD` exit 0; override-check **exit 0, no drift** (first train since 50 without the C4 carry); all 17 workflow YAML parse under `yaml.safe_load`; invariant-coverage PASS (119 + 63 wired); `next build --webpack` clean with no `.env.local`; `coverage-report.json` regenerated (897 governed, 859 COVERED, 38 EXEMPT, 0 GAPS); CI npmtest glob 798/798 | this train; next: the open items in HANDOFF.md (OrganizationsTable has no artboard; the admin sub-tab row belongs inside the card head; What-changed reclassification data; research vertical/source-class facets have no data behind them; mobile specs), the Community and Settings compositions are the two least-composed pages, land via browser transport |
| **TRAIN 56 / ASSEMBLE-56 (2026-09-07)** | Mobile 390 delivered as the operator's own text (artboard `20-mobile-390.png` still not in the repo): three mobile-390 lanes cherry-picked plus ten fixes (F1-F10). 375px rendering-guard exemptions cleared on the five list pages, with the `/map` exemption confirmed to expire wave 58 same as the rest. Rule 018 allowlist covers watchlist/privacy surfaces. Onboarding's 4th "Briefing" step built from `BriefingScheduleSection` (the Settings card), landing R7's own open question. Rail prev/next wired from the list-position contract. F10's `capture-detail-mobile-screenshots.mjs` (`npm run capture:detail-mobile-screenshots`) confirmed live, not dormant: reachable via `fsi-app/package.json`'s scripts section (F25 `findDispatchRoots` Source 2, no allowlist entry, no expiry), documented as the repeatable way to regenerate `built/m-detail-*.png` in `HANDOFF.md`'s new "Regenerating the built screenshots" section, `DEVIATION-LOG.md` row already records its origin. `assemble-train --fold --propose --ledger`: folded 0 (27 already folded, 0 conflicts), 0 new proposer briefs, 0 dispatch-ledger rows appended (no new dispatches since row 81 / Maintenance #74). Gates green (fitness runner, governance/fitness/rendering/rules `node --test`, closure-gate, rendering guard, run-test-suite.sh, override-check, discipline runner, tsc, workflow YAML parse, invariant-coverage, `next build --webpack`); `coverage-report.json` unchanged (no governance-surface files touched) | this train; next: operator confirmation of `20-mobile-390.png`/updated README-HANDOFF against the built shape when it lands; overlays; community thread page; tablet 1024; ImpactMeter 8px + meta-omitted defaults awaiting operator confirmation; land via browser transport |
| **TRAIN 57 / ASSEMBLE-57 (2026-09-07)** | Folded lane/uxfix-system-2026-09-07 (7 commits), lane/uxfix-detail-2026-09-07 (6 commits) and lane/uxfix-lists-2026-09-07 (3 commits) onto master, in order  -  the three lanes dispatched against operator audit "UI DISCREPANCY REPORT 2026-09-07" + the Claude Design rulings (5.1/5.2 CLOSED, item 4.1 unblocked). One real conflict, DEVIATION-LOG.md (chronological union, both entries kept); ActionRow.tsx and ImpactMeter/Chips auto-merged clean. F25 flagged uxfix-detail's `capture-defect-fix-screenshots.mjs` as unwired (uxfix-lists' equivalent capture scripts were wired into `package.json`, this one was not)  -  fixed in the same train. Then the four coordinator-named GAPS the three lanes left, each its own commit: **G1** (SectionRule sitewide, partial)  -  uxfix-system's own mount was dashboard-only; extended to DetailShell.tsx (DetailHeader/DetailExposure/DetailTimeline/DetailSection/AtAGlanceCard/RailLegend/ImpactRailCard/InThisListStat, 8 sites), the shared `ui/Masthead` (ruling 5.2 names "masthead" among the dark-grey-gradation's homes), ListSurfaceShell's Card (covers all four ledgers), AccountCard, MapPageView's Card+CardHead, WatchlistSurface's card  -  removing each's matching border-bottom-under-title; admin cards / community table card / auth-onboarding panels NOT reached (logged as a named follow-up, `SectionRule.coverage.npmtest.mjs` locks what was fixed as a positive allowlist rather than a full repo walk). **G2**  -  uxfix-detail's item-2.3 fix removed DetailHeader's inline ask box but left detail pages with NO command bar at all; added `DetailMasthead` (DetailShell.tsx) mounting the shared `ui/Masthead` with the item title, the R4 breadcrumb format in the VOL line, and the ONE scoped CommandBar ("Ask about this regulation"/"this signal"/"this finding"/"this profile") on all four detail surfaces; DetailHeader's own `<h1>`/meta line removed (the artboard shows the title once)  -  DetailHeader keeps chips/tags/actions only, `aria-label={title}` on the landmark. F35's row-ux-coverage registry entry for the guarded title moved from DetailShell.tsx to Masthead.tsx (its real new home). **G3** (audit item A9)  -  Anton title letter-spacing was 0.04em on Masthead only and 0.02em/0.03em elsewhere; now 0.04em on DetailSection, DashboardBrief's SectionHeading, PageMasthead's `<h1>`, WatchlistSurface's card title and MapPageView's CardHead too, locked by `AntonTitleLetterSpacing.npmtest.mjs` (6 sites). **G4**  -  re-verified item 1.2 (PPWR title) with the LITERAL production title/slug (`eu-ppwr-2025-40` / "EU Packaging and Packaging Waste Regulation (PPWR)"), not a synthetic long-title fixture: a new `detail-surfaces-smoke.mjs` state plus a one-off Playwright script (`verify-ppwr-title-style.mjs`, wired as `npm run verify:ppwr-title-style`) reading the mounted `[data-guard-title]` element's COMPUTED style  -  result `fontFamily: "Anton, system-ui, sans-serif"`, `textTransform: "uppercase"`, `letterSpacing: "1.12px"` (= 0.04em at 28px). Defect does not reproduce; logged in DEVIATION-LOG.md. `coverage-report.json` regenerated: 878 governed files, 841 COVERED, 37 EXEMPT, 0 GAPS. Gates: `tsc --noEmit` clean; fitness runner 33/33, 0 violations; `node --test` over `.discipline/governance/*.test.mjs`, `.discipline/fitness/*.test.mjs`, `.discipline/fitness/functions/*.test.mjs`, `.discipline/*.test.mjs`, `.discipline/rendering/*.test.mjs`: 628/628 PASS; closure-gate `--report` PASS on all four checks; `run-test-suite.sh` 5879/5884 pass, 0 fail, 5 skipped (the pre-existing audit-finding-status informational report and the kill-switch-refusal-without-creds tests, same as every prior train); `node .discipline/runner.mjs --mode=ci --range=origin/master..HEAD` 0 fail across every commit in range; override-check `--range=origin/master..HEAD` fails only C4 (this container's own sibling `/root/work/lanes/*` worktrees not listed in `docs/inventories/worktrees.md`, the same pre-existing artefact trains 50-56 already recorded); all 18 `.github/workflows`/`.github/actions` YAML files parse under `yaml.safe_load`; `invariant-coverage.mjs` PASS (119 invariants + 63 doctrines all wired); `next build --webpack` clean (full route manifest, only the expected `SUPABASE_SERVICE_ROLE_KEY is not configured` fail-closed warnings, no `.env.local` present); the CI npmtest glob (`git ls-files '**/*.npmtest.mjs'`) 628/628 PASS after widening one pre-existing test's fixed-length slice window (`DashboardBrief.npmtest.mjs`, 1000->1600 chars  -  too small to reach its own assertion text once the item-1.1 doc comment grew, caught only by the full CI selection). 10 commits this session (3 merges + fitness-wiring fix + G1/detail-and-masthead + G1/rest-and-G3 + G4 + npmtest-slice fix + the STEP 3 docs commit), on top of the 16 folded lane commits (7+6+3)  -  26 total including this docs commit (its own tip hash is necessarily unknown to itself; see the bundle for the actual value); bundle `/tmp/train57.bundle` (origin/master..train/wave57-2026-09-07), `git bundle verify` PASS | this train; next: admin/community/auth-onboarding SectionRule sweep (named in DEVIATION-LOG.md), tighten `SectionRule.coverage.npmtest.mjs` from an allowlist to a true repo walk, land via browser transport |
| **TRAIN 58 / ASSEMBLE-58 (2026-09-07)** | The measured design-audit harness (train 56's own uxaudit-harness lane) ran a full sweep against the 44-item B-list; four fix lanes dispatched against disjoint write sets  -  fix58-tokens (theme.css `--radius-pill` root fix, band border tokens, Chips/TabRow/Absence/StatBlock/StateNote/CommandBar/Sidebar+DashboardBrief padding), fix58-lists (ListRow/ImpactMeter/MilestoneTimeline, map register, the ResearchLedger theme tag, rail-card SectionRule), fix58-detail (DetailShell padding relocation, SectionIndex pill-group redesign, AtAGlance grid, FactCard/FactBlocks, GfmSection table, operations-matrix mount), fix58-account (AccountCard/Notifications/ToggleSwitch, admin-issues-rail + onboarding-stepper specs, AdminIssuesRail rebuild)  -  folded onto train 57 in that order, `--no-ff` merges. Two lanes independently found and fixed the SAME root cause twice: `theme.css`'s self-referential `--radius-pill: var(--radius-pill);` redeclaration (fix58-tokens and fix58-lists both deleted the identical line  -  converged to one deletion at the fold, DEVIATION-LOG.md notes both) and `run-audit.mjs`'s dropped-`textMatch`-on-non-forbid-targets harness bug (fix58-tokens and fix58-detail both found and fixed it identically in `targetsOf`/`readTarget`/the `probe()` call site  -  kept fix58-detail's more complete three-site comment, verified fix58-tokens's own tests still pass against it). `results.json`/`AUDIT-2026-09-07.md`/`mounts.mjs`/`page-frame.json` conflicts resolved by taking the more complete side then regenerating; DEVIATION-LOG.md conflicts were pure chronological unions (no true overlap in content, despite git flagging them). Step 2 (the two items fix58-account could not finish, full write access this train): (a) `StatBlock` gained an additive `size="tile"` variant (11px label/16px Anton numeral/11.5px note, dc.html id="p13") and `AdminDashboard.tsx`'s 8 summary tiles now use it inside a new `.cl-admin-stat-tile` wrapper class carrying the artboard's card geometry (padding `0 14px 12px 14px`, `border:1px solid rgba(0,0,0,.12)`, the two-layer card shadow); new `admin-stat-tiles.json` spec + mount (the real `AdminDashboard`, same auth-stub technique `admin-issues-rail` already used). (b) `OrganizationsTable` has no artboard in this handoff  -  left exactly as is, logged as an open item in `HANDOFF.md` for Claude Design rather than an invented spec. Step 3, the measured gate: `npm run audit:design` reached **0 MISMATCH, 0 NOT BUILT, 0 NOT IN SPEC across all 43 specs, 786/786 MATCH** after fixing two specs that had gone stale relative to fixes their own sibling lanes already made and this train's own admin-stat-tiles addition: `market-research-rows.json` had asserted TagChip as a 999px pill (WorkspaceTagPill's value, wrong from the start  -  TagChip's real shape, already 0 MISMATCH in `chips.json`, is a 4px square) and additionally still framed the ResearchLedger theme tag as an open gap fix58-lists had already closed; `settings-section-index.json` still described SectionIndex's pre-fix58-detail bare-flex-row shape (middot separator, 10.5px, a bottom border) after fix58-detail's redesign into a bordered pill group with a scroll-spy active pill (`sectionindex.json`, already passing)  -  both corrected to measure what the shared components actually render, with the dc.html/chips.json citations recorded in each spec's own notes. `coverage-report.json` regenerated: 888 governed files, 850 COVERED, 38 EXEMPT, 0 GAPS. Gates: `tsc --noEmit` clean; fitness runner 33/33, 0 violations (also under `--mode=ci --range=origin/master..HEAD`, exit 0); rendering guard PASS (11 fixtures, 392 checks, 6 SM + 11 UX smoke specs); closure-gate `--report` PASS on all four checks; `.discipline/rendering/audit/*.test.mjs` + `.discipline/rules/*.test.mjs` + closure-gate/invariant-coverage/override-check `.test.mjs`: 157/157 PASS; `run-test-suite.sh` 5899/5894 pass, 0 fail, 5 skipped (the pre-existing audit-finding-status informational report; the previously-known "kill switch ON but no DB creds" failure did not reproduce this run), exit 0; override-check `--range=origin/master..HEAD` fails only C4 (this container's own sibling `/root/work/lanes/*` worktrees, the same pre-existing artefact every prior train recorded  -  no C3 findings); all 18 workflow/action YAML files parse under `yaml.safe_load`; `invariant-coverage.mjs` PASS (119 invariants + 63 doctrines wired); `next build --webpack` clean (full route manifest, only the expected `SUPABASE_SERVICE_ROLE_KEY is not configured` fail-closed warnings, no `.env.local`); the CI npmtest glob (748/748) and bug-class-guard's 8 named tests (45/45) both PASS. 6 commits this session (4 fold merges + the StatBlock tile feature + the two-spec audit fix), on top of 47 commits already on the branch from train 56's uxaudit-harness lane and its four uxaudit-a/b/c/d specs lanes  -  53 total ahead of origin/master; bundle `/tmp/train58.bundle` (origin/master..train/wave58-2026-09-07, 421 KB, well under the split threshold), `git bundle verify` PASS | this train; next: mobile specs still pending (artboard `20-mobile-390.png` not yet in the repo), `OrganizationsTable` needs a Claude Design artboard before any spec can be written for it, admin/onboarding full sweep beyond the four parts this train and train 57 touched (community table card, auth panels  -  named in DEVIATION-LOG.md's GAP G1), land via browser transport |
| **TRAIN 54 / ASSEMBLE-54 (2026-09-07)** | Train 53 landed as PR #602 (`c25922f8`); master then went **RED** on `closure-gate --report`'s NEVER-RUN check for `downstream-chain.yml` (that workflow's NEVER-RUN grace, 3 trains from its train-49 build, expired). Fix, evidence-first: the coordinator hand-dispatched Downstream chain #1 (`workflow_dispatch`, mode=dry, run `34078833140`)  -  the first run ever of the population/corpus → tier-opinions/derive-obligations/tag-proposals/apply-classifications hop, green in 45 s, artifact `downstream-chain-34078833140`; its `workflow_run` completion chained `propagation-drain.yml` (run `34078881801`, apply), which drained the queue for real (`propagation-run-006`: `queue_depth_before` 1,772, `events_considered`/`events_drained` 500, 0 invalidated/recomputed/errors) and superseded the family's own stale `PENDING-RUN.md` marker (its named condition  -  `harness_version sha256:ebe93513ffa2a4f9` landing in a real run artifact  -  was met, so the marker was deleted per its own instructions, clearing an F28 harness-run-integrity violation the fitness runner caught). Cherry-picked `lane/canonical-autoverify-4-2026-09-07` (`4fe10852`, zero conflicts) onto train 53: canonical-autoverify's `decideRow` now routes a CANDIDATE-side access wall (`bot_challenge`) to `deferred` with a 3-attempt cap (`reviewer_notes` round-trips the attempt count; rejects only after attempt 3) instead of an immediate reject  -  a wall is "could not verify from this network," not "the page is unfit," and rejecting permanently discarded a valid replacement since idempotency only re-reads `decision='pending'` rows; the CURRENT source's own wall handling (`checkAuthority`'s `downgrade_walled`) is unchanged. `coverage-report.json` regenerated: 849 governed files, 814 COVERED, 35 EXEMPT, 0 GAPS  -  no drift. Dispatch ledger: three rows appended (#71 canonical-autoverify dry, run `34078398318`, 16 pending → 4 approved / 11 rejected / 1 deferred, the two candidate-side wall rejections  -  BSR/SAFA, NAPA/Blue Visby  -  are exactly what lane CANONICAL-AUTOVERIFY-4 fixes, apply held until then; #72 downstream-chain #1 dry, run `34078833140`; #73 propagation-drain apply, run `34078881801`, merged with `assemble-train`'s own derived row rather than left duplicated). `assemble-train --fold --propose --ledger`: folded 1 (the `propagation/34078881801` artifact branch  -  its commit lands the drain's own record), 26 already folded, 0 conflicts, 0 new proposer briefs. `closure-gate --report` verified PASS on all four checks post-fold, **NEVER-RUN specifically confirmed PASS for `downstream-chain.yml`**  -  the point of this train. No `.tsx`/`.css` touched this train  -  no UX compliance block required | this train; Addendum 86 ps 8; next: canonical-autoverify dry then apply (re-run against the wall-defer fix), tier-opinions dry then apply, UI train 55; land via browser transport |
| **TRAIN 50 / ASSEMBLE-50 (2026-09-06)** | Two lane branches folded onto train 49: **READBACK-CHUNK** (1b9b07c7) built `readAllByIds()` in `fsi-app/scripts/lib/db.mjs`  -  the shared chunking helper that fixes the defect maintenance #57 hit live (a Supabase `in()` filter GET with 911 ids 400'd, the read-back masked by `\|\| true` so the apply itself still landed correctly; provisional 911→562 was CONFIRMED by SQL despite the read-back failure)  -  and rewired all four `review-apply-*.mjs` wrappers plus `export-census-rows.mjs`'s `fetchRowsIn` to delegate to it, each with its own updated test; **CONSOLIDATE-ATTACH** (six commits, 860033af) fixed the seed extractor to read STEP C's `orphans[]` (not `source[]`), produced the 441-row seed, ran 8 Haiku slice lanes that sourced 176 of 441 orphans with url+quote (150 this train + 26 counted earlier), reconstructed the complete 265-row unsourced set with dispositions (`not_found` / `analytical_inference` / `item_specific`), flagged 46 rows on low-authority hosts without a note and 40 quote-token mismatches rather than dropping them (the attach-found-sources apply step proves each against the fetched page), and added `consolidate-attach-worklist.mjs` + test plus the `worklist:consolidate-attach` package.json script and MAINTENANCE-RUNBOOK section 8b. Both lanes independently carried the same upstream deletions (the retired `docs/design/audit-2026-09-06/` capture set and a `docs/ops/session-log.md` trim already landed on master by an earlier train); neither lane's own commits touched those files, so both merges auto-resolved to master's side with zero conflicts, `grep -rn '<<<<<<<'` clean. `coverage-scan.mjs` regenerated (governed files touched: db.mjs, the four review-apply wrappers, export-census-rows.mjs, consolidate-attach-worklist.mjs): 848 governed files, 813 COVERED, 35 EXEMPT, 0 GAPS. `assemble-train --fold --propose --ledger`: folded 0 (26 already folded from train 49, 0 conflicts), 0 new proposer briefs, 0 derived ledger rows  -  train 49's fold already covered every stranded artifact branch. Four coordinator-confirmed dispatch-ledger rows appended (maintenance #58-59 review-apply-canonical-candidates dry/apply: 11 applied  -  6 accepts auto-resolved, 5 rejects  -  16 rows routed to individual review; maintenance #60-61 review-apply-coverage-gaps dry/apply: 91 of 91 applied across 37 groups, read-back rows_named_in_ruling 91 = rows_now_live 91). `review-apply-portal-links` (57,469 ids) deliberately NOT dispatched this train  -  held until this train's own readAllByIds fix actually lands on origin/master, since it is by far the widest read-back the defect could hit. Gates: fitness runner 32/0 (grep-confirmed 0 expiry-carrying F25/F38/F14/closure-gate allowlist entries  -  all three allowlist objects are empty `{}` in source), governance+fitness+discipline `node --test` 190/190, closure-gate `--report` PASS on all four checks (NEVER-RUN, STALE-NEXT, WRITER-READER 0 write-orphans, LANE-CONTRACT; allowlist section empty, no grace deadlines to state), `run-test-suite.sh` 5768/5763/0 (5 skipped, EXIT 0; audit-finding-status's 593-unlabeled-line report is informational and pre-existing, untouched by this train), override-check C3 clean (C4 fails only on this container's own stray `/root/work/lanes/*` worktrees not listed in `docs/inventories/worktrees.md`, pre-existing per train 48/49, unrelated to this train's content), `tsc --noEmit` clean, all 18 `.github/workflows`/`.github/actions` YAML files parse under `yaml.safe_load`, `invariant-coverage.mjs` PASS (118 invariants + 63 doctrines all wired), `next build --webpack` clean (EXIT 0, full route manifest emitted). No `.tsx`/`.css` touched this train  -  no UX compliance block required. **Continued, same train**: two further lane branches merged after the section above landed. **INCLAUSE-CLASS** (c97f4a65) audited every live `.in()` call site in `fsi-app/src` and `fsi-app/scripts` - 138 total: 26 already BOUNDED (literal or enum, no change needed), 109 BOUNDED-MARKED (individually investigated and annotated `fitness-allow: F39` with the reason the site's own row set is bounded, not corpus-scale), and 18 genuinely UNBOUNDED sites rewritten to route through one shared chunking core, `fetchAllByIdChunks` in `fsi-app/src/lib/db/paginate.mjs` - `readAllByIds` (reads) and `guardedUpdateByIds`/`guardedDelete` (writes) all delegate to it rather than each carrying its own ad hoc chunk loop (several pre-existing duplicate ad hoc helpers were removed in the same pass). A new fitness function, F39 unbounded-in-filter, scans both trees for a raw `.in()` call whose id list is not provably bounded and is registered as invariant RD-64-unbounded-in-filter; `docs/audits/in-filter-audit-2026-09-06.md` records the full 138-site table. `census-off-vertical`'s own maintenance #63 apply (dispatch ledger below) hit this exact class live - a 1,655-id post-archive read-back came back as an HTML error page - the second live instance of the defect READBACK-CHUNK fixed once for the `review-apply-*` family; F39 now guards the whole codebase against a third. **R-B** (0f052df6, ec926c5a) adds one `ON_VERTICAL` rule to `fsi-app/scripts/mint/screen-rules.mjs` matching the EU Weekly Oil Bulletin's `document_url`/`title` (mechanism: PRICES, freight-fuel-cost benchmark series), per the operator's ruling in `docs/ratifications/2026-09/RULING-2026-09-06.md` (reclassify, not archive - these six rows were minted via an R-D `rows_file` batch with no `census_worklist` row, so a `reviewed-verdicts.json` entry could never bind them; a rule-engine fix was the only enactable path). The rule takes effect on the next `screen-reconcile-records` dispatch, at which point the six oil-bulletin `market_signal` rows move from `ambiguous` to `on_vertical`; `RULING-2026-09-06.md` gained an "Enacted" line naming the commit. Both lanes merged onto `train/wave50-2026-09-06` with zero conflicts against each other and against `origin/master` (f17d7e77, PR #597, a docs-only UI-audit-evidence PR); the one real conflict was in `docs/ops/session-log.md`, resolved by keeping both postscripts in file order. `coverage-scan.mjs` regenerated again: 849 governed files, 814 COVERED, 35 EXEMPT, 0 GAPS. Four more coordinator-confirmed dispatch-ledger rows appended (maintenance #62-63 census-off-vertical dry/apply arg=archive: 1,655 `census_worklist` rows archived, CONFIRMED by SQL - 1,655 archived, 1,806 live = 1,550 on_vertical + 256 ambiguous parked - read-back failure as above; maintenance #64-65 origin-class-backfill dry/apply arg=R-E-accepted: 1,179 rows classified, 1,173 official + 6 community-corroborated, read-back present). `assemble-train --fold --propose --ledger` re-run: folded 0 (26 already folded), 0 new proposer briefs, 0 derived ledger rows. **UX**: INCLAUSE-CLASS touches `fsi-app/src/app/community/browse/page.tsx` - a two-line `fitness-allow: F39` comment added above two pre-existing bounded `.in()` calls, no visual or behavioral change [CONFIRMED by `git diff`] | this train; Addendum 86 ps 4 continued; next: review-apply-portal-links dry then apply (now unblocked), attach-found-sources dry then apply arg=scripts/_worklists/attach-found-sources-2026-09-06.json, source-role-cleanup, downstream-chain's first real run, backfill-derivation-edges apply, the 16 canonical-candidate individual reviews as a ruling digest for the operator, the map smoke spec, a THETIS-MRV row, retiring `docs/design/redesign/`; land via browser transport |
| **TRAIN 49 / ASSEMBLE-49 (2026-09-06)** | Nine lane branches folded onto train 48 (rulings, oneshots, chain, mintflywheel, w4data, fueleu, scopereader, uxfix, detailwf), each with its own section 0 verdict: **MINT-FLYWHEEL** found rule 17's tandem already built by lane TANDEM and refuted the audit's own finding, documented rather than re-built; **CHAIN** built `downstream-chain.yml` (the population-turn/corpus-turn → four-step chain → propagation-drain hop, closing the loop MAINTENANCE-RUNBOOK section 1 first named); **SCOPE-READER** built the first real `entity_scope` reader plus human-readable corridor labels everywhere, and both `entity_scope` allowlist entries (F14, closure-gate WRITER-READER) are gone; **ONESHOTS** deleted 3 dead scripts and wired 5 as real dispatch roots, zero expiry-carrying F25 entries remain anywhere in the gate; **W4-DAG** wired producer-authored `market_series` edges (3 producers + a third `backfill-derivation-edges` delegate), 12 edges expected on the first live backfill; **FUELEU** found the statutory rows-file needs per-ship MRV figures the regulation itself does not publish, so it built the validator gate and the six-period Annex I/IV constants rather than fabricate rows, `statutory_computations` stays at 0 until a THETIS-MRV row is sourced; **RULINGS** transcribed the operator's ruling on all four review queues plus R-A/R-B/R-E, and found R-B needs a `screen-rules.mjs` code change (no rows_file row for the reclassified EU Weekly Oil Bulletin market_signal rows); **UX-FIX** fixed P1-P5 live UX defects and built P6's mode-tag-coverage audit as a named editorial gap (no deterministic classifier exists); **DETAIL-WATERFALL** fixed the Summary/Full/Complete three-state toggle and re-measured the operator's reported 44-fetch/34s waterfall against production, not reproduced in a foreground capture (7-8 fetches, all under 2.2s), narrowed to hypotheses, not refuted. Conflicts, all resolved by union of intent: `.github/workflows/maintenance.yml` auto-merged clean (chain + oneshots steps and options both survive); `docs/runbooks/MAINTENANCE-RUNBOOK.md`, both chain and oneshots wrote a section 33, kept chain's (already cited by number in PROPAGATION-DRAIN-RUNBOOK.md), renumbered oneshots' four sections to section 34-section 37; `.github/workflows/propagation-drain.yml` auto-merged clean (chain's `workflow_run` trigger on `["Data producers", "Downstream chain"]` and fueleu's validator step both survive); `scripts/verify/run-data-audit-lane.mjs`, uxfix's `mode-tag-coverage` and oneshots' `holdings-audit` both added an AUDITS entry at the same spot, kept both; `docs/audits/perf-load-times-2026-09-03.md`, both uxfix and detailwf appended a section 16, kept uxfix's section 16, renumbered detailwf's section and all eight `section 16.x` internal cross-references to section 17/section 17.1-section 17.8; `src/app/regulations/[slug]/page.tsx` no conflict against lane/scopereader (merged earlier), `CorridorsAppliedStrip` survives. `grep -rn '<<<<<<<'` clean. `coverage-scan.mjs` regenerated: 847 governed files, 812 COVERED, 35 EXEMPT, 0 GAPS. `assemble-train --fold --propose --ledger`: folded 0 (26 already folded, 0 conflicts), 0 new proposer briefs, 0 derived ledger rows. Migration 312 (`entities.display_name`) APPLIED LIVE 2026-09-06 by the coordinator (`schema_migrations` 20260906154322); lane w71f14's own unmerged file numbered 312 (`312_drop_entity_scope.sql`) stays superseded by the operator's ruling and must never be applied. Two coordinator-confirmed dispatch-ledger rows appended (maintenance #54 w1-dispositions, #55 provenance-heal, the latter found the attach-found-sources seed worklist is not reproducible from its own summary.json, a defect to diagnose). Gates: fitness runner 0 violations (grep-confirmed 0 expiry-carrying F25/F38/F14/closure-gate entries), governance+fitness+discipline node --test all green, closure-gate --report PASS (regen-quarantined/acquire-primaries/refetch-capped/source-role-cleanup and workflow downstream-chain within their NEVER-RUN grace), run-test-suite.sh green with and without node_modules, override-check C3 clean, tsc --noEmit clean, every workflow/action YAML parses, invariant-coverage PASS, rendering guard PASS, `next build --webpack` clean | this train; Addendum 86 ps 3; next: the ruling applies (review-apply-*, census-off-vertical archive, origin-class R-E), downstream-chain's first real run, backfill-derivation-edges apply, attach-found-sources seed diagnosis, R-B screen-rules fix, the batched viewer-state route, the map smoke spec, a THETIS-MRV row; land via browser transport |
| **TRAIN 48 / ASSEMBLE-48 (2026-09-05)** | Folded lane/docsfold, lane/w71a (`e5315894`), lane/w71c (`889804d8`), lane/w71d (`5ec4d3a0`) onto train 47; lane/w71f14 (drops `entity_scope`) deliberately NOT merged, held for an operator ruling: spec 08 section 1.2 designs the table and it now holds 8 live rows migration 312 would drop. Conflicts (F25-module-liveness.mjs, skill-contract-map.mjs, session-log.md) resolved by union of intent, `grep -rn '<<<<<<<'` clean. `assemble-train --fold --propose --ledger` folded 9 stranded artifact branches (ledger-consume ×8, propagation ×1; 17 already folded, 0 conflicts) and wrote proposer briefs; both were done directly this train rather than dispatched; ledger-consume's pass (reading runs 001-007) confirmed the stationary-loop defect (run-002 pass's finding 1) repeated across two more verdict-batch generations before landing fixed in run-007 (`fetched:0` for 386 matched candidates); propagation's pass (reading runs 001-005) found and fixed a live CAP-1000 defect: `queue_depth_before` read a bare unranged `.select().length`, flat at 1000 across runs 003/004/005 while the true pending queue held 2,272-2,778, fixed via `paginate.mjs`'s `exactCount()`, a fifth CAP-1000 instance in a file F38 does not mechanically scan. Eight coordinator-confirmed dispatch rows landed in `docs/ops/dispatch-ledger.jsonl` (maintenance #49-53, producers #24, propagation-drain, ledger-consume #14), replacing assemble-train's own derived rows for the two folded artifact branches. Those eight rows gave NEVER-RUN real dispatch evidence for the last nine open `maintenance:*` entries; `NEVER_RUN_ALLOWLIST` emptied (0 stale entries re-granted) and `gatherNeverRunTargets()` fixed so a `maintenance:all` dispatch row now counts as evidence for every step it dry-ran, the mechanism that discharged them rather than a tenth re-grant. `closure-gate --report`: NEVER-RUN PASS, STALE-NEXT PASS, WRITER-READER PASS (`entity_scope` is the sole remaining expiry-carrying entry anywhere in the gate, held for the W71-F14 ruling), LANE-CONTRACT PASS, **W7.1-CLOSE's own two closing conditions both met**. Gates: fitness 32/0, governance+fitness node --test (268 tests incl. closure-gate LIVE), run-test-suite.sh 5684/5679/0 (with node_modules) and 5684/5677/0 (without, +2 unpdf self-skips), override-check C3 clean (C4 fails on this container's own stray worktrees, pre-existing, unrelated to this train's content), tsc clean, invariant-coverage PASS, rendering guard PASS (one `.tsx` touched, see UX compliance below), `next build --webpack` clean | this train; Addendum 86 ps 2; next: operator ruling on migration 312/`entity_scope` (W71-F14); land via browser transport |
| **TRAIN 47 / ASSEMBLE (2026-09-05)** | Eleven lane branches folded onto train 46 (assembly W1.6, kitbackfill W2.3/2.4, rulingsexec W2.2, attachsrc W3.1/3.3, corridors W4.2, chips W3.4, spec09a+spec09b W5.1, w71wire W7.1, notices W4.3, ledgerchain2 W1.4). Four lanes independently numbered a migration 308; renumbered on apply order  -  308 RULINGS-EXEC `census_worklist_archive_columns`, 309 ATTACH-SOURCES `source_tier_opinions_host_class_table`, 310 CHIPS `listing_rpcs_item_grade`, 311 SPEC09-B `spec09_org_scope_and_pool_drop` (drops `carrier_compliance_pools`)  -  all four **written, not applied**; every self-reference (migration header, migrations.md, shared-dataset-ownership.md, closure-gate.mjs, tests, SOURCES.md) updated to the new number. Cross-lane registrations: `notices-rail-smoke.mjs` wired into `ux-smoke-specs.mjs` + F35 `ROW_COMPONENTS` (RecalculationNotice.tsx), rendering guard green (9 UX specs); `docs/INDEX.md` gained the TRAIN-ASSEMBLY-RUNBOOK line; F28 clean, no stale PENDING-RUN/LAST-PROPOSER-PASS marker. Coordinator follow-up wired in the same train: four spec09 customer-CSV producers (surcharge-audit/dqi/auxiliary-energy/indexation) got their own `maintenance.yml` steps (`spec09-*-csv`, `arg`="<csv-path>,<org-id>")  -  the exact gap their own F25 entries named. F14 caught a real write-orphan from lane CORRIDORS-STATUTORY (`entity_scope`, zero readers); allowlisted with a named future reader, not silently wired shut. Ratchet event, unrelated to any of the eleven lanes: ~49 F25/F38 `expiry:46` legacy entries (pre-existing on origin/master, confirmed byte-identical before this train's first cherry-pick) flipped EXPIRED the instant wave46 itself landed; re-granted to wave52 with a named, undischarged-backlog disclosure rather than silently resolved  -  a dedicated lane must read and disposition each. Gates: fitness 32/0, governance+fitness node --test, closure-gate 4/4, run-test-suite.sh (with and without node_modules symlink), override-check C3 clean, tsc clean, `next build --webpack` clean | this train; Addendum 85 ps 58; next: coordinator applies 308-311 live (DDL before code), then the maintenance/producer dispatches each folded lane's own report named |
| **CLOSED  -  W7.1-CLOSE (2026-09-05, closed train 48)** | The discipline engine's own closure gate was red (NEVER-RUN 10, STALE-NEXT 7 since train 47) with F25/F38/F14 showing 0 violations only because every expired entry had been re-granted to wave52 by ASSEMBLE-47 (the third re-grant in a row, 41 → 43 → 46 → 52  -  a disclosed non-fix, not a hidden one). Split into four lanes on disjoint write sets: **W71-A**, **W71-C**, **W71-D** (closure-gate docs half  -  the 7 STALE-NEXT board rows + `workflow:inspect-oil-bulletin.yml`  -  plus two duplicate-module questions, both REFUTED on inspection), **W71-F14**. Both closing conditions met in train 48: zero F25/F38/F14/closure-gate allowlist entries carry an expiry except `entity_scope` (WRITER-READER, expiry wave52, explicitly held for W71-F14's operator ruling, not an oversight); NEVER-RUN reached 0 once ASSEMBLE-48's eight dispatch-ledger rows landed and `gatherNeverRunTargets()` was fixed to credit a `maintenance:all` dispatch as evidence for every step it covered | train 48; Addendum 85 ps 59, Addendum 86 ps 2; next: none, superseded by the entity_scope/migration-312 ruling W71-F14 still owns |
| **TRAIN 46a / CAP-1000 (2026-09-05)** | FE-DEDUP cleanup ran (Maintenance #47: forward events 1,152 → 821, obligations 1,149 → 818), migration 307 applied. Live measurement found PostgREST's 1,000-row cap: only the first 1,000 slugs prerendered (index ≥1,000 MISS at 580-710 ms), the register masthead "60 of 1000", filters over-fetching 2,000 into JS. Fixed at five sites through one `fetchAllRows`/`exactCount` helper, filters in SQL with exact counts, F38 guards new capped reads; every apply-mode job now flushes the public caches (only mint did before; the static register served pre-cleanup data). Measurement method corrected: a background Chrome tab throttles timers and never fires IntersectionObserver; the sentinel works when the tab is visible. Sweeps #20/#21 inside budget (403 hosts remain) | this train; Addendum 85 ps 57; next: re-measure (every slug PRERENDER, register count exact, flush after #22), ledger-consume plan with verdicts 001+002, LEDGER-CHAIN-2, T46 validation |
| **TRAIN 45 / PERF-13 + FE-DEDUP + REG-GRAIN (2026-09-05)** | Live measurement of train 44 (TTFB 21 ms, 277 KB, 12 DOM rows, no #418) found: un-rendered detail slugs cost 760-950 ms with a frozen screen (on-demand ● + no loading boundary on client nav), duplicate upcoming fetch, a scroll sentinel that never fired (AppShell never overflowed; observer recreated per toggle), a permanent "Loading 169" band, and every register row twice. Fixed: build-time slug enumeration (1,431 pages, every click an edge hit), 6 h / 1 h revalidate behind tag invalidation, sentinel once per viewport (Playwright), honest band state with per-band Load more, one upcoming fetch; forward-events extractor dedupe floor bug (`fe1-2026-09-04.6`), retext step deletes the 296 section-backed twins through the guarded delete, migration 307 tightens the key (apply after the cleanup); register rows carry the obligation text. Verdicts batch 002 (356, $0). Sweep #18 first budgeted run (35/40 hosts, 15,371 candidates); proposer: 30 hosts per run | this train; Addendum 85 ps 56; next: retext apply dispatch, apply 307, re-measure, sweep #20 at 30, ledger-consume plan with 001+002, T46 |
| **TRAIN 44 / PERF-10 + PERF-12 + LEDGER-WALLS + SWEEP-BUDGET (2026-09-04)** | The four index pages build ○ (static, 1 min revalidate) and the four detail routes ● (SSG) instead of ƒ: org-independent `*_public` RPCs (migration 306, applied before this landed) read under one shared `unstable_cache` tag, per-org overrides merged client-side; rows page by keyset cursor (30/page, `/api/listings/cursor`, cacheable, no org hop) and render through TanStack Virtual (713-row band → 12 DOM rows); the 5,000-row remainder fetch, both fail-soft RPC ladders, the org-scoped cursor migration and the unused JWT-claim hook migration deleted; React #418 root cause = unpinned `toLocaleString` (one shared formatter, repo-wide sweep, grep test); "No workspace yet" false state = three-valued orgId; obligations register SSR-seeded again. LEDGER-WALLS: shared access-wall detector, Federal Register/eCFR API transport, EUR-Lex HTML endpoint (308 of 338 exported texts were walls; verdict batch pruned to 30). SWEEP-BUDGET: 20 s fetch timeout + 1500 s walk budget after sweeps #16/#17 died at the job timeout. Ledger-consume proposer pass: chained plans are a stationary loop (fix proposed, not built) | this train; Addendum 85 ps 55; next: measure live (bytes, rows, click-to-paint), sweep #18 (40 hosts), ledger-consume plan with verdicts-001, export batch 002 through the new transports, LEDGER-CHAIN-2, ADR-027 D5, T46 |
| **TRAIN 43 / PERF-11 + PERF-ARCH (2026-09-04)** | Obligations register shipped the whole table (500 of 1,141 rows, 297 KB) on every /regulations load → 60-row first page + Load more, filters re-fetch via `/api/obligations/register`; ledger domain leak fixed (39/60 rows were regulations), migration 305 domain param (fail-soft until applied); Server-Timing instrumentation; F37 perf budget in CI; ADR-027 the standard architecture (Cache Components, PostgREST range + TanStack Query/Virtual, org id as JWT claim); waterfall audit on record; closure-gate allowlists re-granted to T46 | this train; Addendum 85 ps 54; next: apply 305, PERF-12 (cursor rows + virtualization replacing the 5,000-row remainder fetch), PERF-10 landing |
| **TRAIN 42 (2026-09-04)** | LEDGER-TEXT (one `htmlToText`, four copies retired; fetcher returns text, PDFs extracted); first automatic `workflow_run` chain (Source sweep → ledger-consume plan) on record; corpus-turn apply #9 (tickets 1,709 → 915, edges 21,973); sweep all-hosts #14/#15 (1,360 candidates, coverage columns live); proposer passes 13-15; three markers discharged | this train; Addendum 85 ps 53; next: re-run the candidate export, Haiku classification, sweep #16+ |
| **SPEED (operator, 2026-09-04 18:05-18:40)** | "I can't show this app to anyone until this is resolved"; no spend-based fixes; one screen of rows; no blank pages; do what the standard stack does. Lanes: PERF-10 (no function invocation for public content), PERF-11 (payload/cursor), PERF-ARCH (Server-Timing waterfall, ADR-027, F37 budget gate) | trains 40-44 landed the architecture (ADR-027); live measurement after train 44 decides whether the thread closes; Addendum 85 ps 52-55 |
| **TRAIN 41 / REGION (2026-09-04)** | Functions moved iad1 → sfo1 (Supabase is us-west-1); cold starts identified as the 10 s event (Fluid Compute already enabled, no provisioned-concurrency control on Pro); Function CPU Performance tier awaiting the operator's decision | this train; Addendum 85 ps 52; next: measure /api/version and /regulations after deploy; PERF-11 payload; PERF-10 cacheComponents |
| **TRAIN 40 / PERF-9 (2026-09-04)** | ADR-026 (public content cached by tag, viewer state client-side), RootLayout no longer calls headers(); item click loses one sequential round trip (two-stage identity resolver at five call sites); four post-render per-user calls → one `/api/workspace/bootstrap`. Route table still 130/130 dynamic: Cache Components (Next 16) and the org-parameterised RPC split are the next lane (PERF-10); the instant-click law is not yet met | this train; Addendum 85 ps 51; next: measure live in Chrome, PERF-10 |
| **TRAIN 39 (2026-09-04)** | T38b + T39 proving dispatches: SITEMAP-3 (all-hosts mode, migration 304 written), FWD-TEXT-4 (fe1-2026-09-04.5), PERF-8 (React #418 closed, SSR payload trim, F36), LEDGER-EXPORT (`--with-text` export on the runtime's one fetcher, artifact branch), TICKET-CORPUS (594 archived-item tickets closed by label, not consumed as work); dispatches recorded in `docs/ops/dispatch-ledger.jsonl`: institution-canonicalize dry+apply (T4 live), corpus-turn #8 dry, propagation drain #4 dry + #5 apply (edges 6→24, values 6→22, 500 events drained: the propagation loop's first movement); `POPULATION_PAUSED=true` set | this train; Addendum 85 ps 50; next: migration 304, source-sweep all_hosts, ledger-consume export_candidates, corpus-turn apply, Haiku classification; PERF-9 → train 40 |
| **TRAIN 38 (2026-09-04)** | Nine plan lanes in one train: ledger-consume armed on the $0 verdict path (LEDGER-ZERO); four review-apply steps wired, seeders deleted (REVIEW-WIRE); corpus-turn selects from `corpus_turn_requests` (TURNREQ); `standards_body` T4 class + Part C apply (T4-OVERRIDE); `workflow_run` chaining + `POPULATION_PAUSED` (CHAIN); closure gate in CI + dispatch ledger (CLOSURE-GATE); F25 widened + orphan census (F25-WIDE); register deletes 3/10/12/18, 16 dead exports, shims gone (DEAD-EXEC); DAG authorship at write time, `backfill-derivation-edges`, first statutory writer (DAG-AUTHOR). Plan section 3 table corrected: T39/T40/T42 code landed here | this train; Addendum 85 ps 49; next: T39 proving dispatches (see plan section 3), migration 304 after T38b |
| **LANES INTERRUPTED 15:32 (2026-09-04)** | PERF-8, SITEMAP-3, FWD-TEXT-4 stopped mid-work; relaunched 16:45 on their worktrees with a resume preamble; all three landed in train 39 | train 39; Addendum 85 ps 49/50 |
| **FE-SLOT-2 (2026-09-04)** | One corpus reader (three mirrors collapsed); due_date slot claims get captured-source context, 61 → 90 events on 118 calendar-dated slots; 638 relative deadlines refused by name; pool read scoped to 0.54% of the table | this train `4135c48e`+`6021fff4`; Addendum 85 ps 47 |
| **HEAL APPLY #42 (2026-09-04)** | 86 processed, 3 verified; cited pages checked: 1,720 tokens absent, 30 grounded, 443 unresolved on 76 items; $0 grounding exhausted → ⛔ operator: search-backed grounding (cost) or stay quarantined | Addendum 85 ps 47 |
| **BACKLOG CLEARED (2026-09-04)** | Apply #33 connected mint-run-017..022 (934 items, 176 events); `--check-gate` over master + stamps: every minted slice carries its section 9 outcomes; PROPOSER-7 names run-028; 190 `slot_date_unclassified` skips → FE-SLOT-2 (638/756 due_date slots are relative deadlines, correct refusals; 89 calendar-dated spans lack the deontic inside the mint window) | this train; Addendum 85 ps 46; population gate OPEN: R-D six `rows_file` apply, then limit 200 |
| **RETEXT APPLY #38 (2026-09-04)** | 921 targets, 173 collision rows deleted (snapshotted), 748 rewritten, read-back 748/748; live 0 collisions, 0 residue classes; residual: record-facts slot template as obligation_text (122 rows / 90 items) → FWD-TEXT-3 `4a57d4c5` (unwrap to the quoted passage, marker `4187cd5f5f26d005`) | this train; Addendum 85 ps 46; retext dry → apply under fe1-2026-09-04.3 |
| **HEAL DRY #39 + META-8 (2026-09-04)** | Under the fixed scanner: unprovable 527 → 453, would-ground 1,247; brief-honest strip REFUSED on evidence (would delete ReFuelEU 0.7% PtL sub-target, FY2024, Melbourne 53%); heal apply next. meta-harness-run-008 discharges the marker: 1 of 17 defects gate-caught this wave | this train `57321f8a`; Addendum 85 ps 46 |
| **BACKLOG #31/#32 folded (2026-09-04)** | Stamps on mint-run-005/006/011/012/013/014/016 and forward-events-run-010..022 landed; six twin pairs agree exactly (deterministic extractor); `dedupe_dropped` plumbed through; forward-events marker discharged, PROPOSER-6 names run-022 | this train; Addendum 85 ps 45; next backlog apply AFTER landing (max 6) |
| **RETEXT-COLLIDE (2026-09-04)** | Retext apply #35 hit `uq_item_forward_events_dedupe`: derived duplicates the index forbids are deleted (snapshotted, restorable) and the survivor rewritten; whole-table post-rewrite key check, 26 tests | this train `bf31de44`; Addendum 85 ps 43; dispatch retext dry → apply |
| **CLOSED (structural)** | shared write-sequence module: landed as `src/lib/intake/write-item.ts` (WSEQ, Addendum 85 line ~8107); `apply-mint-batch.mjs` calls it whole, `canonical-pipeline.ts` for gate-A and citation rows | Addendum 85 ps 46 |
| **SUPERSEDED (lane W71-D 2026-09-05)** | Overtaken by trains 1-47 (rows above): corpus-turn, population slices, ledger-consume, change-detection and the producer dry/apply pairs this row named all ran repeatedly (see the TRAIN NN rows above, e.g. T47/T46a). Current dispatch-next line lives in the latest session-log addendum, not here | see TRAIN rows above; docs/ops/session-log.md latest addendum |
| **OPERATOR** | `EIA_API_KEY` GitHub secret; "Allow GitHub Actions to create and approve pull requests" | Addendum 84 |
| **NOT BUILDABLE (no data source)** | corridor rate board, lead-time chart, peer cohort, capacity panel | SURF report |
...
| **FIXED  -  runs no longer fail on the refused PR** | `deliver-artifact-branch.sh`: PR when permitted; otherwise the branch is filed on one tracking issue, warning annotation, green run; any other PR error still fails | Addendum 83 |
...
| **RAN  -  source-sweep-run-005 (apply)** | register answered: 7 acts, 2 duplicate-edition days, 27 s (1 req/s), marker discharged; 7 candidates re-parented to the existing "EUR-Lex" portal source `260089a9` (correction: not a new row) | Addendum 82 postscript 4 |
| **SUPERSEDED (lane W71-D 2026-09-05)** | WO-26 stamp landed (Wave, row 1764, "POPULATION  -  runs #6-#8"); tag ratification, mint runs and the ledger-consume hop were all executed and superseded again by trains 1-47 above. Current dispatch-next line lives in the latest session-log addendum, not here | row 1764; TRAIN rows above; docs/ops/session-log.md latest addendum |

## Completion lanes + system review  -  LANDED as PR #507 (2026-09-01)

| State | Item | Evidence |
|---|---|---|
| **REVIEW** | `docs/audits/system-review-2026-09-01.md`: intent vs actual scorecard; 322 live items vs 2,561 sources / 21,609 census / 3,661 would_mint; 491 unstamped WO-26 archives; root cause of all blockage = no runtime layer; interface verdict; competitive field | Addendum 81 |
| **BUILT  -  runtime layer (RT)** | `corpus-turn.yml` (dispatch + push `turn/**`), `source-sweep.yml` (dispatch), `scripts/turns/**`, source-sweep family (first run pending, hash-pinned marker) | Addendum 81; `CORPUS-TURN-RUNBOOK.md` |
| **BUILT  -  record-grade tier (POP)** | migration 278 (**applied**), record mint path, validator `grade` profile, `RecordGradeBadge`, WO-26 stamp script, population plan for 3,661 rows | `docs/plans/record-tier-population-plan-2026-09-01.md` |
| **BUILT  -  event-driven (EV)** | migration 277 (**applied**): `corpus_turn_requests` + trigger; admin "Run intake now" + "Request corpus turn" | Addendum 81 |
| **BUILT  -  change detection (CD + intake-updates)** | fingerprint → queue → in-process reconcile → change-sweep → staged `update_item` → run-intake drains + $0 re-verify; `ChangedSinceStrip` on home; migration 279 (**applied**, intelligence_changes RLS) | Addendum 81 |
...
## W9  -  Brief chain: PLANNED, awaiting operator go (2026-09-11)
