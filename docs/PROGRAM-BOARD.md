...
## System completion train — BUILT, GATED, LANDING (2026-09-02)
...
| **NEXT (coordinator)** | land via browser path → apply 281–287 live in order → `backfill-entities --apply` → `seed-derived-values --apply` → one dispatch per new workflow, artifact read against the live table before the next | Addendum 84 |
