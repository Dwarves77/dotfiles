...
| **BUILT — spec 08 (decision propagation)** | ADR-024; migrations 282–287 (entity spine, progressive FKs, outbox + triggers, DAG + `derived_values` + admissibility view, statutory/estimate isolation, aggregate safeguards §5.2 a–d + dominance + forward-looking); `admissibleFor`/`FLOOR`; `runPropagationDrain`; F30/F31/F32 | Addendum 84 |
...
| **PERF (2026-09-03)** | Measured, fixed, re-measured [audit `perf-load-times-2026-09-03.md` §1–§9]: skeleton on every click (<2 ms); detail server render market 1,905→432 ms, operations 1,262→329 ms, research 1,597→715 ms, regulations 1,257→647 ms; one cached parallel loader for the four detail pages, index ledgers cached per org, middleware `getClaims()`, cache flush after mint apply. OPEN: a self-healing 503 on router RSC navigations, never in a function log, not reproducible by `fetch()` with RSC headers; hypothesis deploy-transition skew; settle with a quiet-window measurement | Addendum 85 ps 1, 3, 4 |
...
| **503 CLOSED (2026-09-03)** | No production 503 on item navigation: the extension's network reader mislabels completed RSC responses (in-page `responseStatus` 200 and Vercel's edge log 200 for the identical request). Audit §11; four earlier sections retained as the record of an instrument artefact | Addendum 85 ps 6 |
...
| **TANDEM (2026-09-04)** | The flywheel is a workflow step now: `run-population-flywheel.mjs` runs §8/§9 after every apply (batch-scoped discovery, forward events, recluster, obligations, tags, ratification, outcomes into the mint artifact) and THE GATE refuses a new apply while the last one is unconnected. Runs #15–#20 (~650 items) still owe their pass: the first dispatch under the gate must be told their ids | lane tandem `4afbf7a8`; this train |
...
| **THE PLAN (2026-09-04)** | `docs/plans/complete-system-build-plan-2026-09-04.md`: §0 definition of done (reachable, run, populated, visible, gated, documented); W0–W8; trains T37–T46. **Population stopped until T46 (full-system validation) passes.** Nine of twelve lanes landed in train 38; PERF-8, SITEMAP-3, FWD-TEXT-4 in flight (T38b) | train 37; Addendum 85 ps 48/49 |
...
| **TRAIN 38 (2026-09-04)** | Nine plan lanes in one train: ledger-consume armed on the $0 verdict path (LEDGER-ZERO); four review-apply steps wired, seeders deleted (REVIEW-WIRE); corpus-turn selects from `corpus_turn_requests` (TURNREQ); `standards_body` T4 class + Part C apply (T4-OVERRIDE); `workflow_run` chaining + `POPULATION_PAUSED` (CHAIN); closure gate in CI + dispatch ledger (CLOSURE-GATE); F25 widened + orphan census (F25-WIDE); register deletes 3/10/12/18, 16 dead exports, shims gone (DEAD-EXEC); DAG authorship at write time, `backfill-derivation-edges`, first statutory writer (DAG-AUTHOR). Plan §3 table corrected: T39/T40/T42 code landed here | this train; Addendum 85 ps 49; next: T39 proving dispatches (see plan §3), migration 304 after T38b |
