...
| **NEXT (coordinator)** | dispatch population-turn dry at the new hash (mint-run-009), read first-pass rate + Cellar evidence in the held file → apply (limit 50) → verify `intelligence_items` item_grade=record → ledger-consume plan → change-detection dry → FR + feed dry → ecb-fx and lc_lci_lev dry/apply; read each artifact against the live table | Addendum 84 postscript 7 |
