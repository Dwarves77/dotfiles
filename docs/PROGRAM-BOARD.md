...
| **NEXT (coordinator)** | land the post-landing train (drain workflow opt-in backfill/seed inputs) → dispatch propagation-drain dry then apply with backfill+seed → ledger-consume plan → population-turn dry/apply → change-detection dry → FR + feed dry → ecb-fx and lc_lci_lev dry/apply; read each artifact against the live table | Addendum 84 postscript 1 |
