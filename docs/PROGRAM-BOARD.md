...
| **⚠ OPEN — how did EPA seed?** | EPA has 2 live rows via this same broken module; an `--apply` should have aborted identically. Either a different route or a different run condition. Does not block DESNZ; the fix corrects both. Flagged, not guessed at | Addendum 60 |
