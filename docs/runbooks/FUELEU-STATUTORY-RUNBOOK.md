...
(`docs/audits/plan-completion-audit-2026-09-05/W3-W4-sourcing-propagation.md` section "section 4, FuelEU Annex IV first
statutory writer").
...
3. **A published, numeric worked example from an authoritative body** , searched for a Commission/EMSA/
   ...
   examples in its own section 1.4 (pages 30-42) , `WebFetch` truncated the document before reaching the numeric
...
## Statutory rows: upload, dry, apply (lane M7a, 2026-09-20)

The CLI/fixture path above is still the only path that has ever produced a real row (0 to date). Plan
row M7's data-layer half asked for a product-reachable replacement: `POST /api/admin/statutory-rows`
(`fsi-app/src/app/api/admin/statutory-rows/route.ts`), gated with the same platform-admin gate every
other `/api/admin/**` route uses (`requireAdminRoute`), and an admin panel (`StatutoryRowsUpload.tsx`,
mounted on `/admin` under Ingest > Statutory rows).

**ONE PATH, not a fork.** The route's orchestration (`./logic.mjs`) imports
`validateRowsFile` from `scripts/propagation/validate-statutory-rows-file.mjs` (the SAME module
`propagation-drain.yml`'s pre-flight gate calls) and `parseRow`/`writeOneRow` from
`scripts/propagation/write-statutory.mjs` (the SAME functions the CLI `--apply` path calls). Neither
row validation nor the write itself is reimplemented anywhere in `src/`; `logic.test.mjs` asserts the
import specifiers so a future edit that forks the logic fails the test.

**Flow.**
1. Operator (platform admin) pastes or picks a reviewed rows-file JSON document (same `_schema` the
   fixture documents: `{ rows: [ { shipKey, targetYear, ghgIntensityActual, energyUsedMJ,
   consecutiveDeficitYears }, ... ] }`, every `StatutoryInput` carrying a real `citation` and a
   structured `source` block).
2. **Run dry** (`POST .../statutory-rows?mode=dry`, the default): `validateRowsFile` runs first , a
   FIXTURE/SYNTHETIC/PLACEHOLDER marker or a missing/unrated source block refuses the WHOLE body
   (HTTP 422, the validator's violation strings verbatim, nothing parsed or written). A clean file is
   parsed row by row and `writeOneRow` previews each row's outcome (would-write / already-computed /
   refused-inadmissible) without writing.
3. **Apply** unlocks in the panel only after a clean dry run of the exact same file text (any edit
   re-locks it , the panel's own guard, not a server-side state). `POST .../statutory-rows?mode=apply`
   re-validates and writes each admissible row through the same `writeOneRow` path the CLI uses.
4. Response: per-row outcome array plus counts (`written`, `wouldWrite`, `skippedAlready`, `refused`,
   `errored`); a validator refusal returns its violations verbatim and writes nothing.

**What did not change.** `write-statutory.mjs`'s row semantics, `SUPPORTED_TARGET_YEARS` (2025 only),
and `validate-statutory-rows-file.mjs`'s rules are untouched , this lane only added a second caller of
the same functions. The CLI path (`propagation-drain.yml`'s opt-in `backfill_and_statutory` step) is
unchanged and still reads its own fixture-path file independently; the two callers do not share state.
Preparing real, reviewed FuelEU rows is still a separate, later phase (6.3) , this upload path makes
that phase's output reachable from the product once it exists; it does not itself produce reviewed rows.
