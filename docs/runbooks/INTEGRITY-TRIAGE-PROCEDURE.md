...
- [INTEGRITY-TRIAGE-REPORT](../audits/INTEGRITY-TRIAGE-REPORT.md) — This runbook's script emits docs/INTEGRITY-TRIAGE-REPORT.md as its human-readable artifact; the report is this procedure's output
- [migrations](../inventories/migrations.md) — The whole procedure consumes the migration-035 (agent_integrity_flags) trigger output on intelligence_items — that migration defines the flag…
- [SPOT-CHECK-PROCEDURE](./SPOT-CHECK-PROCEDURE.md) — Sister read-only runbook using a canonical-host allowlist and Haiku re-classification; same 'strictly read-only classifier over a flagged sample'…
